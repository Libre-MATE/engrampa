...
static void extract_to_callback(CajaMenuItem *item, gpointer user_data) {
  GList *files, *scan;
  char *default_dir;
  ...
  g_string_append_printf(cmd, " --default-dir=%s --extract",
                         quoted_default_dir);
...
static void extract_here_callback(CajaMenuItem *item, gpointer user_data) {
  GList *files, *scan;
  GString *cmd;
...
static void add_callback(CajaMenuItem *item, gpointer user_data) {
  GList *files, *scan;
  char *uri, *dir;
  char *quoted_dir;
  GString *cmd;
...
} archive_mime_types[] = {{"application/vnd.debian.binary-package", TRUE},
...
static FileMimeInfo get_file_mime_info(CajaFileInfo *file) {
  ...
  for (i = 0; archive_mime_types[i].mime_type != NULL; i++)
    if (caja_file_info_is_mime_type(file, archive_mime_types[i].mime_type)) {
      ...
      content_type_mime_compare =
          g_content_type_from_mime_type(archive_mime_types[i].mime_type);

      file_mime_info.is_archive = TRUE;
      file_mime_info.is_compressed_archive =
          archive_mime_types[i].is_compressed;
      if ((content_type_mime_file != NULL) &&
          (content_type_mime_compare != NULL))
        file_mime_info.is_derived_archive = !g_content_type_equals(
            content_type_mime_file, content_type_mime_compare);
...
static gboolean unsupported_scheme(CajaFileInfo *file) {
  ...
  if (scheme != NULL) {
    ...
    for (i = 0; unsupported[i] != NULL; i++)
      if (strcmp(scheme, unsupported[i]) == 0) result = TRUE;
  }
...
static GList *caja_fr_get_file_items(CajaMenuProvider *provider,
  ...
  if (unsupported_scheme((CajaFileInfo *)files->data)) return NULL;
  ...
  for (scan = files; scan; scan = scan->next) {
    ...
    if (all_archives && !file_mime_info.is_archive) all_archives = FALSE;

    if (all_archives_compressed && file_mime_info.is_archive &&
        !file_mime_info.is_compressed_archive)
      all_archives_compressed = FALSE;

    if (all_archives_derived && file_mime_info.is_archive &&
        !file_mime_info.is_derived_archive)
  ...
  if (all_archives && can_write) {
    ...
    item = caja_menu_item_new(
        "CajaFr::extract_here", _("Extract Here"),
        /* Translators: the current position is the current folder */
        _("Extract the selected archive to the current position"),
        "drive-harddisk");
    g_signal_connect(item, "activate", G_CALLBACK(extract_here_callback),
                     provider);
  ...
  if (all_archives) {
    ...
    item =
        caja_menu_item_new("CajaFr::extract_to", _("Extract To..."),
                           _("Extract the selected archive"), "drive-harddisk");
    g_signal_connect(item, "activate", G_CALLBACK(extract_to_callback),
                     provider);
    g_object_set_data_full(G_OBJECT(item), "files",
                           caja_file_info_list_copy(files),
                           (GDestroyNotify)caja_file_info_list_free);
  ...
  if (!one_compressed_archive || one_derived_archive) {
    ...
    item = caja_menu_item_new(
        "CajaFr::add", _("Compress..."),
        _("Create a compressed archive with the selected objects"),
        "package-x-generic");
    g_signal_connect(item, "activate", G_CALLBACK(add_callback), provider);
    g_object_set_data_full(G_OBJECT(item), "files",
                           caja_file_info_list_copy(files),
                           (GDestroyNotify)caja_file_info_list_free);
...
static void caja_fr_menu_provider_iface_init(CajaMenuProviderIface *iface) {
  iface->get_file_items = caja_fr_get_file_items;
}

static void caja_fr_instance_init(CajaFr *fr) {}

static void caja_fr_class_init(CajaFrClass *class) {
  parent_class = g_type_class_peek_parent(class);
}

static GType fr_type = 0;

GType caja_fr_get_type(void) { return fr_type; }

void caja_fr_register_type(GTypeModule *module) {
  static const GTypeInfo info = {sizeof(CajaFrClass),
                                 (GBaseInitFunc)NULL,
  ...
  static const GInterfaceInfo menu_provider_iface_info = {
      (GInterfaceInitFunc)caja_fr_menu_provider_iface_init, NULL, NULL};

  fr_type = g_type_module_register_type(module, G_TYPE_OBJECT, "CajaEngrampa",
                                        &info, 0);

  g_type_module_add_interface(module, fr_type, CAJA_TYPE_MENU_PROVIDER,
