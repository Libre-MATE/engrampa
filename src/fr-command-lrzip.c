...
static void list__process_line(char *line, gpointer data) {
  ...
  if (!g_str_has_prefix(line, "Decompressed file size:")) return;
...
static void fr_command_lrzip_list(FrCommand *comm) {
...
static void fr_command_lrzip_add(FrCommand *comm, const char *from_file,
...
static void fr_command_lrzip_extract(FrCommand *comm, const char *from_file,
  ...
  if (overwrite) fr_process_add_arg(comm->process, "-f");
...
static const char **fr_command_lrzip_get_mime_types(FrCommand *comm) {
  return lrzip_mime_type;
}

static FrCommandCaps fr_command_lrzip_get_capabilities(FrCommand *comm,
...
static const char *fr_command_lrzip_get_packages(FrCommand *comm,
                                                 const char *mime_type) {
...
static void fr_command_lrzip_init(FrCommand *comm) {
...
GType fr_command_lrzip_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandLrzipClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(FR_TYPE_COMMAND, "FRCommandLrzip", &type_info,
                                  0);
  }
