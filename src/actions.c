...
static void new_archive(DlgNewData *data, char *uri) {
  ...
  new_window = fr_window_archive_is_present(data->window) &&
               !fr_window_is_batch_mode(data->window);
  if (new_window)
...
*/
static gboolean is_supported_extension(GtkWidget *file_sel, char *filename,
                                       int *file_type) {
  int i;
...
static char *get_full_uri(DlgNewData *data) {
  char *full_uri = NULL;
  char *uri;
  const char *filename;
  int idx;
...
static char *get_archive_filename_from_selector(DlgNewData *data) {
  char *uri = NULL;
  GFile *file, *dir;
  GFileInfo *info;
  GError *err = NULL;
  ...
  info = g_file_query_info(dir,
                           ...
                           0, NULL, &err);
  if (err != NULL) {
    g_warning("Failed to get permission for extraction dir: %s", err->message);
    g_clear_error(&err);
    g_object_unref(info);
    g_object_unref(dir);
    g_object_unref(file);
  ...
  if (!g_file_info_get_attribute_boolean(info,
    ...
    dialog = _gtk_error_dialog_new(
        GTK_WINDOW(data->dialog), GTK_DIALOG_DESTROY_WITH_PARENT, NULL,
        _("Could not create the archive"), "%s",
  ...
  /* if the user did not specify a valid extension use the filetype combobox
   * current type or tar.gz if automatic is selected. */
  ...
  if (uri_exists(uri)) {
    if (!is_supported_extension(data->dialog, uri, data->supported_types)) {
      GtkWidget *dialog;
      dialog = _gtk_error_dialog_new(GTK_WINDOW(data->dialog), GTK_DIALOG_MODAL,
                                     NULL, _("Could not create the archive"),
    ...
    if (err != NULL) {
      ...
      dialog = _gtk_error_dialog_new(
          GTK_WINDOW(data->dialog), GTK_DIALOG_DESTROY_WITH_PARENT, NULL,
          _("Could not delete the old archive."), "%s", err->message);
      gtk_dialog_run(GTK_DIALOG(dialog));
      gtk_widget_destroy(GTK_WIDGET(dialog));
      g_error_free(err);
      g_free(uri);
...
static void new_file_response_cb(GtkWidget *w, int response, DlgNewData *data) {
  char *path;
...
static void window_archive_loaded_cb(FrWindow *window, gboolean success,
                                     GtkWidget *file_sel) {
  if (success) {
    g_signal_handlers_disconnect_by_data(window, file_sel);
    gtk_widget_destroy(file_sel);
  } else {
    FrWindow *original_window =
        g_object_get_data(G_OBJECT(file_sel), "fr_window");
    if (window != original_window) fr_window_destroy_with_error_dialog(window);
  }
}

static void open_file_response_cb(GtkWidget *w, int response,
                                  GtkWidget *file_sel) {
  ...
  if ((window == NULL) || (uri == NULL)) return;
...
void activate_action_open(GtkAction *action, gpointer data) {
  ...
  file_sel = gtk_file_chooser_dialog_new(
      _("Open"), GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_OPEN, "gtk-cancel",
      GTK_RESPONSE_CANCEL, "gtk-open", GTK_RESPONSE_OK, NULL);
  gtk_dialog_set_default_response(GTK_DIALOG(file_sel), GTK_RESPONSE_OK);
  gtk_file_chooser_set_local_only(GTK_FILE_CHOOSER(file_sel), FALSE);
  gtk_file_chooser_set_current_folder_uri(
      GTK_FILE_CHOOSER(file_sel), fr_window_get_open_default_dir(window));
...
static void save_file_response_cb(GtkWidget *w, gint response,
                                  DlgNewData *data) {
  char *path;
  const char *password;
  gboolean encrypt_header;
  ...
  if (response == GTK_RESPONSE_HELP) {
    show_help_dialog(GTK_WINDOW(data->dialog),
                     "engrampa-create#engrampa-convert-archive");
    return;
  }
  ...
  fr_window_archive_save_as(data->window, path, password, encrypt_header,
                            volume_size);
  gtk_widget_destroy(data->dialog);
...
void activate_action_save_as(GtkAction *action, gpointer callback_data) {
...
void activate_action_extract_folder_from_sidebar(GtkAction *action,
                                                 gpointer data) {
...
void activate_action_copy_folder_from_sidebar(GtkAction *action,
                                              gpointer data) {
...
void activate_action_paste_folder_from_sidebar(GtkAction *action,
                                               gpointer data) {
  fr_window_paste_selection((FrWindow *)data, TRUE);
}

void activate_action_rename_folder_from_sidebar(GtkAction *action,
                                                gpointer data) {
  fr_window_rename_selection((FrWindow *)data, TRUE);
}

void activate_action_delete_folder_from_sidebar(GtkAction *action,
                                                gpointer data) {
...
void activate_action_open_folder_from_sidebar(GtkAction *action,
                                              gpointer data) {
  FrWindow *window = data;
...
void activate_action_view_statusbar(GtkAction *action, gpointer data) {
  ...
  settings = g_settings_new(ENGRAMPA_SCHEMA_UI);
  g_settings_set_boolean(
      settings, PREF_UI_VIEW_STATUSBAR,
      gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action)));
  g_object_unref(settings);
}
...
void activate_action_sort_reverse_order(GtkAction *action, gpointer data) {
  ...
  fr_window_set_sort_type(
      window, gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action))
                  ? GTK_SORT_DESCENDING
                  : GTK_SORT_ASCENDING);
}
...
void activate_action_about(GtkAction *action, gpointer gp) {
  FrWindow *window = gp;
  ...
  const char *license[] = {
      ...
      N_("You should have received a copy of the GNU General Public License "
         "along with Engrampa; if not, write to the Free Software Foundation, "
         "Inc., "
  ...
  bytes = g_resources_lookup_data(ENGRAMPA_RESOURCE_UI_PATH G_DIR_SEPARATOR_S
                                  "engrampa.about",
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(window), "version", VERSION, "copyright",
      _("Copyright \xc2\xa9 2001–2010 Free Software Foundation, Inc.\n"
        "Copyright \xc2\xa9 2012–2021 The MATE developers"),
      "comments",
      _("Engrampa is an archive manager for the MATE Desktop Environment."),
      "authors", authors, "documenters", documenters, "translator-credits",
      _("translator-credits"), "logo-icon-name", "engrampa", "license",
      license_text, "wrap-license", TRUE, "website", PACKAGE_URL, NULL);
