...
static GtkTargetEntry folder_tree_targets[] = {{"XdndEngrampa0", 0, 0},
                                               {"XdndDirectSave0", 0, 2}};
...
typedef enum { FR_CLIPBOARD_OP_CUT, FR_CLIPBOARD_OP_COPY } FRClipboardOp;
...
static FrClipboardData *fr_clipboard_data_new(void) {
...
static FrClipboardData *fr_clipboard_data_ref(FrClipboardData *clipboard_data) {
  clipboard_data->refs++;
  return clipboard_data;
}

static void fr_clipboard_data_unref(FrClipboardData *clipboard_data) {
  if (clipboard_data == NULL) return;
  if (--clipboard_data->refs > 0) return;
...
static void fr_clipboard_data_set_password(FrClipboardData *clipboard_data,
                                           const char *password) {
  if (clipboard_data->archive_password != password)
    g_free(clipboard_data->archive_password);
  if (password != NULL) clipboard_data->archive_password = g_strdup(password);
}

/**/

enum { ARCHIVE_LOADED, PROGRESS, READY, LAST_SIGNAL };
...
static void fr_window_free_batch_data(FrWindow *window) {
  GList *scan;
...
static void gh_unref_pixbuf(gpointer key, gpointer value, gpointer user_data) {
  g_object_unref(value);
}

static void fr_window_clipboard_remove_file_list(FrWindow *window,
                                                 GList *file_list) {
  GList *scan1;

  if (window->priv->copy_data == NULL) return;
  ...
  for (scan1 = file_list; scan1; scan1 = scan1->next) {
    ...
    for (scan2 = window->priv->copy_data->files; scan2;) {
      ...
      if (strcmp(name1, name2) == 0) {
        ...
        window->priv->copy_data->files =
            g_list_remove_link(window->priv->copy_data->files, scan2);
        g_free(scan2->data);
        g_list_free(scan2);
        scan2 = tmp;
      } else
...
static void fr_window_history_clear(FrWindow *window) {
  if (window->priv->history != NULL) path_list_free(window->priv->history);
...
static void fr_window_free_open_files(FrWindow *window) {
  GList *scan;

  for (scan = window->priv->open_files; scan; scan = scan->next) {
    OpenFile *file = scan->data;

    if (file->monitor != NULL) g_file_monitor_cancel(file->monitor);
    open_file_free(file);
  }
...
static void fr_window_convert_data_free(FrWindow *window, gboolean all) {
  if (all) {
...
static void fr_window_free_private_data(FrWindow *window) {
  ...
  while (window->priv->activity_ref > 0) fr_window_stop_activity_mode(window);
  ...
  g_settings_set_enum(window->priv->settings_listing, PREF_LISTING_SORT_METHOD,
                      window->priv->sort_method);
  g_settings_set_enum(window->priv->settings_listing, PREF_LISTING_SORT_TYPE,
...
static void fr_window_finalize(GObject *object) {
  ...
  if (gtk_application_get_windows(
          GTK_APPLICATION(g_application_get_default())) == NULL) {
    if (tree_pixbuf_hash != NULL) {
      g_hash_table_foreach(tree_pixbuf_hash, gh_unref_pixbuf, NULL);
...
static gboolean close__step2(gpointer data) {
  gtk_widget_destroy(GTK_WIDGET(data));
  return FALSE;
}

void fr_window_close(FrWindow *window) {
  if (window->priv->activity_ref > 0) return;
  ...
  if (gtk_widget_get_realized(GTK_WIDGET(window))) {
    ...
    g_settings_set_int(window->priv->settings_ui, PREF_UI_WINDOW_HEIGHT,
                       height);

    width = gtk_paned_get_position(GTK_PANED(window->priv->paned));
    if (width > 0)
      g_settings_set_int(window->priv->settings_ui, PREF_UI_SIDEBAR_WIDTH,
                         width);

    width = gtk_tree_view_column_get_width(window->priv->filename_column);
    if (width > 0)
      g_settings_set_int(window->priv->settings_listing,
                         PREF_LISTING_NAME_COLUMN_WIDTH, width);
  }

  g_idle_add(close__step2, window);
}

static void fr_window_class_init(FrWindowClass *class) {
  GObjectClass *gobject_class;

  parent_class = g_type_class_peek_parent(class);

  fr_window_signals[ARCHIVE_LOADED] = g_signal_new(
      "archive-loaded", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FrWindowClass, archive_loaded), NULL, NULL,
      fr_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
  fr_window_signals[PROGRESS] =
      g_signal_new("progress", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrWindowClass, progress), NULL, NULL,
                   fr_marshal_VOID__DOUBLE_STRING, G_TYPE_NONE, 2,
                   G_TYPE_DOUBLE, G_TYPE_STRING);
  fr_window_signals[READY] =
      g_signal_new("ready", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrWindowClass, ready), NULL, NULL,
                   fr_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  gobject_class = (GObjectClass *)class;
  gobject_class->finalize = fr_window_finalize;
}

static void fr_window_update_paste_command_sensitivity(FrWindow *,
                                                       GtkClipboard *);

static void clipboard_owner_change_cb(GtkClipboard *clipboard, GdkEvent *event,
                                      gpointer user_data) {
  fr_window_update_paste_command_sensitivity((FrWindow *)user_data, clipboard);
}

static void fr_window_realized(GtkWidget *window, gpointer *data) {
  GtkClipboard *clipboard;

  clipboard = gtk_widget_get_clipboard(window, FR_CLIPBOARD);
  g_signal_connect(clipboard, "owner_change",
                   G_CALLBACK(clipboard_owner_change_cb), window);
}

static void fr_window_unrealized(GtkWidget *window, gpointer *data) {
  GtkClipboard *clipboard;

  clipboard = gtk_widget_get_clipboard(window, FR_CLIPBOARD);
  g_signal_handlers_disconnect_by_func(
      clipboard, G_CALLBACK(clipboard_owner_change_cb), window);
}

static void fr_window_init(FrWindow *window) {
  ...
  g_signal_connect(window, "realize", G_CALLBACK(fr_window_realized), NULL);
  g_signal_connect(window, "unrealize", G_CALLBACK(fr_window_unrealized), NULL);
}
...
GType fr_window_get_type(void) {
  ...
  if (!type) {
    GTypeInfo type_info = {sizeof(FrWindowClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(GTK_TYPE_APPLICATION_WINDOW, "FrWindow",
                                  &type_info, 0);
  }
...
static void fr_window_history_add(FrWindow *window, const char *path) {
  if ((window->priv->history_current == NULL) ||
    ...
    if (new_current != NULL) {
      ...
      for (scan = window->priv->history;
           scan && (scan != window->priv->history_current);
           /* void */) {
      ...
      window->priv->history =
          g_list_prepend(window->priv->history, g_strdup(path));
      window->priv->history_current = window->priv->history;
    }
  }
}

static void fr_window_history_pop(FrWindow *window) {
  GList *first;
...
static GPtrArray *fr_window_get_current_dir_list(FrWindow *window) {
  GPtrArray *files;
  guint i;
  ...
  for (i = 0; i < window->archive->command->files->len; i++) {
    ...
    if (fdata->list_name == NULL) continue;
...
static gint sort_by_name(gconstpointer ptr1, gconstpointer ptr2) {
...
static gint sort_by_size(gconstpointer ptr1, gconstpointer ptr2) {
  FileData *fdata1 = *((FileData **)ptr1);
  FileData *fdata2 = *((FileData **)ptr2);
...
static gint sort_by_type(gconstpointer ptr1, gconstpointer ptr2) {
  FileData *fdata1 = *((FileData **)ptr1);
  FileData *fdata2 = *((FileData **)ptr2);
  int result;
  char *desc1, *desc2;
...
static gint sort_by_time(gconstpointer ptr1, gconstpointer ptr2) {
  FileData *fdata1 = *((FileData **)ptr1);
  FileData *fdata2 = *((FileData **)ptr2);
...
static gint sort_by_path(gconstpointer ptr1, gconstpointer ptr2) {
  FileData *fdata1 = *((FileData **)ptr1);
  FileData *fdata2 = *((FileData **)ptr2);
  int result;
...
static guint64 get_dir_size(FrWindow *window, const char *current_dir,
                            const char *name) {
  guint64 size;
  char *dirname;
  int dirname_l;
  ...
  for (i = 0; i < window->archive->command->files->len; i++) {
    ...
    if (strncmp(dirname, fd->full_path, dirname_l) == 0) size += fd->size;
  }
...
static gboolean file_data_respects_filter(FrWindow *window, FileData *fdata) {
  const char *filter;

  filter = gtk_entry_get_text(GTK_ENTRY(window->priv->filter_entry));
  if ((fdata == NULL) || (filter == NULL) || (*filter == '\0')) return TRUE;

  if (fdata->dir || (fdata->name == NULL)) return FALSE;

  return strncasecmp(fdata->name, filter, strlen(filter)) == 0;
}

static gboolean compute_file_list_name(FrWindow *window, FileData *fdata,
  ...
  *different_name = FALSE;
  ...
  if (!file_data_respects_filter(window, fdata)) return FALSE;

  if (window->priv->list_mode == FR_WINDOW_LIST_MODE_FLAT) {
    if (!(fdata->dir)) file_data_set_list_name(fdata, fdata->name);
    return FALSE;
  }
  ...
  if (strlen(fdata->full_path) == current_dir_len) return FALSE;
  ...
  if ((end == NULL) && !fdata->dir) { /* file */
    ...
    if ((end != NULL) && (*(end + 1) != '\0')) fdata->list_dir = TRUE;
...
static void fr_window_compute_list_names(FrWindow *window, GPtrArray *files) {
  const char *current_dir;
  size_t current_dir_len;
  GHashTable *names_hash;
  guint i;
  ...
  for (i = 0; i < files->len; i++) {
    ...
    if (visible_list_completed) continue;

    if (compute_file_list_name(window, fdata, current_dir, current_dir_len,
                               names_hash, &different_name)) {
...
static gboolean fr_window_dir_exists_in_archive(FrWindow *window,
                                                const char *dir_name) {
  int dir_name_len;
  guint i;
...
static char *get_parent_dir(const char *current_dir) {
  char *dir;
  char *new_dir;
  char *retval;

  if (current_dir == NULL) return NULL;
  if (strcmp(current_dir, "/") == 0) return g_strdup("/");
...
static GdkPixbuf *get_mime_type_icon(const char *mime_type) {
...
static GdkPixbuf *get_icon(FileData *fdata) {
...
static GdkPixbuf *get_emblem(FileData *fdata) {
  if (!fdata->encrypted) return NULL;

  return get_mime_type_icon("emblem-nowrite");
}

static int get_column_from_sort_method(FrWindowSortMethod sort_method) {
  switch (sort_method) {
...
static int get_sort_method_from_column(int column_id) {
  switch (column_id) {
    case COLUMN_NAME:
...
static void add_selected_from_list_view(GtkTreeModel *model, GtkTreePath *path,
...
static void add_selected_from_tree_view(GtkTreeModel *model, GtkTreePath *path,
                                        GtkTreeIter *iter, gpointer data) {
  GList **list = data;
  char *dir_path;

  gtk_tree_model_get(model, iter, TREE_COLUMN_PATH, &dir_path, -1);
  *list = g_list_prepend(*list, dir_path);
}

static void add_selected_fd(GtkTreeModel *model, GtkTreePath *path,
                            GtkTreeIter *iter, gpointer data) {
  GList **list = data;
  FileData *fdata;

  gtk_tree_model_get(model, iter, COLUMN_FILE_DATA, &fdata, -1);
  if (!fdata->list_dir) *list = g_list_prepend(*list, fdata);
}

static GList *get_selection_as_fd(FrWindow *window) {
  GtkTreeSelection *selection;
  GList *list = NULL;

  if (!gtk_widget_get_realized(window->priv->list_view)) return NULL;
...
static void fr_window_update_statusbar_list_info(FrWindow *window) {
  ...
  if (window->priv->archive_present) {
    ...
    for (scan = selection; scan; scan = scan->next) {
      ...
      sel_n++;
      if (!file_data_is_dir(fd)) sel_size += fd->size;
    }
  ...
  if (tot_n == 0)
    ...
    archive_info =
        g_strdup_printf(g_dngettext(GETTEXT_PACKAGE, "%lu object (%s)",
                                    "%lu objects (%s)", tot_n),
                        tot_n, size_txt);
  ...
  if (sel_n == 0)
    ...
    selected_info =
        g_strdup_printf(g_dngettext(GETTEXT_PACKAGE, "%lu object selected (%s)",
                                    "%lu objects selected (%s)", sel_n),
                        sel_n, sel_size_txt);

  info = g_strconcat(archive_info, ((sel_n == 0) ? NULL : ", "), selected_info,
                     NULL);
...
static void fr_window_populate_file_list(FrWindow *window, GPtrArray *files) {
  guint i;

  gtk_list_store_clear(window->priv->list_store);

  gtk_tree_sortable_set_sort_column_id(
      GTK_TREE_SORTABLE(window->priv->list_store),
  ...
  for (i = 0; i < files->len; i++) {
    ...
    if (file_data_is_dir(fdata)) {
      ...
      gtk_list_store_set(window->priv->list_store, &iter, COLUMN_FILE_DATA,
                         fdata, COLUMN_ICON, icon, COLUMN_NAME, utf8_name,
                         COLUMN_EMBLEM, emblem, COLUMN_TYPE, _("Folder"),
                         COLUMN_SIZE, s_size, COLUMN_TIME, s_time, COLUMN_PATH,
                         utf8_path, -1);
      g_free(utf8_path);
      g_free(s_size);
      g_free(s_time);
      ...
      gtk_list_store_set(window->priv->list_store, &iter, COLUMN_FILE_DATA,
                         fdata, COLUMN_ICON, icon, COLUMN_NAME, utf8_name,
                         COLUMN_EMBLEM, emblem, COLUMN_TYPE, desc, COLUMN_SIZE,
                         s_size, COLUMN_TIME, s_time, COLUMN_PATH, utf8_path,
                         -1);
      g_free(utf8_path);
      g_free(s_size);
      g_free(s_time);
    ...
    if (emblem != NULL) g_object_unref(emblem);
  }
...
static gboolean get_tree_iter_from_path(FrWindow *window, const char *path,
                                        GtkTreeIter *parent,
                                        GtkTreeIter *iter) {
  gboolean result = FALSE;

  if (!gtk_tree_model_iter_children(GTK_TREE_MODEL(window->priv->tree_store),
                                    iter, parent))
    return FALSE;
  ...
  do {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(window->priv->tree_store), iter,
                       TREE_COLUMN_PATH, &iter_path, -1);
    ...
    g_free(iter_path);
      ...
      gtk_tree_model_iter_next(GTK_TREE_MODEL(window->priv->tree_store), iter));

  return result;
}

static void set_sensitive(FrWindow *window, const char *action_name,
                          gboolean sensitive) {
  GtkAction *action;
...
static void fr_window_update_current_location(FrWindow *window) {
  ...
  gtk_entry_set_text(GTK_ENTRY(window->priv->location_entry),
                     window->priv->archive_present ? current_dir : "");

  set_sensitive(window, "GoBack",
                window->priv->archive_present && (current_dir != NULL) &&
                    (window->priv->history_current != NULL) &&
                    (window->priv->history_current->next != NULL));
  set_sensitive(window, "GoForward",
                window->priv->archive_present && (current_dir != NULL) &&
                    (window->priv->history_current != NULL) &&
                    (window->priv->history_current->prev != NULL));
  set_sensitive(window, "GoUp",
                window->priv->archive_present && (current_dir != NULL) &&
  ...
  if (get_tree_iter_from_path(window, path, NULL, &iter)) {
    ...
    t_path = gtk_tree_model_get_path(GTK_TREE_MODEL(window->priv->tree_store),
                                     &iter);
    gtk_tree_view_expand_to_path(GTK_TREE_VIEW(window->priv->tree_view),
                                 t_path);
    gtk_tree_path_free(t_path);

    selection =
        gtk_tree_view_get_selection(GTK_TREE_VIEW(window->priv->tree_view));
...
static void fr_window_update_dir_tree(FrWindow *window) {
  ...
  if (!window->priv->view_folders || !window->priv->archive_present ||
  ...
  for (i = 0; i < window->archive->command->files->len; i++) {
    ...
    if (gtk_entry_get_text(GTK_ENTRY(window->priv->filter_entry)) != NULL) {
      if (!file_data_respects_filter(window, fdata)) continue;
    }
    ...
    while ((dir != NULL) && (strcmp(dir, "/") != 0)) {
      ...
      if (g_hash_table_lookup(dir_cache, dir) != NULL) break;
  ...
  dir_cache = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
  ...
  icon = get_mime_type_icon(MIME_TYPE_ARCHIVE);
  {
    ...
    gtk_tree_store_set(window->priv->tree_store, &node, TREE_COLUMN_ICON, icon,
                       TREE_COLUMN_NAME, name, TREE_COLUMN_PATH, "/",
                       TREE_COLUMN_WEIGHT, PANGO_WEIGHT_BOLD, -1);
    g_hash_table_replace(dir_cache, "/",
                         gtk_tree_model_get_path(
                             GTK_TREE_MODEL(window->priv->tree_store), &node));
...
static void fr_window_update_filter_bar_visibility(FrWindow *window) {
  const char *filter;
...
static void fr_window_update_file_list(FrWindow *window, gboolean update_view) {
  GPtrArray *files;
  ...
  if (!window->priv->archive_present || window->priv->archive_new) {
    if (update_view) gtk_list_store_clear(window->priv->list_store);
  ...
  if (update_view) fr_window_populate_file_list(window, files);

  if (free_files) g_ptr_array_free(files, TRUE);
}
...
static void fr_window_update_title(FrWindow *window) {
  if (!window->priv->archive_present)
...
static void check_whether_has_a_dir(GtkTreeModel *model, GtkTreePath *path,
                                    GtkTreeIter *iter, gpointer data) {
  gboolean *has_a_dir = data;
  FileData *fdata;

  gtk_tree_model_get(model, iter, COLUMN_FILE_DATA, &fdata, -1);
  if (file_data_is_dir(fdata)) *has_a_dir = TRUE;
}

static gboolean selection_has_a_dir(FrWindow *window) {
  GtkTreeSelection *selection;
  gboolean has_a_dir = FALSE;

  if (!gtk_widget_get_realized(window->priv->list_view)) return FALSE;

  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(window->priv->list_view));
  if (selection == NULL) return FALSE;

  gtk_tree_selection_selected_foreach(selection, check_whether_has_a_dir,
                                      &has_a_dir);

  return has_a_dir;
}

static void set_active(FrWindow *window, const char *action_name,
                       gboolean is_active) {
  GtkAction *action;
...
static void fr_window_update_paste_command_sensitivity(
    FrWindow *window, GtkClipboard *clipboard) {
  gboolean running;
  gboolean no_archive;
  gboolean ro;
  gboolean compr_file;
  ...
  set_sensitive(window, "Paste",
                !no_archive && !ro && !running && !compr_file &&
                    ...
                    gtk_clipboard_wait_is_target_available(
                        clipboard, FR_SPECIAL_URI_LIST));
}

static void fr_window_update_sensitivity(FrWindow *window) {
  gboolean no_archive;
  gboolean ro;
  gboolean file_op;
  gboolean running;
  ...
  if (window->priv->batch_mode) return;
  ...
  set_sensitive(window, "AddFiles",
                !no_archive && !ro && !running && !compr_file);
  set_sensitive(window, "AddFiles_Toolbar",
                !no_archive && !ro && !running && !compr_file);
  set_sensitive(window, "AddFolder",
                !no_archive && !ro && !running && !compr_file);
  set_sensitive(window, "AddFolder_Toolbar",
                !no_archive && !ro && !running && !compr_file);
  set_sensitive(window, "Copy",
                !no_archive && !ro && !running && !compr_file && sel_not_null &&
                    (window->priv->list_mode != FR_WINDOW_LIST_MODE_FLAT));
  set_sensitive(window, "Cut",
                !no_archive && !ro && !running && !compr_file && sel_not_null &&
                    (window->priv->list_mode != FR_WINDOW_LIST_MODE_FLAT));
  set_sensitive(window, "Delete",
                !no_archive && !ro && !window->priv->archive_new && !running &&
                    !compr_file);
  ...
  set_sensitive(
      window, "LastOutput",
      ((window->archive != NULL) && (window->archive->process != NULL) &&
  ...
  set_sensitive(window, "OpenSelection",
                file_op && sel_not_null && !dir_selected);
  set_sensitive(window, "OpenFolder",
                file_op && one_file_selected && dir_selected);
  set_sensitive(
      window, "Password",
      !running && (window->priv->asked_for_password ||
                   (!no_archive && window->archive->command->propPassword)));
  ...
  set_sensitive(
      window, "Rename",
      !no_archive && !ro && !running && !compr_file && one_file_selected);
  ...
  set_sensitive(window, "TestArchive",
                !no_archive && !running && window->archive->command->propTest);
  set_sensitive(window, "ViewSelection",
                file_op && one_file_selected && !dir_selected);
  set_sensitive(window, "ViewSelection_Toolbar",
                file_op && one_file_selected && !dir_selected);
  ...
  set_sensitive(window, "SelectAll",
                (window->priv->current_view_length > 0) &&
  ...
  set_sensitive(window, "ViewFolders",
                (window->priv->list_mode == FR_WINDOW_LIST_MODE_AS_DIR));

  set_sensitive(window, "ViewAllFiles", !window->priv->filter_mode);
  set_sensitive(window, "ViewAsFolder", !window->priv->filter_mode);
}

static gboolean location_entry_key_press_event_cb(GtkWidget *widget,
                                                  GdkEventKey *event,
                                                  FrWindow *window) {
  if ((event->keyval == GDK_KEY_Return) ||
      (event->keyval == GDK_KEY_KP_Enter) ||
      (event->keyval == GDK_KEY_ISO_Enter)) {
    fr_window_go_to_location(
        window, gtk_entry_get_text(GTK_ENTRY(window->priv->location_entry)),
        FALSE);
  }
...
static gboolean real_close_progress_dialog(gpointer data) {
...
static void close_progress_dialog(FrWindow *window, gboolean close_now) {
  if (window->priv->progress_timeout != 0) {
    g_source_remove(window->priv->progress_timeout);
    window->priv->progress_timeout = 0;
  }
  ...
  if (window->priv->progress_dialog == NULL) return;
  ...
  if (close_now) {
    ...
    if (window->priv->hide_progress_timeout != 0) return;
    window->priv->hide_progress_timeout = g_timeout_add(
        HIDE_PROGRESS_TIMEOUT_MSECS, real_close_progress_dialog, window);
  }
  close_suspend_process(window);
}

static gboolean progress_dialog_delete_event(GtkWidget *caller, GdkEvent *event,
                                             FrWindow *window) {
...
static void open_folder(GtkWindow *parent, const char *folder) {
  GError *error = NULL;
  ...
  if (!gtk_show_uri_on_window(parent, folder, GDK_CURRENT_TIME, &error)) {
    ...
    message =
        g_strdup_printf(_("Could not display the folder \"%s\""), utf8_name);
    g_free(utf8_name);

    d = _gtk_error_dialog_new(parent, GTK_DIALOG_MODAL, NULL, message, "%s",
                              error->message);
...
static void fr_window_view_extraction_destination_folder(FrWindow *window) {
  open_folder(GTK_WINDOW(window),
              fr_archive_get_last_extraction_destination(window->archive));
}

static void change_button_label(FrWindow *window, GtkWidget *button) {
  const gchar *state;
...
static void progress_dialog_response(GtkDialog *dialog, int response_id,
                                     FrWindow *window) {
...
static char *get_action_description(FrAction action, const char *uri) {
  char *basename;
  char *message;
  ...
  switch (action) {
    ...
    case FR_ACTION_COPYING_FILES_FROM_REMOTE:
      /* Translators: %s is a filename */
      message =
          g_strdup_printf(_("Copying the files to add to \"%s\""), basename);
      break;
...
static void progress_dialog_update_action_description(FrWindow *window) {
  ...
  description = get_action_description(window->priv->action,
                                       window->priv->pd_last_archive);
  description_markup = g_markup_printf_escaped(
      "<span weight=\"bold\" size=\"larger\">%s</span>", description);
...
static gboolean fr_window_working_archive_cb(FrCommand *command,
...
static gboolean fr_window_message_cb(FrCommand *command, const char *msg,
                                     FrWindow *window) {
  ...
  if (window->priv->progress_dialog == NULL) return TRUE;
  ...
  if (msg != NULL) {
    ...
    g_signal_emit(G_OBJECT(window), fr_window_signals[PROGRESS], 0,
...
static void create_the_progress_dialog(FrWindow *window) {
  GtkWindow *parent;
  ...
  window->priv->progress_dialog = gtk_dialog_new_with_buttons(
      (window->priv->batch_mode ? window->priv->batch_title : NULL), parent,
      flags, NULL, NULL);

  window->priv->pd_quit_button = dialog_add_button_with_icon_name(
      GTK_DIALOG(window->priv->progress_dialog), _("_Quit"), "application-exit",
      DIALOG_RESPONSE_QUIT);
  window->priv->pd_open_archive_button = gtk_dialog_add_button(
      GTK_DIALOG(window->priv->progress_dialog), _("_Open the Archive"),
      DIALOG_RESPONSE_OPEN_ARCHIVE);
  window->priv->pd_open_destination_button = gtk_dialog_add_button(
      GTK_DIALOG(window->priv->progress_dialog), _("_Show the Files"),
      DIALOG_RESPONSE_OPEN_DESTINATION_FOLDER);
  window->priv->pd_open_destination_and_quit_button = gtk_dialog_add_button(
      GTK_DIALOG(window->priv->progress_dialog), _("Show the _Files and Quit"),
      DIALOG_RESPONSE_OPEN_DESTINATION_FOLDER_AND_QUIT);
  window->priv->pd_close_button = dialog_add_button_with_icon_name(
      GTK_DIALOG(window->priv->progress_dialog), _("_Close"), "window-close",
      GTK_RESPONSE_CLOSE);
  window->priv->pd_cancel_button = dialog_add_button_with_icon_name(
      GTK_DIALOG(window->priv->progress_dialog), _("_Cancel"), "process-stop",
      GTK_RESPONSE_CANCEL);
  /*add start button default suspend*/
  window->priv->pd_state_button = dialog_add_button_with_icon_name(
      GTK_DIALOG(window->priv->progress_dialog), _("_Pause"),
      "media-playback-pause", GTK_RESPONSE_ACCEPT);
  ...
  gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(d)), hbox, FALSE,
                     FALSE, 10);

  icon = get_mime_type_pixbuf(
      "package-x-generic",
      _gtk_widget_lookup_for_size(GTK_WIDGET(window), GTK_ICON_SIZE_DIALOG),
      NULL);
  ...
  gtk_progress_bar_set_pulse_step(
      GTK_PROGRESS_BAR(window->priv->pd_progress_bar), ACTIVITY_PULSE_STEP);
  gtk_box_pack_start(GTK_BOX(progress_vbox), window->priv->pd_progress_bar,
                     TRUE, TRUE, 0);
  ...
  g_signal_connect(G_OBJECT(window->priv->progress_dialog), "response",
                   G_CALLBACK(progress_dialog_response), window);
  g_signal_connect(G_OBJECT(window->priv->progress_dialog), "delete_event",
                   G_CALLBACK(progress_dialog_delete_event), window);
}

static gboolean display_progress_dialog(gpointer data) {
  FrWindow *window = data;
  ...
  if (window->priv->use_progress_dialog &&
    ...
    gtk_dialog_set_response_sensitive(GTK_DIALOG(window->priv->progress_dialog),
                                      GTK_RESPONSE_OK, window->priv->stoppable);
    if (!window->priv->non_interactive) gtk_widget_show(GTK_WIDGET(window));
    gtk_widget_hide(window->priv->progress_bar);
...
static void open_progress_dialog(FrWindow *window, gboolean open_now) {
  if (window->priv->hide_progress_timeout != 0) {
    g_source_remove(window->priv->hide_progress_timeout);
    window->priv->hide_progress_timeout = 0;
  }
  ...
  if ((window->priv->progress_timeout != 0) ||
      ((window->priv->progress_dialog != NULL) &&
       gtk_widget_get_visible(window->priv->progress_dialog)))
    return;
  ...
  if (open_now)
    ...
    window->priv->progress_timeout =
        g_timeout_add(PROGRESS_TIMEOUT_MSECS, display_progress_dialog, window);
}

static gboolean fr_window_progress_cb(FrArchive *archive, double fraction,
                                      FrWindow *window) {
  ...
  if (!window->priv->progress_pulse) {
    ...
    if (window->priv->progress_dialog != NULL)
      gtk_progress_bar_set_fraction(
          GTK_PROGRESS_BAR(window->priv->pd_progress_bar), fraction);
    gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(window->priv->progress_bar),
                                  fraction);

    if ((archive != NULL) && (archive->command != NULL) &&
        (archive->command->n_files > 0)) {
      char *message = NULL;
      gulong remaining_files;

      remaining_files =
          (gulong)(archive->command->n_files - archive->command->n_file + 1);
      ...
      switch (window->priv->action) {
        ...
        case FR_ACTION_DELETING_FILES:
          message = g_strdup_printf(
              g_dngettext(GETTEXT_PACKAGE, "%lu file remaining",
                          "%lu files remaining", remaining_files),
              remaining_files);
          break;
        default:
          break;
      ...
      if (message != NULL) fr_command_message(archive->command, message);
    }
...
static void open_progress_dialog_with_open_destination(FrWindow *window) {
...
static void open_progress_dialog_with_open_archive(FrWindow *window) {
  if (window->priv->hide_progress_timeout != 0) {
    g_source_remove(window->priv->hide_progress_timeout);
    window->priv->hide_progress_timeout = 0;
  }
...
void fr_window_push_message(FrWindow *window, const char *msg) {
  if (!gtk_widget_get_mapped(GTK_WIDGET(window))) return;

  gtk_statusbar_push(GTK_STATUSBAR(window->priv->statusbar),
                     window->priv->progress_cid, msg);
}
...
static void action_started(FrArchive *archive, FrAction action, gpointer data) {
  FrWindow *window = data;
  char *message;
  ...
  switch (action) {
    case FR_ACTION_EXTRACTING_FILES:
      open_progress_dialog(
          window, window->priv->ask_to_open_destination_after_extraction ||
                      window->priv->convert_data.converting ||
...
static void fr_window_add_to_recent_list(FrWindow *window, char *uri) {
  if (window->priv->batch_mode) return;

  if (is_temp_dir(uri)) return;
  ...
  if (window->archive->content_type != NULL) {
    ...
    recent_data->mime_type =
        g_content_type_get_mime_type(window->archive->content_type);
...
static void fr_window_remove_from_recent_list(FrWindow *window,
                                              char *filename) {
  if (filename != NULL)
    gtk_recent_manager_remove_item(gtk_recent_manager_get_default(), filename,
                                   NULL);
}

static void error_dialog_response_cb(GtkDialog *dialog, gint arg1,
                                     gpointer user_data) {
  ...
  if ((dialog_parent != NULL) &&
      (gtk_widget_get_toplevel(GTK_WIDGET(dialog_parent)) !=
...
static void fr_window_show_error_dialog(FrWindow *window, GtkWidget *dialog,
  ...
  if (window->priv->batch_mode && !window->priv->use_progress_dialog) {
    ...
    error = g_error_new_literal(
        FR_ERROR, FR_PROC_ERROR_GENERIC,
        details ? details : _("Command exited abnormally."));
    g_signal_emit(window, fr_window_signals[READY], 0, error);
  ...
  if (window->priv->batch_mode) fr_window_destroy_with_error_dialog(window);

  if (dialog_parent != NULL) gtk_window_set_modal(dialog_parent, FALSE);
  g_signal_connect(dialog, "response", G_CALLBACK(error_dialog_response_cb),
                   window);
...
static gboolean handle_errors(FrWindow *window, FrArchive *archive,
                              FrAction action, FrProcError *error) {
  if (error->type == FR_PROC_ERROR_ASK_PASSWORD) {
    ...
    if ((action == FR_ACTION_LISTING_CONTENT) ||
        (action == FR_ACTION_LOADING_ARCHIVE))
    ...
    dialog = _gtk_error_dialog_new(dialog_parent, 0, output, msg,
                                   ((details != NULL) ? "%s" : NULL), details);
...
static void convert__action_performed(FrArchive *archive, FrAction action,
                                      FrProcError *error, gpointer data) {
  FrWindow *window = data;
  ...
  if ((action == FR_ACTION_GETTING_FILE_LIST) ||
      (action == FR_ACTION_ADDING_FILES)) {
    fr_window_stop_activity_mode(window);
    fr_window_pop_message(window);
    close_progress_dialog(window, FALSE);
  }

  if (action != FR_ACTION_ADDING_FILES) return;
...
static void action_performed(FrArchive *archive, FrAction action,
                             FrProcError *error, gpointer data) {
  FrWindow *window = data;
  ...
  switch (action) {
    ...
    case FR_ACTION_LOADING_ARCHIVE:
      ...
      g_signal_emit(window, fr_window_signals[ARCHIVE_LOADED], 0,
    ...
    case FR_ACTION_LISTING_CONTENT:
      ...
      fr_window_update_title(window);
      fr_window_go_to_location(window, fr_window_get_current_location(window),
                               TRUE);
      fr_window_update_dir_tree(window);
    ...
    case FR_ACTION_ADDING_FILES:
      ...
      if (error->type == FR_PROC_ERROR_NONE) {
        if (window->priv->archive_new) window->priv->archive_new = FALSE;
        fr_window_add_to_recent_list(window, window->priv->archive_uri);
      }
    ...
    case FR_ACTION_EXTRACTING_FILES:
      ...
      if (window->priv->convert_data.converting) {
        ...
        source_dir =
            g_filename_to_uri(window->priv->convert_data.temp_dir, NULL, NULL);
        fr_archive_add_with_wildcard(
            window->priv->convert_data.new_archive, "*", NULL, NULL, source_dir,
            NULL, FALSE, TRUE, window->priv->convert_data.password,
            window->priv->convert_data.encrypt_header,
            window->priv->compression, window->priv->convert_data.volume_size);
        g_free(source_dir);
      } else {
...
static GList *get_dir_list_from_path(FrWindow *window, char *path) {
  char *dirname;
  int dirname_l;
  GList *list = NULL;
  guint i;
  ...
  for (i = 0; i < window->archive->command->files->len; i++) {
    ...
    if (fd->dir) {
      ...
      if ((full_path_l == dirname_l - 1) &&
          (strncmp(dirname, fd->full_path, full_path_l) == 0))
        /* example: dirname is '/path/to/dir/' and fd->full_path is
         * '/path/to/dir' */
        matches = TRUE;
...
static GList *get_dir_list_from_file_data(FrWindow *window, FileData *fdata) {
  char *dirname;
  GList *list;
...
GList *fr_window_get_file_list_selection(FrWindow *window, gboolean recursive,
                                         gboolean *has_dirs) {
  GtkTreeSelection *selection;
  ...
  if (has_dirs != NULL) *has_dirs = FALSE;

  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(window->priv->list_view));
  if (selection == NULL) return NULL;
  gtk_tree_selection_selected_foreach(selection, add_selected_from_list_view,
                                      &selections);
  ...
  for (scan = selections; scan; scan = scan->next) {
    ...
    if (file_data_is_dir(fd)) {
      if (has_dirs != NULL) *has_dirs = TRUE;
  ...
  if (selections) g_list_free(selections);

  return g_list_reverse(list);
}

GList *fr_window_get_folder_tree_selection(FrWindow *window, gboolean recursive,
                                           gboolean *has_dirs) {
  ...
  tree_selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(window->priv->tree_view));
  if (tree_selection == NULL) return NULL;

  selections = NULL;
  gtk_tree_selection_selected_foreach(tree_selection,
                                      add_selected_from_tree_view, &selections);
...
GList *fr_window_get_file_list_from_path_list(FrWindow *window,
  ...
  if (selections != NULL) g_list_free(selections);

  return g_list_reverse(list);
}

GList *fr_window_get_file_list_pattern(FrWindow *window, const char *pattern) {
  GRegex **regexps;
  GList *list;
  guint i;
...
static GList *fr_window_get_file_list(FrWindow *window) {
  GList *list;
  guint i;
...
static int dir_tree_button_press_cb(GtkWidget *widget, GdkEventButton *event,
                                    gpointer data) {
  FrWindow *window = data;
  GtkTreeSelection *selection;

  if (event->window !=
      gtk_tree_view_get_bin_window(GTK_TREE_VIEW(window->priv->tree_view)))
    return FALSE;
  ...
  if ((event->type == GDK_BUTTON_PRESS) && (event->button == 3)) {
    ...
    if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(window->priv->tree_view),
                                      ...
                                      NULL, NULL)) {
      if (!gtk_tree_model_get_iter(GTK_TREE_MODEL(window->priv->tree_store),
                                   &iter, path)) {
      ...
      gtk_menu_popup_at_pointer(
          GTK_MENU(window->priv->sidebar_folder_popup_menu),
...
static FileData *fr_window_get_selected_item_from_file_list(FrWindow *window) {
  GtkTreeSelection *tree_selection;
  GList *selection;
  FileData *fdata = NULL;
  ...
  selection = NULL;
  gtk_tree_selection_selected_foreach(tree_selection,
                                      add_selected_from_list_view, &selection);
...
static char *fr_window_get_selected_folder_in_tree_view(FrWindow *window) {
  GtkTreeSelection *tree_selection;
  GList *selections;
  char *path = NULL;
...
void fr_window_current_folder_activated(FrWindow *window,
  ...
  if (!from_sidebar) {
    ...
    dir_path = g_strconcat(fr_window_get_current_location(window), dir_name,
                           "/", NULL);
    g_free(dir_name);
    file_data_free(fdata);
  } else
...
static gboolean row_activated_cb(GtkTreeView *tree_view, GtkTreePath *path,
...
static int file_button_press_cb(GtkWidget *widget, GdkEventButton *event,
                                gpointer data) {
  FrWindow *window = data;
  GtkTreeSelection *selection;
...
static int file_button_release_cb(GtkWidget *widget, GdkEventButton *event,
                                  gpointer data) {
  FrWindow *window = data;
  GtkTreeSelection *selection;
  ...
  if (window->priv->path_clicked == NULL) return FALSE;
...
static gboolean file_motion_notify_callback(GtkWidget *widget,
  ...
  if (!window->priv->single_click) return FALSE;
  ...
  gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), (int)event->x,
                                (int)event->y, &window->priv->list_hover_path,
                                NULL, NULL, NULL);
  ...
  if (!(last_hover_path == NULL && window->priv->list_hover_path == NULL) &&
    ...
    if (last_hover_path) {
      gtk_tree_model_get_iter(GTK_TREE_MODEL(window->priv->list_store), &iter,
                              last_hover_path);
...
static gboolean file_leave_notify_callback(GtkWidget *widget,
...
static GList *get_uri_list_from_selection_data(char *uri_list) {
  GList *list = NULL;
  char **uris;
  int i;

  if (uri_list == NULL) return NULL;
...
static gboolean fr_window_drag_motion(GtkWidget *widget,
...
static void fr_window_paste_from_clipboard_data(FrWindow *window,
                                                FrClipboardData *data);

static FrClipboardData *get_clipboard_data_from_selection_data(
    FrWindow *window, const char *data) {
  FrClipboardData *clipboard_data;
  char **uris;
  int i;
  ...
  if (window->priv->password_for_paste != NULL)
    clipboard_data->archive_password =
        g_strdup(window->priv->password_for_paste);
  else if (strcmp(uris[1], "") != 0)
    clipboard_data->archive_password = g_strdup(uris[1]);
  clipboard_data->op = (strcmp(uris[2], "copy") == 0) ? FR_CLIPBOARD_OP_COPY
                                                      : FR_CLIPBOARD_OP_CUT;
  clipboard_data->base_dir = g_strdup(uris[3]);
  for (i = 4; uris[i] != NULL; i++)
    if (uris[i][0] != '\0')
      clipboard_data->files =
          g_list_prepend(clipboard_data->files, g_strdup(uris[i]));
...
static void fr_window_drag_data_received(GtkWidget *widget,
  ...
  if (gtk_selection_data_get_target(data) == XFR_ATOM) {
    ...
    dnd_data = get_clipboard_data_from_selection_data(
        window, (char *)gtk_selection_data_get_data(data));
  ...
  if (window->priv->archive_present && (window->archive != NULL) &&
      !window->archive->read_only && !window->archive->is_compressed_file) {
    if (one_file && is_an_archive) {
      GtkWidget *d;
      gint r;

      d = _gtk_message_dialog_new(
          GTK_WINDOW(window), GTK_DIALOG_MODAL, "dialog-question",
          _("Do you want to add this file to the current archive or open it as "
            "a new archive?"),
          NULL, "gtk-cancel", GTK_RESPONSE_CANCEL, "gtk-add", 0, "gtk-open", 1,
          NULL);
    ...
    if (one_file && is_an_archive)
      ...
      d = _gtk_message_dialog_new(
          GTK_WINDOW(window), GTK_DIALOG_MODAL, "dialog-question",
          _("Do you want to create a new archive with these files?"), NULL,
          "gtk-cancel", GTK_RESPONSE_CANCEL, _("Create _Archive"),
      ...
      if (r == GTK_RESPONSE_YES) {
        ...
        fr_window_free_batch_data(window);
        fr_window_append_batch_action(window, FR_BATCH_ACTION_ADD,
                                      path_list_dup(list),
                                      (GFreeFunc)path_list_free);

        first_item = (char *)list->data;
        folder = remove_level_from_path(first_item);
        if (folder != NULL) fr_window_set_open_default_dir(window, folder);
        ...
        if ((list->next != NULL) && (folder != NULL)) {
          ...
          if (uri_is_local(first_item)) {
            ...
            if (!utf8_path) utf8_path = g_strdup(first_item);
            g_free(local_path);
          } else {
...
static gboolean file_list_drag_begin(GtkWidget *widget, GdkDragContext *context,
                                     gpointer data) {
  FrWindow *window = data;

  debug(DEBUG_INFO, "::DragBegin -->\n");

  if (window->priv->activity_ref > 0) return FALSE;
...
static void file_list_drag_end(GtkWidget *widget, GdkDragContext *context,
                               gpointer data) {
  FrWindow *window = data;
  ...
  if (window->priv->drag_error != NULL) {
    _gtk_error_dialog_run(GTK_WINDOW(window), _("Extraction not performed"),
                          "%s", window->priv->drag_error->message);
    g_clear_error(&window->priv->drag_error);
  } else if (window->priv->drag_destination_folder != NULL) {
    fr_window_archive_extract(window, window->priv->drag_file_list,
                              window->priv->drag_destination_folder,
                              window->priv->drag_base_dir, FALSE,
                              FR_OVERWRITE_ASK, FALSE, FALSE);
    path_list_free(window->priv->drag_file_list);
    window->priv->drag_file_list = NULL;
  }
...
static char *get_xds_atom_value(GdkDragContext *context) {
  ...
  g_return_val_if_fail(gdk_drag_context_get_source_window(context) != NULL,
                       NULL);

  if (gdk_property_get(gdk_drag_context_get_source_window(context), XDS_ATOM,
                       TEXT_ATOM, 0, MAX_XDS_ATOM_VAL_LEN, FALSE, NULL, NULL,
                       &actual_length, (unsigned char **)&data)) {
...
static gboolean caja_xds_dnd_is_valid_xds_context(GdkDragContext *context) {
  char *tmp;
  gboolean ret;
...
static char *get_selection_data_from_clipboard_data(FrWindow *window,
                                                    FrClipboardData *data) {
  GString *list;
  char *local_filename;
  GList *scan;
...
static gboolean fr_window_folder_tree_drag_data_get(
  ...
  if (gtk_selection_data_get_target(selection_data) == XFR_ATOM) {
    ...
    gtk_selection_data_set(selection_data, XFR_ATOM, 8, (guchar *)data,
                           strlen(data));
  ...
  if (!caja_xds_dnd_is_valid_xds_context(context)) return FALSE;
  ...
  if (!check_permissions(destination_folder, R_OK | W_OK)) {
    ...
    destination_folder_display_name =
        g_filename_display_name(destination_folder);
    window->priv->drag_error =
        g_error_new(FR_ERROR, 0,
                    _("You don't have the right permissions to extract "
                      "archives in the folder \"%s\""),
                    destination_folder_display_name);
  ...
  if (window->priv->drag_error == NULL) {
    ...
    window->priv->drag_base_dir =
        fr_window_get_selected_folder_in_tree_view(window);
  ...
  gtk_selection_data_set(
      ...
      (guchar *)((window->priv->drag_error == NULL) ? "S" : "E"), 1);
...
gboolean fr_window_file_list_drag_data_get(FrWindow *window,
  ...
  if (window->priv->drag_error == NULL) {
    ...
    window->priv->drag_destination_folder = g_strdup(destination_folder);
    window->priv->drag_base_dir =
        g_strdup(fr_window_get_current_location(window));
    window->priv->drag_file_list =
        fr_window_get_file_list_from_path_list(window, path_list, NULL);
  }
...
static void fr_window_deactivate_filter(FrWindow *window) {
  window->priv->filter_mode = FALSE;
...
static gboolean key_press_cb(GtkWidget *widget, GdkEventKey *event,
  ...
  if (gtk_widget_has_focus(window->priv->location_entry)) return FALSE;
  ...
  switch (event->keyval) {
    case GDK_KEY_Escape:
      activate_action_stop(NULL, window);
      if (window->priv->filter_mode) fr_window_deactivate_filter(window);
      retval = TRUE;
      break;
...
static gboolean dir_tree_selection_changed_cb(GtkTreeSelection *selection,
                                              gpointer user_data) {
  FrWindow *window = user_data;
  GtkTreeIter iter;
...
static void fr_window_delete_event_cb(GtkWidget *caller, GdkEvent *event,
                                      FrWindow *window) {
  fr_window_close(window);
}

static gboolean is_single_click_policy(FrWindow *window) {
  gboolean result = FALSE;
  ...
  if (window->priv->settings_caja) {
    ...
    value =
        g_settings_get_string(window->priv->settings_caja, CAJA_CLICK_POLICY);
...
static void filename_cell_data_func(GtkTreeViewColumn *column,
  ...
  gtk_tree_model_get(model, iter, COLUMN_NAME, &text, -1);
  ...
  g_object_set(G_OBJECT(renderer), "text", text, "underline", underline, NULL);

  g_free(text);
}

static void add_dir_tree_columns(FrWindow *window, GtkTreeView *treeview) {
  GtkCellRenderer *renderer;
  ...
  gtk_tree_view_column_set_attributes(column, renderer, "text",
                                      TREE_COLUMN_NAME, "weight",
                                      TREE_COLUMN_WEIGHT, NULL);
...
static void add_file_list_columns(FrWindow *window, GtkTreeView *treeview) {
  static const char *titles[] = {NC_("File", "Size"), NC_("File", "Type"),
  ...
  gtk_tree_view_column_set_attributes(column, renderer, "pixbuf", COLUMN_EMBLEM,
                                      NULL);
  ...
  w = g_settings_get_int(window->priv->settings_listing,
                         PREF_LISTING_NAME_COLUMN_WIDTH);
  if (w <= 0) w = DEFAULT_NAME_COLUMN_WIDTH;
  gtk_tree_view_column_set_fixed_width(column, w);
  gtk_tree_view_column_set_resizable(column, TRUE);
  gtk_tree_view_column_set_sort_column_id(column, COLUMN_NAME);
  gtk_tree_view_column_set_cell_data_func(
      column, renderer, (GtkTreeCellDataFunc)filename_cell_data_func, window,
      NULL);
  ...
  for (j = 0, i = COLUMN_SIZE; i < NUMBER_OF_COLUMNS; i++, j++) {
    ...
    column = gtk_tree_view_column_new_with_attributes(
        g_dpgettext2(NULL, "File", titles[j]), renderer, "text", i, NULL);
...
static int name_column_sort_func(GtkTreeModel *model, GtkTreeIter *a,
...
static int size_column_sort_func(GtkTreeModel *model, GtkTreeIter *a,
                                 GtkTreeIter *b, gpointer user_data) {
  FileData *fdata1, *fdata2;
...
static int type_column_sort_func(GtkTreeModel *model, GtkTreeIter *a,
                                 GtkTreeIter *b, gpointer user_data) {
  FileData *fdata1, *fdata2;
...
static int time_column_sort_func(GtkTreeModel *model, GtkTreeIter *a,
                                 GtkTreeIter *b, gpointer user_data) {
  FileData *fdata1, *fdata2;
...
static int path_column_sort_func(GtkTreeModel *model, GtkTreeIter *a,
                                 GtkTreeIter *b, gpointer user_data) {
  FileData *fdata1, *fdata2;
...
static int no_sort_column_sort_func(GtkTreeModel *model, GtkTreeIter *a,
                                    GtkTreeIter *b, gpointer user_data) {
  return -1;
}
...
static void sort_column_changed_cb(GtkTreeSortable *sortable,
  ...
  /*set_active (window, get_action_from_sort_method (window->priv->sort_method),
  TRUE); set_active (window, "SortReverseOrder", (window->priv->sort_type ==
  GTK_SORT_DESCENDING));*/
}

static gboolean fr_window_show_cb(GtkWidget *widget, FrWindow *window) {
  fr_window_update_current_location(window);

  set_active(
      window, "ViewToolbar",
      g_settings_get_boolean(window->priv->settings_ui, PREF_UI_VIEW_TOOLBAR));
  set_active(window, "ViewStatusbar",
             g_settings_get_boolean(window->priv->settings_ui,
                                    PREF_UI_VIEW_STATUSBAR));

  window->priv->view_folders =
      g_settings_get_boolean(window->priv->settings_ui, PREF_UI_VIEW_FOLDERS);
...
static void pref_history_len_changed(GSettings *settings, const char *key,
                                     gpointer user_data) {
  FrWindow *window = user_data;
  int limit;
  GtkAction *action;
  ...
  action =
      gtk_action_group_get_action(window->priv->actions, "OpenRecent_Toolbar");
  gtk_recent_chooser_set_limit(GTK_RECENT_CHOOSER(action), limit);
}

static void pref_view_toolbar_changed(GSettings *settings, const char *key,
                                      gpointer user_data) {
  FrWindow *window = user_data;

  fr_window_set_toolbar_visibility(window,
                                   g_settings_get_boolean(settings, key));
}

static void pref_view_statusbar_changed(GSettings *settings, const char *key,
                                        gpointer user_data) {
  FrWindow *window = user_data;

  fr_window_set_statusbar_visibility(window,
                                     g_settings_get_boolean(settings, key));
}

static void pref_view_folders_changed(GSettings *settings, const char *key,
                                      gpointer user_data) {
  FrWindow *window = user_data;

  fr_window_set_folders_visibility(window,
                                   g_settings_get_boolean(settings, key));
}

static void pref_show_field_changed(GSettings *settings, const char *key,
                                    gpointer user_data) {
  FrWindow *window = user_data;
...
static void pref_click_policy_changed(GSettings *settings, const char *key,
                                      gpointer user_data) {
  FrWindow *window = user_data;
  GdkWindow *win =
      gtk_tree_view_get_bin_window(GTK_TREE_VIEW(window->priv->list_view));
...
static void pref_use_mime_icons_changed(GSettings *settings, const char *key,
                                        gpointer user_data) {
  FrWindow *window = user_data;
...
static void theme_changed_cb(GtkIconTheme *theme, FrWindow *window) {
  file_list_icon_size =
      _gtk_widget_lookup_for_size(GTK_WIDGET(window), FILE_LIST_ICON_SIZE);
...
static gboolean fr_window_stoppable_cb(FrCommand *command, gboolean stoppable,
                                       FrWindow *window) {
  window->priv->stoppable = stoppable;
  set_sensitive(window, "Stop", stoppable);
  if (window->priv->progress_dialog != NULL)
    gtk_dialog_set_response_sensitive(GTK_DIALOG(window->priv->progress_dialog),
                                      GTK_RESPONSE_OK, stoppable);
  return TRUE;
}
...
static void menu_item_deselect_cb(GtkMenuItem *proxy, FrWindow *window) {
  gtk_statusbar_pop(GTK_STATUSBAR(window->priv->statusbar),
                    window->priv->help_message_cid);
}

static void disconnect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                                GtkWidget *proxy, FrWindow *window) {
  if (GTK_IS_MENU_ITEM(proxy)) {
    g_signal_handlers_disconnect_by_func(proxy, G_CALLBACK(menu_item_select_cb),
                                         window);
    g_signal_handlers_disconnect_by_func(
...
static void connect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                             GtkWidget *proxy, FrWindow *window) {
  if (GTK_IS_MENU_ITEM(proxy)) {
...
static void view_as_radio_action(GtkAction *action, GtkRadioAction *current,
                                 gpointer data) {
  FrWindow *window = data;
  fr_window_set_list_mode(window, gtk_radio_action_get_current_value(current));
}

static void sort_by_radio_action(GtkAction *action, GtkRadioAction *current,
                                 gpointer data) {
  FrWindow *window = data;
...
static void fr_window_init_recent_chooser(FrWindow *window,
  ...
  for (i = 0; open_type[i] != -1; i++)
    gtk_recent_filter_add_mime_type(filter,
                                    mime_type_desc[open_type[i]].mime_type);
  ...
  gtk_recent_chooser_set_limit(
      chooser,
      g_settings_get_int(window->priv->settings_ui, PREF_UI_HISTORY_LEN));
  gtk_recent_chooser_set_show_not_found(chooser, TRUE);
  gtk_recent_chooser_set_sort_type(chooser, GTK_RECENT_SORT_MRU);

  g_signal_connect(G_OBJECT(chooser), "item_activated",
                   G_CALLBACK(recent_chooser_item_activated_cb), window);
}

static void close_sidepane_button_clicked_cb(GtkButton *button,
                                             FrWindow *window) {
  fr_window_set_folders_visibility(window, FALSE);
}

static void fr_window_activate_filter(FrWindow *window) {
...
static void filter_entry_activate_cb(GtkEntry *entry, FrWindow *window) {
  fr_window_activate_filter(window);
}

static void filter_entry_icon_release_cb(GtkEntry *entry,
...
static void fr_window_attach(FrWindow *window, GtkWidget *child,
                             FrWindowArea area) {
  int position;
  ...
  gtk_grid_attach(GTK_GRID(window->priv->layout), child, 0, position, 1, 1);
}

static void set_action_important(GtkUIManager *ui, const char *action_name) {
  GtkAction *action;
...
static void fr_window_construct(FrWindow *window) {
  GtkWidget *menubar;
  GtkWidget *toolbar;
  ...
  caja_schema =
      g_settings_schema_source_lookup(schema_source, CAJA_SCHEMA, FALSE);
  if (caja_schema) {
  ...
  g_signal_connect(G_OBJECT(window), "delete_event",
                   G_CALLBACK(fr_window_delete_event_cb), window);

  g_signal_connect(G_OBJECT(window), "show", G_CALLBACK(fr_window_show_cb),
                   window);

  window->priv->theme_changed_handler_id = g_signal_connect(
      icon_theme, "changed", G_CALLBACK(theme_changed_cb), window);
  ...
  gtk_drag_dest_set(GTK_WIDGET(window), GTK_DEST_DEFAULT_ALL, target_table,
                    G_N_ELEMENTS(target_table), GDK_ACTION_COPY);

  g_signal_connect(G_OBJECT(window), "drag_data_received",
                   G_CALLBACK(fr_window_drag_data_received), window);
  g_signal_connect(G_OBJECT(window), "drag_motion",
                   G_CALLBACK(fr_window_drag_motion), window);
  ...
  g_signal_connect(G_OBJECT(window->archive), "start",
                   G_CALLBACK(action_started), window);
  g_signal_connect(G_OBJECT(window->archive), "done",
                   G_CALLBACK(action_performed), window);
  g_signal_connect(G_OBJECT(window->archive), "progress",
                   G_CALLBACK(fr_window_progress_cb), window);
  g_signal_connect(G_OBJECT(window->archive), "message",
                   G_CALLBACK(fr_window_message_cb), window);
  g_signal_connect(G_OBJECT(window->archive), "stoppable",
                   G_CALLBACK(fr_window_stoppable_cb), window);
  g_signal_connect(G_OBJECT(window->archive), "working_archive",
                   G_CALLBACK(fr_window_working_archive_cb), window);

  fr_archive_set_fake_load_func(window->archive, fr_window_fake_load, window);

  window->priv->sort_method = g_settings_get_enum(
      window->priv->settings_listing, PREF_LISTING_SORT_METHOD);
  window->priv->sort_type = g_settings_get_enum(window->priv->settings_listing,
                                                PREF_LISTING_SORT_TYPE);

  window->priv->list_mode = window->priv->last_list_mode = g_settings_get_enum(
      window->priv->settings_listing, PREF_LISTING_LIST_MODE);
  g_settings_set_boolean(window->priv->settings_listing, PREF_LISTING_SHOW_PATH,
                         (window->priv->list_mode == FR_WINDOW_LIST_MODE_FLAT));
  ...
  window->priv->compression = g_settings_get_enum(
      window->priv->settings_general, PREF_GENERAL_COMPRESSION_LEVEL);
  window->priv->encrypt_header = g_settings_get_boolean(
      window->priv->settings_general, PREF_GENERAL_ENCRYPT_HEADER);
  ...
  window->priv->list_store =
      fr_list_model_new(NUMBER_OF_COLUMNS, G_TYPE_POINTER, GDK_TYPE_PIXBUF,
  ...
  window->priv->list_view =
      gtk_tree_view_new_with_model(GTK_TREE_MODEL(window->priv->list_store));
  ...
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(window->priv->list_store),
                                  COLUMN_NAME, name_column_sort_func, NULL,
                                  NULL);
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(window->priv->list_store),
                                  COLUMN_SIZE, size_column_sort_func, NULL,
                                  NULL);
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(window->priv->list_store),
                                  COLUMN_TYPE, type_column_sort_func, NULL,
                                  NULL);
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(window->priv->list_store),
                                  COLUMN_TIME, time_column_sort_func, NULL,
                                  NULL);
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(window->priv->list_store),
                                  COLUMN_PATH, path_column_sort_func, NULL,
                                  NULL);

  gtk_tree_sortable_set_default_sort_func(
      GTK_TREE_SORTABLE(window->priv->list_store), no_sort_column_sort_func,
      NULL, NULL);
  ...
  g_signal_connect(selection, "changed", G_CALLBACK(selection_changed_cb),
                   window);
  g_signal_connect(G_OBJECT(window->priv->list_view), "row_activated",
                   G_CALLBACK(row_activated_cb), window);

  g_signal_connect(G_OBJECT(window->priv->list_view), "button_press_event",
                   G_CALLBACK(file_button_press_cb), window);
  g_signal_connect(G_OBJECT(window->priv->list_view), "button_release_event",
                   G_CALLBACK(file_button_release_cb), window);
  g_signal_connect(G_OBJECT(window->priv->list_view), "motion_notify_event",
                   G_CALLBACK(file_motion_notify_callback), window);
  g_signal_connect(G_OBJECT(window->priv->list_view), "leave_notify_event",
                   G_CALLBACK(file_leave_notify_callback), window);

  g_signal_connect(G_OBJECT(window->priv->list_store), "sort_column_changed",
                   G_CALLBACK(sort_column_changed_cb), window);

  g_signal_connect(G_OBJECT(window->priv->list_view), "drag_begin",
                   G_CALLBACK(file_list_drag_begin), window);
  g_signal_connect(G_OBJECT(window->priv->list_view), "drag_end",
                   G_CALLBACK(file_list_drag_end), window);
  ...
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(list_scrolled_window),
                                      GTK_SHADOW_IN);
  gtk_container_add(GTK_CONTAINER(list_scrolled_window),
                    window->priv->list_view);

  /* filter bar */

  window->priv->filter_bar = filter_box =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
  gtk_container_set_border_width(GTK_CONTAINER(filter_box), 3);
  fr_window_attach(FR_WINDOW(window), window->priv->filter_bar,
                   FR_WINDOW_AREA_FILTERBAR);

  gtk_box_pack_start(GTK_BOX(filter_box), gtk_label_new(_("Find:")), FALSE,
                     FALSE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(filter_box), window->priv->filter_entry, FALSE,
                     FALSE, 6);

  g_signal_connect(G_OBJECT(window->priv->filter_entry), "activate",
                   G_CALLBACK(filter_entry_activate_cb), window);
  g_signal_connect(G_OBJECT(window->priv->filter_entry), "icon-release",
                   G_CALLBACK(filter_entry_icon_release_cb), window);

  gtk_widget_show_all(filter_box);

  /* tree view */

  window->priv->tree_store =
      gtk_tree_store_new(TREE_NUMBER_OF_COLUMNS, G_TYPE_STRING, GDK_TYPE_PIXBUF,
                         G_TYPE_STRING, PANGO_TYPE_WEIGHT);
  window->priv->tree_view =
      gtk_tree_view_new_with_model(GTK_TREE_MODEL(window->priv->tree_store));
  ...
  g_signal_connect(G_OBJECT(window->priv->tree_view), "button_press_event",
                   G_CALLBACK(dir_tree_button_press_cb), window);

  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(window->priv->tree_view));
  g_signal_connect(selection, "changed",
                   G_CALLBACK(dir_tree_selection_changed_cb), window);

  g_signal_connect(G_OBJECT(window->priv->tree_view), "drag_begin",
                   G_CALLBACK(file_list_drag_begin), window);
  g_signal_connect(G_OBJECT(window->priv->tree_view), "drag_end",
                   G_CALLBACK(file_list_drag_end), window);
  g_signal_connect(G_OBJECT(window->priv->tree_view), "drag_data_get",
                   G_CALLBACK(fr_window_folder_tree_drag_data_get), window);
  gtk_drag_source_set(window->priv->tree_view, GDK_BUTTON1_MASK,
  ...
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(tree_scrolled_window),
                                      GTK_SHADOW_IN);
  gtk_container_add(GTK_CONTAINER(tree_scrolled_window),
                    window->priv->tree_view);
  ...
  gtk_box_pack_start(GTK_BOX(sidepane_title_box), sidepane_title_label, TRUE,
                     TRUE, 0);

  close_sidepane_button = gtk_button_new();
  gtk_container_add(
      GTK_CONTAINER(close_sidepane_button),
  ...
  gtk_widget_set_tooltip_text(close_sidepane_button,
                              _("Close the folders pane"));
  g_signal_connect(close_sidepane_button, "clicked",
                   G_CALLBACK(close_sidepane_button_clicked_cb), window);
  gtk_box_pack_end(GTK_BOX(sidepane_title_box), close_sidepane_button, FALSE,
                   FALSE, 0);

  gtk_box_pack_start(GTK_BOX(window->priv->sidepane), sidepane_title, FALSE,
                     FALSE, 0);
  gtk_box_pack_start(GTK_BOX(window->priv->sidepane), tree_scrolled_window,
                     TRUE, TRUE, 0);

  /* main content */

  window->priv->paned = gtk_paned_new(GTK_ORIENTATION_HORIZONTAL);
  gtk_paned_pack1(GTK_PANED(window->priv->paned), window->priv->sidepane, FALSE,
                  TRUE);
  gtk_paned_pack2(GTK_PANED(window->priv->paned), list_scrolled_window, TRUE,
                  TRUE);
  gtk_paned_set_position(
      GTK_PANED(window->priv->paned),
      g_settings_get_int(window->priv->settings_ui, PREF_UI_SIDEBAR_WIDTH));

  fr_window_attach(FR_WINDOW(window), window->priv->paned,
                   FR_WINDOW_AREA_CONTENTS);
  ...
  action = g_object_new(
      GTK_TYPE_RECENT_ACTION, "name", "OpenRecent",
      /* Translators: this is the label for the "open recent file" sub-menu. */
      "label", _("Open _Recent"), "tooltip", _("Open a recently used archive"),
      "stock-id", "gtk-open", NULL);
  ...
  action = g_object_new(GTK_TYPE_RECENT_ACTION, "name", "OpenRecent_Toolbar",
                        "label", _("Open"), "tooltip",
                        _("Open a recently used archive"), "stock-id",
                        "gtk-open", "is-important", TRUE, NULL);
  fr_window_init_recent_chooser(window, GTK_RECENT_CHOOSER(action));
  g_signal_connect(action, "activate", G_CALLBACK(activate_action_open),
                   window);
  gtk_action_group_add_action(actions, action);
  g_object_unref(action);
  ...
  gtk_action_group_add_actions(actions, action_entries, n_action_entries,
                               window);
  gtk_action_group_add_toggle_actions(actions, action_toggle_entries,
                                      n_action_toggle_entries, window);
  gtk_action_group_add_radio_actions(actions, view_as_entries,
                                     n_view_as_entries, window->priv->list_mode,
                                     G_CALLBACK(view_as_radio_action), window);
  gtk_action_group_add_radio_actions(actions, sort_by_entries,
                                     n_sort_by_entries, window->priv->sort_type,
                                     G_CALLBACK(sort_by_radio_action), window);

  g_signal_connect(ui, "connect_proxy", G_CALLBACK(connect_proxy_cb), window);
  g_signal_connect(ui, "disconnect_proxy", G_CALLBACK(disconnect_proxy_cb),
                   window);
  ...
  gtk_accel_group_connect(
      gtk_ui_manager_get_accel_group(ui), GDK_KEY_q, GDK_CONTROL_MASK, 0,
      g_cclosure_new_swap(G_CALLBACK(fr_window_close), window, NULL));

  if (!gtk_ui_manager_add_ui_from_resource(
          ui, ENGRAMPA_RESOURCE_UI_PATH G_DIR_SEPARATOR_S "menus-toolbars.ui",
          &error)) {
  ...
  gtk_toolbar_set_style(GTK_TOOLBAR(window->priv->location_bar),
                        GTK_TOOLBAR_BOTH_HORIZ);
  gtk_style_context_add_class(
      gtk_widget_get_style_context(window->priv->location_bar),
  ...
  gtk_box_pack_start(GTK_BOX(location_box), window->priv->location_label, FALSE,
                     FALSE, 5);
  ...
  gtk_box_pack_start(GTK_BOX(location_box), window->priv->location_entry, TRUE,
                     TRUE, 5);

  g_signal_connect(G_OBJECT(window->priv->location_entry), "key_press_event",
                   G_CALLBACK(location_entry_key_press_event_cb), window);
  ...
  fr_window_attach(FR_WINDOW(window), window->priv->location_bar,
                   FR_WINDOW_AREA_LOCATIONBAR);
  if (window->priv->list_mode == FR_WINDOW_LIST_MODE_FLAT)
    gtk_widget_hide(window->priv->location_bar);
  else
  ...
  fr_window_attach(FR_WINDOW(window), window->priv->toolbar,
                   FR_WINDOW_AREA_TOOLBAR);
  ...
  window->priv->file_popup_menu =
      gtk_ui_manager_get_widget(ui, "/FilePopupMenu");
  window->priv->folder_popup_menu =
      gtk_ui_manager_get_widget(ui, "/FolderPopupMenu");
  window->priv->sidebar_folder_popup_menu =
      gtk_ui_manager_get_widget(ui, "/SidebarFolderPopupMenu");
  ...
  window->priv->list_info_cid = gtk_statusbar_get_context_id(
      GTK_STATUSBAR(window->priv->statusbar), "list_info");
  window->priv->progress_cid = gtk_statusbar_get_context_id(
      GTK_STATUSBAR(window->priv->statusbar), "progress");
  ...
  gtk_box_set_child_packing(GTK_BOX(statusbar_box),
                            gtk_statusbar_get_message_area(statusbar), TRUE,
                            TRUE, 0, GTK_PACK_START);

  window->priv->progress_bar = gtk_progress_bar_new();
  gtk_progress_bar_set_pulse_step(GTK_PROGRESS_BAR(window->priv->progress_bar),
                                  ACTIVITY_PULSE_STEP);
  gtk_widget_set_size_request(window->priv->progress_bar, -1,
                              PROGRESS_BAR_HEIGHT);
  {
    GtkWidget *vbox;
    ...
    gtk_box_pack_start(GTK_BOX(vbox), window->priv->progress_bar, TRUE, TRUE,
                       1);
    gtk_widget_show(vbox);
  }
  gtk_widget_show(statusbar_box);

  fr_window_attach(FR_WINDOW(window), window->priv->statusbar,
                   FR_WINDOW_AREA_STATUSBAR);
  ...
  g_signal_connect(window->priv->settings_ui, "changed::" PREF_UI_HISTORY_LEN,
                   G_CALLBACK(pref_history_len_changed), window);
  g_signal_connect(window->priv->settings_ui, "changed::" PREF_UI_VIEW_TOOLBAR,
                   G_CALLBACK(pref_view_toolbar_changed), window);
  g_signal_connect(window->priv->settings_ui,
                   "changed::" PREF_UI_VIEW_STATUSBAR,
                   G_CALLBACK(pref_view_statusbar_changed), window);
  g_signal_connect(window->priv->settings_ui, "changed::" PREF_UI_VIEW_FOLDERS,
                   G_CALLBACK(pref_view_folders_changed), window);
  g_signal_connect(window->priv->settings_listing,
                   "changed::" PREF_LISTING_SHOW_TYPE,
                   G_CALLBACK(pref_show_field_changed), window);
  g_signal_connect(window->priv->settings_listing,
  ...
  g_signal_connect(window->priv->settings_listing,
                   ...
                   G_CALLBACK(pref_use_mime_icons_changed), window);

  if (window->priv->settings_caja)
    g_signal_connect(window->priv->settings_caja, "changed::" CAJA_CLICK_POLICY,
                     G_CALLBACK(pref_click_policy_changed), window);
...
GtkWidget *fr_window_new(void) {
  ...
  window = g_object_new(FR_TYPE_WINDOW, "application",
...
static void fr_window_set_archive_uri(FrWindow *window, const char *uri) {
  if (window->priv->archive_uri != NULL) g_free(window->priv->archive_uri);
  window->priv->archive_uri = g_strdup(uri);
}

gboolean fr_window_archive_new(FrWindow *window, const char *uri) {
  ...
  if (!fr_archive_create(window->archive, uri)) {
    ...
    fr_archive_action_completed(window->archive, FR_ACTION_CREATING_NEW_ARCHIVE,
  ...
  fr_archive_action_completed(window->archive, FR_ACTION_CREATING_NEW_ARCHIVE,
                              FR_PROC_ERROR_NONE, NULL);

  return TRUE;
}

FrWindow *fr_window_archive_open(FrWindow *current_window, const char *uri,
                                 GtkWindow *parent) {
  ...
  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_LOAD,
                                     g_strdup(window->priv->archive_uri),
                                     (GFreeFunc)g_free);

  fr_archive_load(window->archive, window->priv->archive_uri,
...
void fr_window_archive_close(FrWindow *window) {
  ...
  if (!window->priv->archive_new && !window->priv->archive_present) return;
...
static SaveAsData *save_as_data_new(const char *uri, const char *password,
                                    gboolean encrypt_header,
                                    guint volume_size) {
  SaveAsData *sdata;

  sdata = g_new0(SaveAsData, 1);
  if (uri != NULL) sdata->uri = g_strdup(uri);
  if (password != NULL) sdata->password = g_strdup(password);
...
static void save_as_data_free(SaveAsData *sdata) {
...
void fr_window_archive_save_as(FrWindow *window, const char *uri,
                               const char *password, gboolean encrypt_header,
                               guint volume_size) {
  g_return_if_fail(window != NULL);
  ...
  if (!fr_archive_create(window->priv->convert_data.new_archive, uri)) {
    ...
    message =
        g_strdup_printf(_("Could not save the archive \"%s\""), utf8_name);
    g_free(utf8_name);

    d = _gtk_error_dialog_new(GTK_WINDOW(window),
                              GTK_DIALOG_DESTROY_WITH_PARENT, NULL, message,
                              "%s", _("Archive type not supported."));
    gtk_dialog_run(GTK_DIALOG(d));
    gtk_widget_destroy(d);
  ...
  fr_window_set_current_batch_action(
      window, FR_BATCH_ACTION_SAVE_AS,
      save_as_data_new(uri, password, encrypt_header, volume_size),
      (GFreeFunc)save_as_data_free);

  g_signal_connect(G_OBJECT(window->priv->convert_data.new_archive), "start",
                   G_CALLBACK(action_started), window);
  g_signal_connect(G_OBJECT(window->priv->convert_data.new_archive), "done",
                   G_CALLBACK(convert__action_performed), window);
  g_signal_connect(G_OBJECT(window->priv->convert_data.new_archive), "progress",
                   G_CALLBACK(fr_window_progress_cb), window);
  g_signal_connect(G_OBJECT(window->priv->convert_data.new_archive), "message",
                   G_CALLBACK(fr_window_message_cb), window);
  g_signal_connect(G_OBJECT(window->priv->convert_data.new_archive),
                   "stoppable", G_CALLBACK(fr_window_stoppable_cb), window);
  ...
  fr_archive_extract_to_local(window->archive, NULL,
                              window->priv->convert_data.temp_dir, NULL, TRUE,
                              FALSE, FALSE, window->priv->password);
...
void fr_window_archive_reload(FrWindow *window) {
  ...
  if (window->priv->activity_ref > 0) return;
  if (window->priv->archive_new) return;
...
void fr_window_archive_add_files(FrWindow *window,
  ...
  base_len = 0;
  if (strcmp(base_dir, "/") != 0) base_len = strlen(base_dir);
  ...
  fr_archive_add_files(window->archive, files, base_uri,
                       fr_window_get_current_location(window), update,
                       window->priv->password, window->priv->encrypt_header,
                       window->priv->compression, window->priv->volume_size);
...
void fr_window_archive_add_with_wildcard(
    FrWindow *window, const char *include_files, const char *exclude_files,
    const char *exclude_folders, const char *base_dir, const char *dest_dir,
    gboolean update, gboolean follow_links) {
  fr_archive_add_with_wildcard(
      window->archive, include_files, exclude_files, exclude_folders, base_dir,
      (dest_dir == NULL) ? fr_window_get_current_location(window) : dest_dir,
      update, follow_links, window->priv->password,
      window->priv->encrypt_header, window->priv->compression,
      window->priv->volume_size);
}

void fr_window_archive_add_directory(FrWindow *window, const char *directory,
                                     const char *base_dir, const char *dest_dir,
                                     gboolean update) {
  fr_archive_add_directory(
      window->archive, directory, base_dir,
      (dest_dir == NULL) ? fr_window_get_current_location(window) : dest_dir,
      update, window->priv->password, window->priv->encrypt_header,
      window->priv->compression, window->priv->volume_size);
}

void fr_window_archive_add_items(FrWindow *window, GList *item_list,
                                 const char *base_dir, const char *dest_dir,
                                 gboolean update) {
  fr_archive_add_items(
      window->archive, item_list, base_dir,
      (dest_dir == NULL) ? fr_window_get_current_location(window) : dest_dir,
      update, window->priv->password, window->priv->encrypt_header,
      window->priv->compression, window->priv->volume_size);
}

void fr_window_archive_add_dropped_items(FrWindow *window, GList *item_list,
                                         gboolean update) {
  fr_archive_add_dropped_items(
      window->archive, item_list, fr_window_get_current_location(window),
      fr_window_get_current_location(window), update, window->priv->password,
      window->priv->encrypt_header, window->priv->compression,
      window->priv->volume_size);
}

void fr_window_archive_remove(FrWindow *window, GList *file_list) {
...
static ExtractData *extract_data_new(GList *file_list,
                                     const char *extract_to_dir,
                                     const char *base_dir, gboolean skip_older,
                                     FrOverwrite overwrite, gboolean junk_paths,
                                     gboolean extract_here,
                                     gboolean ask_to_open_destination) {
  ExtractData *edata;
  ...
  if (extract_to_dir != NULL) edata->extract_to_dir = g_strdup(extract_to_dir);
  edata->skip_older = skip_older;
  edata->overwrite = overwrite;
  edata->junk_paths = junk_paths;
  if (base_dir != NULL) edata->base_dir = g_strdup(base_dir);
...
static ExtractData *extract_to_data_new(const char *extract_to_dir) {
  return extract_data_new(NULL, extract_to_dir, NULL, FALSE, TRUE, FALSE, FALSE,
                          FALSE);
}

static void extract_data_free(ExtractData *edata) {
...
static gboolean archive_is_encrypted(FrWindow *window, GList *file_list) {
  ...
  if (file_list == NULL) {
    ...
    for (i = 0; !encrypted && i < window->archive->command->files->len; i++) {
      ...
      if (fdata->encrypted) encrypted = TRUE;
    }
  } else {
...
void fr_window_archive_extract_here(FrWindow *window, gboolean skip_older,
                                    FrOverwrite overwrite,
                                    gboolean junk_paths) {
  ExtractData *edata;

  edata = extract_data_new(NULL, NULL, NULL, skip_older, overwrite, junk_paths,
                           TRUE, FALSE);
  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_EXTRACT, edata,
  ...
  window->priv->ask_to_open_destination_after_extraction =
      edata->ask_to_open_destination;

  fr_process_clear(window->archive->process);
  if (fr_archive_extract_here(window->archive, edata->skip_older,
                              edata->overwrite, edata->junk_paths,
                              window->priv->password)) {
    fr_process_start(window->archive->process);
  }
}
...
static void _fr_window_archive_extract_from_edata(FrWindow *window,
                                                  ExtractData *edata) {
  window->priv->ask_to_open_destination_after_extraction =
      edata->ask_to_open_destination;

  fr_process_clear(window->archive->process);
  fr_archive_extract(window->archive, edata->file_list, edata->extract_to_dir,
                     edata->base_dir, edata->skip_older,
                     edata->overwrite == FR_OVERWRITE_YES, edata->junk_paths,
                     window->priv->password);
  fr_process_start(window->archive->process);
}
...
static void overwrite_dialog_response_cb(GtkDialog *dialog, int response_id,
                                         gpointer user_data) {
  ...
  switch (response_id) {
    ...
    case _FR_RESPONSE_OVERWRITE_NO: {
      ...
      odata->edata->file_list =
          g_list_remove_link(odata->edata->file_list, odata->current_file);
...
static void _fr_window_ask_overwrite_dialog(OverwriteData *odata) {
  while ((odata->edata->overwrite == FR_OVERWRITE_ASK) &&
    ...
    base_name =
        _g_path_get_base_name((char *)odata->current_file->data,
                              odata->edata->base_dir, odata->edata->junk_paths);
    e_base_name = g_uri_escape_string(
        base_name, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
    dest_uri =
        g_strdup_printf("%s/%s", odata->edata->extract_to_dir, e_base_name);
    ...
    if ((file_type != G_FILE_TYPE_UNKNOWN) &&
      ...
      msg = g_strdup_printf(_("Replace file \"%s\"?"),
      ...
      details = g_strdup_printf(
          ...
          parent_name);
      d = _gtk_message_dialog_new(
          GTK_WINDOW(odata->window), GTK_DIALOG_MODAL, "dialog-question", msg,
          details, "gtk-cancel", GTK_RESPONSE_CANCEL, _("Replace _All"),
          _FR_RESPONSE_OVERWRITE_YES_ALL, _("_Skip"), _FR_RESPONSE_OVERWRITE_NO,
          _("_Replace"), _FR_RESPONSE_OVERWRITE_YES, NULL);
      gtk_dialog_set_default_response(GTK_DIALOG(d),
                                      _FR_RESPONSE_OVERWRITE_YES);
      g_signal_connect(d, "response", G_CALLBACK(overwrite_dialog_response_cb),
                       odata);
      gtk_widget_show(d);
  ...
  if (odata->edata->file_list != NULL) {
    ...
    d = _gtk_message_dialog_new(GTK_WINDOW(odata->window), 0, "dialog-warning",
                                _("Extraction not performed"), NULL, "gtk-ok",
                                GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(d), GTK_RESPONSE_OK);
    fr_window_show_error_dialog(odata->window, d, GTK_WINDOW(odata->window),
...
void fr_window_archive_extract(FrWindow *window, GList *file_list,
                               const char *extract_to_dir, const char *base_dir,
                               gboolean skip_older, FrOverwrite overwrite,
                               gboolean junk_paths,
                               gboolean ask_to_open_destination) {
  ExtractData *edata;
  gboolean do_not_extract = FALSE;
  ...
  edata =
      extract_data_new(file_list, extract_to_dir, base_dir, skip_older,
                       overwrite, junk_paths, FALSE, ask_to_open_destination);

  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_EXTRACT, edata,
                                     (GFreeFunc)extract_data_free);

  if (archive_is_encrypted(window, edata->file_list) &&
      (window->priv->password == NULL)) {
    dlg_ask_password(window);
    return;
  }
  ...
  if (!uri_is_dir(edata->extract_to_dir)) {
    ...
    if (!ForceDirectoryCreation) {
      ...
      d = _gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_MODAL,
                                  "dialog-question", msg, NULL, "gtk-cancel",
                                  GTK_RESPONSE_CANCEL, _("Create _Folder"),
                                  GTK_RESPONSE_YES, NULL);
      ...
      if (r != GTK_RESPONSE_YES) do_not_extract = TRUE;
    }

    if (!do_not_extract &&
        !ensure_dir_exists(edata->extract_to_dir, 0755, &error)) {
      GtkWidget *d;
      char *details;

      details = g_strdup_printf(
          _("Could not create the destination folder: %s."), error->message);
  ...
  if (do_not_extract) {
    ...
    d = _gtk_message_dialog_new(GTK_WINDOW(window), 0, "dialog-warning",
                                _("Extraction not performed"), NULL, "gtk-ok",
                                GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(d), GTK_RESPONSE_OK);
  ...
  if (edata->overwrite == FR_OVERWRITE_ASK) {
    ...
    odata->edata = edata;
    odata->extract_all =
        (edata->file_list == NULL) || (g_list_length(edata->file_list) ==
                                       window->archive->command->files->len);
...
void fr_window_archive_test(FrWindow *window) {
  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_TEST, NULL, NULL);
  fr_archive_test(window->archive, window->priv->password);
}

void fr_window_set_password(FrWindow *window, const char *password) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_encrypt_header(FrWindow *window, gboolean encrypt_header) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_compression(FrWindow *window, FrCompression compression) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_volume_size(FrWindow *window, guint volume_size) {
  g_return_if_fail(window != NULL);
...
void fr_window_go_to_location(FrWindow *window, const char *path,
  ...
  if ((window->priv->last_location == NULL) ||
      (strcmp(window->priv->last_location, dir) != 0)) {
    g_free(window->priv->last_location);
...
void fr_window_go_back(FrWindow *window) {
  ...
  if (window->priv->history == NULL) return;
  if (window->priv->history_current == NULL) return;
  if (window->priv->history_current->next == NULL) return;
...
void fr_window_go_forward(FrWindow *window) {
  ...
  if (window->priv->history == NULL) return;
  if (window->priv->history_current == NULL) return;
  if (window->priv->history_current->prev == NULL) return;
  window->priv->history_current = window->priv->history_current->prev;

  fr_window_go_to_location(window, window->priv->history_current->data, FALSE);
}

void fr_window_set_list_mode(FrWindow *window, FrWindowListMode list_mode) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_sort_type(FrWindow *window, GtkSortType sort_type) {
  window->priv->sort_type = sort_type;
  fr_window_update_list_order(window);
}

void fr_window_stop(FrWindow *window) {
  if (!window->priv->stoppable) return;

  if (window->priv->activity_ref > 0) fr_archive_stop(window->archive);
...
static int activity_cb(gpointer data) {
  FrWindow *window = data;
...
void fr_window_start_activity_mode(FrWindow *window) {
  ...
  window->priv->activity_timeout_handle =
      g_timeout_add(ACTIVITY_DELAY, activity_cb, window);
  fr_window_update_sensitivity(window);
}
...
void fr_window_stop_activity_mode(FrWindow *window) {
  ...
  if (window->priv->activity_timeout_handle == 0) return;

  g_source_remove(window->priv->activity_timeout_handle);
  window->priv->activity_timeout_handle = 0;

  if (!gtk_widget_get_realized(GTK_WIDGET(window))) return;

  if (window->priv->progress_dialog != NULL)
    gtk_progress_bar_set_fraction(
        GTK_PROGRESS_BAR(window->priv->pd_progress_bar), 0.0);

  if (!window->priv->batch_mode) {
    if (window->priv->progress_bar != NULL)
      gtk_progress_bar_set_fraction(
          GTK_PROGRESS_BAR(window->priv->progress_bar), 0.0);
    fr_window_update_sensitivity(window);
  }
}

static gboolean last_output_window__unrealize_cb(GtkWidget *widget,
                                                 gpointer data) {
  pref_util_save_window_geometry(GTK_WINDOW(widget), LAST_OUTPUT_DIALOG_NAME);
  return FALSE;
}

static void fr_window_view_last_output_print(GtkTextBuffer *text_buffer,
                                             GtkTextIter *iter, GList *scan) {
  for (; scan; scan = scan->next) {
    ...
    gtk_text_buffer_insert_with_tags_by_name(text_buffer, iter, utf8_line,
                                             bytes_written, "monospace", NULL);
    g_free(utf8_line);
    gtk_text_buffer_insert(text_buffer, iter, "\n", 1);
  }
}

void fr_window_view_last_output(FrWindow *window, const char *title) {
  GtkWidget *dialog;
  GtkWidget *vbox;
  GtkWidget *text_view;
  GtkWidget *scrolled;
  ...
  gtk_text_buffer_create_tag(text_buffer, "monospace", "family", "monospace",
                             NULL);
  ...
  g_signal_connect(G_OBJECT(dialog), "unrealize",
                   G_CALLBACK(last_output_window__unrealize_cb), NULL);

  gtk_text_buffer_get_iter_at_offset(text_buffer, &iter, 0);
  /* Show STDOUT of process */
  fr_window_view_last_output_print(text_buffer, &iter,
                                   window->archive->process->out.raw);
  /* Show STDERR of process */
  fr_window_view_last_output_print(text_buffer, &iter,
                                   window->archive->process->err.raw);
  ...
  pref_util_restore_window_geometry(GTK_WINDOW(dialog),
                                    LAST_OUTPUT_DIALOG_NAME);
...
static RenameData *rename_data_new(const char *path_to_rename,
                                   const char *old_name, const char *new_name,
                                   const char *current_dir, gboolean is_dir,
                                   gboolean dir_in_archive,
  ...
  if (old_name != NULL) rdata->old_name = g_strdup(old_name);
  if (new_name != NULL) rdata->new_name = g_strdup(new_name);
  if (current_dir != NULL) rdata->current_dir = g_strdup(current_dir);
  rdata->is_dir = is_dir;
  rdata->dir_in_archive = dir_in_archive;
  if (original_path != NULL) rdata->original_path = g_strdup(original_path);

  return rdata;
}

static void rename_data_free(RenameData *rdata) {
...
static void rename_selection(FrWindow *window, const char *path_to_rename,
                             const char *old_name, const char *new_name,
                             const char *current_dir, gboolean is_dir,
                             gboolean dir_in_archive,
                             const char *original_path) {
  ...
  rdata = rename_data_new(path_to_rename, old_name, new_name, current_dir,
                          is_dir, dir_in_archive, original_path);
  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_RENAME, rdata,
  ...
  fr_archive_extract_to_local(archive, file_list, tmp_dir, NULL, FALSE, TRUE,
                              FALSE, window->priv->password);

  /* temporarily add the dir to rename to the list if it's stored in the
   * archive, this way it will be removed from the archive... */
  added_dir = FALSE;
  if (is_dir && dir_in_archive &&
      !g_list_find_custom(file_list, original_path, (GCompareFunc)strcmp)) {
  ...
  new_dirname =
      g_build_filename(rdata->current_dir + 1, rdata->new_name, "/", NULL);
  new_file_list = NULL;
  if (rdata->is_dir) {
    char *old_path;
    char *new_path;
    ...
    old_path =
        g_build_filename(tmp_dir, rdata->current_dir, rdata->old_name, NULL);
    new_path =
        g_build_filename(tmp_dir, rdata->current_dir, rdata->new_name, NULL);
  ...
  for (scan = file_list; scan; scan = scan->next) {
    ...
    if (strlen(filename) >
        (strlen(rdata->current_dir) + strlen(rdata->old_name)))
      common = g_strdup(filename + strlen(rdata->current_dir) +
                        strlen(rdata->old_name));
    new_path = g_build_filename(tmp_dir, rdata->current_dir, rdata->new_name,
                                common, NULL);
    ...
    new_filename =
        g_build_filename(current_dir_relative, rdata->new_name, common, NULL);
  ...
  if (is_dir && dir_in_archive && ! g_list_find_custom (new_file_list,
  new_dirname, (GCompareFunc) strcmp)) new_file_list = g_list_prepend
  (new_file_list, g_build_filename (rdata->current_dir + 1, rdata->new_name,
  NULL));
  */

  fr_archive_add(archive, new_file_list, tmp_dir, NULL, FALSE, FALSE,
                 window->priv->password, window->priv->encrypt_header,
                 window->priv->compression, window->priv->volume_size);
...
static gboolean valid_name(const char *new_name, const char *old_name,
                           char **reason) {
  char *utf8_new_name;
  ...
  if (*new_name == '\0') {
    ...
    *reason = g_strdup(_("New name is void, please type a name."));
     ...
     * when renaming a file. */
    *reason =
        g_strdup(_("New name is the same as old one, please type other name."));
    retval = FALSE;
    ...
    *reason = g_strdup_printf(
        _("Name \"%s\" is not valid because it contains at least one of the "
...
static gboolean name_is_present(FrWindow *window, const char *current_dir,
                                const char *new_name, char **reason) {
  gboolean retval = FALSE;
  guint i;
  char *new_filename;
  int new_filename_l;
  ...
  for (i = 0; i < window->archive->command->files->len; i++) {
    ...
    if ((strncmp(filename, new_filename, new_filename_l) == 0) &&
      ...
      if (filename[new_filename_l] == G_DIR_SEPARATOR)
        *reason =
            g_strdup_printf(_("A folder named \"%s\" already exists.\n\n%s"),
                            utf8_name, _("Please use a different name."));
      else
        *reason =
            g_strdup_printf(_("A file named \"%s\" already exists.\n\n%s"),
                            utf8_name, _("Please use a different name."));
...
void fr_window_rename_selection(FrWindow *window, gboolean from_sidebar) {
  char *path_to_rename;
  char *parent_dir;
  char *old_name;
...
retry__rename_selection:
  ...
  if (utf8_new_name != NULL) {
    ...
    if (!valid_name(new_name, old_name, &reason)) {
      ...
      dlg = _gtk_error_dialog_new(
          GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, NULL,
    ...
    if (name_is_present(window, parent_dir, new_name, &reason)) {
      ...
      dlg = _gtk_message_dialog_new(
          GTK_WINDOW(window), GTK_DIALOG_MODAL, "dialog-question",
          (renaming_dir ? _("Could not rename the folder")
                        : _("Could not rename the file")),
          reason, "gtk-close", GTK_RESPONSE_OK, NULL);
      gtk_dialog_run(GTK_DIALOG(dlg));
      gtk_widget_destroy(dlg);
      g_free(reason);
      g_free(new_name);
    ...
    rename_selection(window, path_to_rename, old_name, new_name, parent_dir,
                     renaming_dir, dir_in_archive, original_path);
...
static void fr_clipboard_get(GtkClipboard *clipboard,
  ...
  data =
      get_selection_data_from_clipboard_data(window, window->priv->copy_data);
...
static void fr_clipboard_clear(GtkClipboard *clipboard,
                               gpointer user_data_or_owner) {
  FrWindow *window = user_data_or_owner;
...
GList *fr_window_get_selection(FrWindow *window, gboolean from_sidebar,
...
static void fr_window_copy_or_cut_selection(FrWindow *window, FRClipboardOp op,
                                            gboolean from_sidebar) {
  GList *files;
  char *base_dir;
  GtkClipboard *clipboard;
  ...
  gtk_clipboard_set_with_owner(
      clipboard, clipboard_targets, G_N_ELEMENTS(clipboard_targets),
      fr_clipboard_get, fr_clipboard_clear, G_OBJECT(window));

  fr_window_update_sensitivity(window);
}

void fr_window_copy_selection(FrWindow *window, gboolean from_sidebar) {
  fr_window_copy_or_cut_selection(window, FR_CLIPBOARD_OP_COPY, from_sidebar);
}

void fr_window_cut_selection(FrWindow *window, gboolean from_sidebar) {
  fr_window_copy_or_cut_selection(window, FR_CLIPBOARD_OP_CUT, from_sidebar);
}

static gboolean always_fake_load(FrArchive *archive, gpointer data) {
  return TRUE;
}

static void add_pasted_files(FrWindow *window, FrClipboardData *data) {
  ...
  for (scan = data->files; scan; scan = scan->next) {
    ...
    char *new_name = g_build_filename(
        current_dir_relative, old_name + strlen(data->base_dir) - 1, NULL);
    ...
    if ((strcmp(old_name, new_name) != 0) &&
      ...
      if (old_name[0] == '/') old_name = old_name + 1;
  ...
  fr_archive_add(window->archive, new_file_list, data->tmp_dir, NULL, FALSE,
                 FALSE, window->priv->password, window->priv->encrypt_header,
                 window->priv->compression, window->priv->volume_size);
...
static void copy_from_archive_action_performed_cb(FrArchive *archive,
                                                  FrAction action,
                                                  FrProcError *error,
                                                  gpointer data) {
  FrWindow *window = data;
  ...
  switch (action) {
    case FR_ACTION_LISTING_CONTENT:
      ...
      fr_archive_extract_to_local(
          window->priv->copy_from_archive, window->priv->clipboard_data->files,
          window->priv->clipboard_data->tmp_dir, NULL, FALSE, TRUE, FALSE,
...
static void fr_window_paste_from_clipboard_data(FrWindow *window,
  ...
  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_PASTE,
  ...
  if (window->priv->copy_from_archive == NULL) {
    ...
    g_signal_connect(G_OBJECT(window->priv->copy_from_archive), "start",
                     G_CALLBACK(action_started), window);
    g_signal_connect(G_OBJECT(window->priv->copy_from_archive), "done",
                     G_CALLBACK(copy_from_archive_action_performed_cb), window);
    g_signal_connect(G_OBJECT(window->priv->copy_from_archive), "progress",
                     G_CALLBACK(fr_window_progress_cb), window);
    g_signal_connect(G_OBJECT(window->priv->copy_from_archive), "message",
                     G_CALLBACK(fr_window_message_cb), window);
    g_signal_connect(G_OBJECT(window->priv->copy_from_archive), "stoppable",
                     G_CALLBACK(fr_window_stoppable_cb), window);
    fr_archive_set_fake_load_func(window->priv->copy_from_archive,
                                  always_fake_load, NULL);
  }
  fr_archive_load_local(window->priv->copy_from_archive, data->archive_filename,
                        data->archive_password);
}

static void fr_window_paste_selection_to(FrWindow *window,
                                         const char *current_dir) {
  GtkClipboard *clipboard;
  GtkSelectionData *selection_data;
  ...
  clipboard = gtk_clipboard_get(FR_CLIPBOARD);
  selection_data =
      gtk_clipboard_wait_for_contents(clipboard, FR_SPECIAL_URI_LIST);
  if (selection_data == NULL) return;

  paste_data = get_clipboard_data_from_selection_data(
      window, (char *)gtk_selection_data_get_data(selection_data));
...
void fr_window_paste_selection(FrWindow *window, gboolean from_sidebar) {
  char *utf8_path, *utf8_old_path, *destination;
  char *current_dir;

  if (window->priv->list_mode == FR_WINDOW_LIST_MODE_FLAT) return;

  /**/

  utf8_old_path = g_filename_to_utf8(fr_window_get_current_location(window), -1,
                                     NULL, NULL, NULL);
  utf8_path = _gtk_request_dialog_run(
      GTK_WINDOW(window), (GTK_DIALOG_DESTROY_WITH_PARENT | GTK_DIALOG_MODAL),
      _("Paste Selection"), _("_Destination folder:"), utf8_old_path, 1024,
      _("_Cancel"), _("_Paste"));
  ...
  if (destination[0] != '/')
    current_dir =
        build_uri(fr_window_get_current_location(window), destination, NULL);
  else
...
void fr_window_open_files_with_command(FrWindow *window, GList *file_list,
                                       char *command) {
  GAppInfo *app;
  GError *error = NULL;

  app = g_app_info_create_from_commandline(command, NULL,
                                           G_APP_INFO_CREATE_NONE, &error);
  if (error != NULL) {
    _gtk_error_dialog_run(GTK_WINDOW(window),
                          _("Could not perform the operation"), "%s",
                          error->message);
    g_clear_error(&error);
    return;
  }
...
void fr_window_open_files_with_application(FrWindow *window, GList *file_list,
                                           GAppInfo *app) {
  GList *uris;
  GList *scan;
  ...
  if (!g_app_info_launch_uris(app, uris, G_APP_LAUNCH_CONTEXT(context),
                              &error)) {
    _gtk_error_dialog_run(GTK_WINDOW(window),
                          _("Could not perform the operation"), "%s",
                          error->message);
...
static OpenFilesData *open_files_data_new(FrWindow *window, GList *file_list,
                                          gboolean ask_application)

{
  ...
  for (scan = file_list; scan; scan = scan->next) {
    ...
    filename = g_strconcat(odata->cdata->temp_dir, "/", file, NULL);
...
static void open_files_data_free(OpenFilesData *odata) {
...
gboolean fr_window_update_files(FrWindow *window, GList *file_list) {
  GList *scan;

  if (window->priv->activity_ref > 0) return FALSE;

  if (window->archive->read_only) return FALSE;
  ...
  for (scan = file_list; scan; scan = scan->next) {
    ...
    fr_archive_add(window->archive, local_file_list, file->temp_dir, "/", FALSE,
                   FALSE, window->priv->password, window->priv->encrypt_header,
                   window->priv->compression, window->priv->volume_size);
...
static void open_file_modified_cb(GFileMonitor *monitor, GFile *monitor_file,
                                  GFile *other_file,
...
static void fr_window_monitor_open_file(FrWindow *window, OpenFile *file) {
  GFile *f;
  ...
  g_signal_connect(file->monitor, "changed", G_CALLBACK(open_file_modified_cb),
                   window);
  g_object_unref(f);
}

static void monitor_extracted_files(OpenFilesData *odata) {
  FrWindow *window = odata->window;
  GList *scan1, *scan2;

  for (scan1 = odata->file_list, scan2 = odata->cdata->file_list;
       scan1 && scan2; scan1 = scan1->next, scan2 = scan2->next) {
    OpenFile *ofile;
    ...
    if (ofile != NULL) fr_window_monitor_open_file(window, ofile);
  }
}

static gboolean fr_window_open_extracted_files(OpenFilesData *odata) {
  ...
  if (first_file == NULL) return FALSE;

  if (!odata->window->archive->read_only) monitor_extracted_files(odata);
  ...
  files_to_open =
      g_list_append(files_to_open, g_filename_to_uri(first_file, NULL, NULL));
  ...
  if (g_app_info_supports_files(app)) {
    ...
    for (scan = file_list->next; scan; scan = scan->next) {
      ...
      if (strcmp(mime_type, first_mime_type) == 0) {
        files_to_open =
            g_list_append(files_to_open, g_filename_to_uri(path, NULL, NULL));
      } else {
        GAppInfo *app2;
  ...
  context = gdk_display_get_app_launch_context(
      gtk_widget_get_display(GTK_WIDGET(odata->window)));
  gdk_app_launch_context_set_screen(
      context, gtk_widget_get_screen(GTK_WIDGET(odata->window)));
  gdk_app_launch_context_set_timestamp(context, 0);
  result = g_app_info_launch_uris(app, files_to_open,
...
static void fr_window_open_files__extract_done_cb(FrArchive *archive,
                                                  FrAction action,
                                                  FrProcError *error,
                                                  gpointer callback_data) {
  OpenFilesData *odata = callback_data;

  g_signal_handlers_disconnect_matched(G_OBJECT(archive), G_SIGNAL_MATCH_DATA,
                                       0, 0, NULL, 0, odata);

  if (error->type == FR_PROC_ERROR_NONE) fr_window_open_extracted_files(odata);
}

void fr_window_open_files(FrWindow *window, GList *file_list,
                          gboolean ask_application) {
  OpenFilesData *odata;
  ...
  fr_window_set_current_batch_action(window, FR_BATCH_ACTION_OPEN_FILES, odata,
                                     (GFreeFunc)open_files_data_free);

  g_signal_connect(G_OBJECT(window->archive), "done",
                   G_CALLBACK(fr_window_open_files__extract_done_cb), odata);

  fr_process_clear(window->archive->process);
  fr_archive_extract_to_local(window->archive, odata->file_list,
                              odata->cdata->temp_dir, NULL, FALSE, TRUE, FALSE,
                              window->priv->password);
  fr_process_start(window->archive->process);
}
...
static char *get_default_dir(const char *dir) {
  if (!is_temp_dir(dir))
    return g_strdup(dir);
  else
    return NULL;
...
void fr_window_set_open_default_dir(FrWindow *window, const char *default_dir) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_add_default_dir(FrWindow *window, const char *default_dir) {
  g_return_if_fail(window != NULL);
  g_return_if_fail(default_dir != NULL);
...
void fr_window_set_extract_default_dir(FrWindow *window,
  ...
  if (window->priv->extract_interact_use_default_dir) return;
...
void fr_window_set_default_dir(FrWindow *window, const char *default_dir,
                               gboolean freeze) {
  g_return_if_fail(window != NULL);
  g_return_if_fail(default_dir != NULL);
...
void fr_window_update_columns_visibility(FrWindow *window) {
  ...
  gtk_tree_view_column_set_visible(
      column, g_settings_get_boolean(window->priv->settings_listing,
                                     PREF_LISTING_SHOW_SIZE));

  column = gtk_tree_view_get_column(tree_view, 2);
  gtk_tree_view_column_set_visible(
      column, g_settings_get_boolean(window->priv->settings_listing,
                                     PREF_LISTING_SHOW_TYPE));

  column = gtk_tree_view_get_column(tree_view, 3);
  gtk_tree_view_column_set_visible(
      column, g_settings_get_boolean(window->priv->settings_listing,
                                     PREF_LISTING_SHOW_TIME));

  column = gtk_tree_view_get_column(tree_view, 4);
  gtk_tree_view_column_set_visible(
      column, g_settings_get_boolean(window->priv->settings_listing,
                                     PREF_LISTING_SHOW_PATH));
}

void fr_window_set_toolbar_visibility(FrWindow *window, gboolean visible) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_statusbar_visibility(FrWindow *window, gboolean visible) {
  g_return_if_fail(window != NULL);
...
void fr_window_set_folders_visibility(FrWindow *window, gboolean value) {
  g_return_if_fail(window != NULL);
...
void fr_window_use_progress_dialog(FrWindow *window, gboolean value) {
...
static void fr_window_exec_batch_action(FrWindow *window,
                                        FRBatchAction *action) {
  ExtractData *edata;
  RenameData *rdata;
  OpenFilesData *odata;
  ...
  switch (action->type) {
    case FR_BATCH_ACTION_LOAD:
      ...
      if (!uri_exists((char *)action->data))
        ...
        fr_window_archive_open(window, (char *)action->data,
                               GTK_WINDOW(window));
      break;
    ...
    case FR_BATCH_ACTION_EXTRACT:
      ...
      edata = action->data;
      fr_window_archive_extract(window, edata->file_list, edata->extract_to_dir,
                                edata->base_dir, edata->skip_older,
                                edata->overwrite, edata->junk_paths, TRUE);
      break;

    case FR_BATCH_ACTION_EXTRACT_HERE:
      debug(DEBUG_INFO, "[BATCH] EXTRACT HERE\n");

      fr_window_archive_extract_here(window, FALSE, FR_OVERWRITE_NO, FALSE);
      break;
    ...
    case FR_BATCH_ACTION_EXTRACT_INTERACT:
      ...
      if (window->priv->extract_interact_use_default_dir &&
        ...
        fr_window_archive_extract(window, NULL,
                                  window->priv->extract_default_dir, NULL,
                                  FALSE, FR_OVERWRITE_ASK, FALSE, TRUE);
      } else {
    ...
    case FR_BATCH_ACTION_RENAME:
      ...
      rdata = action->data;
      rename_selection(window, rdata->path_to_rename, rdata->old_name,
                       rdata->new_name, rdata->current_dir, rdata->is_dir,
                       rdata->dir_in_archive, rdata->original_path);
      break;

    case FR_BATCH_ACTION_PASTE:
      debug(DEBUG_INFO, "[BATCH] PASTE\n");

      fr_window_paste_from_clipboard_data(window,
                                          (FrClipboardData *)action->data);
      break;
    ...
    case FR_BATCH_ACTION_SAVE_AS:
      ...
      fr_window_archive_save_as(window, sdata->uri, sdata->password,
                                sdata->encrypt_header, sdata->volume_size);
      break;
    ...
    case FR_BATCH_ACTION_QUIT:
      ...
      g_signal_emit(window, fr_window_signals[READY], 0, NULL);

      if ((window->priv->progress_dialog != NULL) &&
          (gtk_widget_get_parent(window->priv->progress_dialog) !=
           GTK_WIDGET(window)))
...
void fr_window_set_current_batch_action(FrWindow *window,
                                        FrBatchActionType action, void *data,
                                        GFreeFunc free_func) {
  FRBatchAction *adata = &window->priv->current_batch_action;
...
void fr_window_append_batch_action(FrWindow *window, FrBatchActionType action,
                                   void *data, GFreeFunc free_func) {
  ...
  window->priv->batch_action_list =
      g_list_append(window->priv->batch_action_list, a_desc);
}
...
void fr_window_start_batch(FrWindow *window) {
  ...
  if (window->priv->batch_action_list == NULL) return;
  ...
  window->priv->batch_action = window->priv->batch_action_list;
  window->archive->can_create_compressed_file =
      window->priv->batch_adding_one_file;

  fr_window_exec_current_batch_action(window);
}

void fr_window_stop_batch(FrWindow *window) {
  if (!window->priv->non_interactive) return;
...
void fr_window_new_batch(FrWindow *window, const char *title) {
  fr_window_free_batch_data(window);
...
void fr_window_set_batch__extract_here(FrWindow *window, const char *filename) {
  g_return_if_fail(window != NULL);
  g_return_if_fail(filename != NULL);

  fr_window_append_batch_action(window, FR_BATCH_ACTION_LOAD,
                                g_strdup(filename), (GFreeFunc)g_free);
  fr_window_append_batch_action(window, FR_BATCH_ACTION_EXTRACT_HERE,
                                extract_to_data_new(NULL),
                                (GFreeFunc)extract_data_free);
  fr_window_append_batch_action(window, FR_BATCH_ACTION_CLOSE, NULL, NULL);
}

void fr_window_set_batch__extract(FrWindow *window, const char *filename,
                                  const char *dest_dir) {
  g_return_if_fail(window != NULL);
  g_return_if_fail(filename != NULL);
  ...
  fr_window_append_batch_action(window, FR_BATCH_ACTION_LOAD,
                                g_strdup(filename), (GFreeFunc)g_free);
  if (dest_dir != NULL)
    fr_window_append_batch_action(window, FR_BATCH_ACTION_EXTRACT,
                                  extract_to_data_new(dest_dir),
                                  (GFreeFunc)extract_data_free);
  else
    fr_window_append_batch_action(window, FR_BATCH_ACTION_EXTRACT_INTERACT,
                                  NULL, NULL);
  fr_window_append_batch_action(window, FR_BATCH_ACTION_CLOSE, NULL, NULL);
}

void fr_window_set_batch__add(FrWindow *window, const char *archive,
                              GList *file_list) {
  window->priv->batch_adding_one_file =
      (file_list->next == NULL) && (uri_is_file(file_list->data));

  if (archive != NULL)
    fr_window_append_batch_action(window, FR_BATCH_ACTION_LOAD,
                                  g_strdup(archive), (GFreeFunc)g_free);
  else
    fr_window_append_batch_action(window, FR_BATCH_ACTION_OPEN, file_list,
                                  NULL);
  fr_window_append_batch_action(window, FR_BATCH_ACTION_ADD, file_list, NULL);
  fr_window_append_batch_action(window, FR_BATCH_ACTION_CLOSE, NULL, NULL);
}
