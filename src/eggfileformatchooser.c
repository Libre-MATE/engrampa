...
enum { SIGNAL_SELECTION_CHANGED, SIGNAL_LAST };
...
G_DEFINE_TYPE_WITH_PRIVATE(EggFileFormatChooser, egg_file_format_chooser,
                           GTK_TYPE_EXPANDER);
static EGG_DEFINE_QUARK(EggFileFormatFilterInfo, egg_file_format_filter_info);

static EggFileFormatFilterInfo *egg_file_format_filter_info_new(
    const gchar *name, gboolean show_extensions) {
...
static void egg_file_format_filter_info_free(gpointer boxed) {
  if (boxed) {
...
static gboolean egg_file_format_filter_filter(const GtkFileFilterInfo *info,
                                              gpointer data) {
  EggFileFormatFilterInfo *self = data;

  return NULL != g_hash_table_find(self->extension_set,
                                   egg_file_format_filter_find, (gpointer)info);
}

static GtkFileFilter *egg_file_format_filter_new(const gchar *name,
                                                 gboolean show_extensions) {
  ...
  gtk_file_filter_add_custom(filter, GTK_FILE_FILTER_FILENAME,
                             egg_file_format_filter_filter, info, NULL);
  g_object_set_qdata_full(G_OBJECT(filter), egg_file_format_filter_info_quark(),
                          info, egg_file_format_filter_info_free);

  return filter;
}

static void egg_file_format_filter_add_extensions(GtkFileFilter *filter,
  ...
  info =
      g_object_get_qdata(G_OBJECT(filter), egg_file_format_filter_info_quark());

  info->extension_list =
      g_slist_prepend(info->extension_list, g_strdup(extensions));
  ...
  for (ptr = strings; *ptr; ptr++) {
    ...
    if (filter_name) {
      if (ptr != strings) g_string_append(filter_name, ", ");
...
static void selection_changed_cb(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    if (self->priv->chooser) {
      while (gtk_tree_model_iter_parent(model, &parent, &iter)) iter = parent;
...
static gboolean select_default_file_format(gpointer data) {
...
static gboolean find_in_list(gchar *list, const gchar *needle) {
  gchar *saveptr;
  gchar *token;
  ...
  for (token = strtok_r(list, ",", &saveptr); NULL != token;
    ...
    if (strcasecmp(needle, token) == 0) return TRUE;
  }

  return FALSE;
}

static gboolean accept_filename(gchar *extensions, const gchar *filename) {
  gchar *saveptr;
  gchar *token;
  gsize length;
  ...
  for (token = strtok_r(extensions, ",", &saveptr); NULL != token;
    ...
    if (extptr > filename && '.' == *extptr && !strcmp(extptr + 1, token))
      return TRUE;
  }
...
static gboolean find_by_extension(GtkTreeModel *model,
  ...
  gtk_tree_model_get(model, iter, MODEL_COLUMN_EXTENSIONS, &extensions,
                     MODEL_COLUMN_ID, &format, -1);
...
static int emit_default_size_changed(gpointer user_data) {
...
static void expander_unmap_cb(GtkWidget *widget, gpointer user_data) {
  EggFileFormatChooser *self = user_data;

  if (self->priv->size_changed_event == 0)
    self->priv->size_changed_event =
        gdk_threads_add_idle(emit_default_size_changed, self);
}

static void egg_file_format_chooser_init(EggFileFormatChooser *self) {
  GtkWidget *scroller;
  GtkWidget *view;
  ...
  self->priv->supported_files =
      egg_file_format_filter_new(_("All Supported Files"), FALSE);

  /* tree model */

  self->priv->model = gtk_tree_store_new(
      7, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
      GTK_TYPE_FILE_FILTER, G_TYPE_POINTER, G_TYPE_POINTER);

  gtk_tree_store_append(self->priv->model, &iter, NULL);
  gtk_tree_store_set(self->priv->model, &iter, MODEL_COLUMN_NAME,
                     _("By Extension"), MODEL_COLUMN_FILTER,
                     self->priv->supported_files, MODEL_COLUMN_ID, 0, -1);
  ...
  gtk_tree_view_column_set_attributes(column, cell, "icon-name",
                                      MODEL_COLUMN_ICON, NULL);

  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, cell, TRUE);
  gtk_tree_view_column_set_attributes(column, cell, "text", MODEL_COLUMN_NAME,
                                      NULL);

  /* extensions column */

  column = gtk_tree_view_column_new_with_attributes(
      _("Extension(s)"), gtk_cell_renderer_text_new(), "text",
      MODEL_COLUMN_EXTENSIONS, NULL);
  ...
  g_signal_connect_after(scroller, "unmap", G_CALLBACK(expander_unmap_cb),
                         self);
}

static void reset_model(EggFileFormatChooser *self) {
  ...
  if (gtk_tree_model_get_iter_first(model, &iter)) {
    do {
      ...
      gtk_tree_model_get(model, &iter, MODEL_COLUMN_DESTROY, &destroy,
                         MODEL_COLUMN_DATA, &data, -1);
...
static void egg_file_format_chooser_dispose(GObject *obj) {
...
static void egg_file_format_chooser_finalize(GObject *obj) {
  EggFileFormatChooser *self = EGG_FILE_FORMAT_CHOOSER(obj);
...
static void filter_changed_cb(GObject *object, GParamSpec *spec,
                              gpointer data) {
  EggFileFormatChooser *self;
  ...
  if (gtk_tree_selection_get_selected(self->priv->selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, MODEL_COLUMN_FILTER, &format_filter, -1);
  ...
  if (current_filter && current_filter != format_filter &&
    ...
    if (current_filter == self->priv->all_files)
      ...
      do {
        gtk_tree_model_get(model, &iter, MODEL_COLUMN_FILTER, &format_filter,
                           -1);
        g_object_unref(format_filter);

        if (format_filter == current_filter) break;
      } while (gtk_tree_model_iter_next(model, &iter));
    }
...
static void error_message_with_parent(GtkWindow *parent, const char *msg,
                                      const char *detail) {
  GtkWidget *dialog;

  g_warning("%s: Merge with the code in Gtk{File,Recent}ChooserDefault.",
            G_STRLOC);
...
static GtkWindow *get_toplevel(GtkWidget *widget) {
  GtkWidget *toplevel;
...
static void error_message(EggFileFormatChooser *impl, const char *msg,
                          const char *detail) {
  error_message_with_parent(get_toplevel(GTK_WIDGET(impl)), msg, detail);
}

static void chooser_response_cb(GtkDialog *dialog, gint response_id,
                                gpointer data) {
  EggFileFormatChooser *self;
  ...
  if (EGG_IS_POSITIVE_RESPONSE(response_id)) {
    ...
    if (0 == format) {
      ...
      error_message(self, _("File format not recognized"), message);
      ...
      filename =
          egg_file_format_chooser_append_extension(self, basename, format);
...
static void egg_file_format_chooser_realize(GtkWidget *widget) {
  EggFileFormatChooser *self;
  GtkWidget *parent;
  ...
  gtk_file_chooser_set_filter(self->priv->chooser, self->priv->supported_files);
...
static void egg_file_format_chooser_unrealize(GtkWidget *widget) {
  EggFileFormatChooser *self;
  ...
  g_signal_handlers_disconnect_by_func(self->priv->chooser, filter_changed_cb,
                                       self);
  g_signal_handlers_disconnect_by_func(self->priv->chooser, chooser_response_cb,
                                       self);
...
static void egg_file_format_chooser_class_init(EggFileFormatChooserClass *cls) {
  ...
  signals[SIGNAL_SELECTION_CHANGED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(EggFileFormatChooserClass, selection_changed), NULL, NULL,
...
static guint egg_file_format_chooser_add_format_impl(EggFileFormatChooser *self,
                                                     guint parent,
                                                     const gchar *name,
                                                     const gchar *icon,
                                                     const gchar *extensions) {
  ...
  if (parent > 0) {
    gtk_tree_model_foreach(GTK_TREE_MODEL(self->priv->model), find_by_format,
                           &search);
  ...
  gtk_tree_store_set(self->priv->model, &iter, MODEL_COLUMN_ID,
                     ++self->priv->last_id, MODEL_COLUMN_EXTENSIONS, extensions,
                     MODEL_COLUMN_FILTER, filter, MODEL_COLUMN_NAME, name,
                     MODEL_COLUMN_ICON, icon, -1);
  ...
  if (extensions) {
    ...
    egg_file_format_filter_add_extensions(self->priv->supported_files,
                                          extensions);
    egg_file_format_filter_add_extensions(filter, extensions);

    if (parent > 0) g_object_unref(filter);
  }
...
static gchar *get_icon_name(const gchar *mime_type) {
  ...
  if (first_call) {
    g_warning(
        "%s: Replace by g_content_type_get_icon "
        "when GVFS is merged into GLib.",
        G_STRLOC);
    first_call = FALSE;
  }
  ...
  if (mime_type) {
    ...
    for (s = name; *s; ++s) {
      if (!isalpha(*s) || !isascii(*s)) *s = '-';
    }
  }

  if (!name || !gtk_icon_theme_has_icon(gtk_icon_theme_get_default(), name)) {
    g_free(name);
...
void egg_file_format_chooser_add_pixbuf_formats(EggFileFormatChooser *self,
  ...
  if (formats) *formats = g_new0(guint, g_slist_length(pixbuf_formats) + 1);
...
void egg_file_format_chooser_remove_format(EggFileFormatChooser *self,
  ...
  gtk_tree_model_get(model, &search.iter, MODEL_COLUMN_FILTER, &filter,
                     MODEL_COLUMN_DESTROY, &destroy, MODEL_COLUMN_DATA, &data,
                     -1);
...
guint egg_file_format_chooser_get_format(EggFileFormatChooser *self,
  ...
  if (0 == format && NULL != filename) {
    ...
    if (search.success) format = search.format;
  }
...
void egg_file_format_chooser_set_format_data(EggFileFormatChooser *self,
  ...
  gtk_tree_store_set(self->priv->model, &search.iter, MODEL_COLUMN_DESTROY,
                     destroy, MODEL_COLUMN_DATA, data, -1);
}
...
gpointer egg_file_format_chooser_get_format_data(EggFileFormatChooser *self,
  ...
  gtk_tree_model_get(model, &search.iter, MODEL_COLUMN_DATA, &data, -1);
  return data;
}
...
gchar *egg_file_format_chooser_append_extension(EggFileFormatChooser *self,
  ...
  if (0 == format) format = egg_file_format_chooser_get_format(self, NULL);

  if (0 == format) {
    g_warning("%s: No file format selected. Cannot append extension.",
              G_STRFUNC);
    return NULL;
  }
  ...
  gtk_tree_model_get(model, &search.iter, MODEL_COLUMN_EXTENSIONS, &extensions,
                     -1);

  if (NULL == extensions &&
      gtk_tree_model_iter_nth_child(model, &child, &search.iter, 0)) {
    gtk_tree_model_get(model, &child, MODEL_COLUMN_EXTENSIONS, &extensions, -1);
  }

  if (NULL == extensions) {
    g_warning(
        "%s: File format %d doesn't provide file extensions. "
        "Cannot append extension.",
        G_STRFUNC, format);
    return NULL;
  }
