...
static time_t mktime_from_string(char *date, char *time) {
  struct tm tm = {
      0,
  };
  char **fields;
...
static void process_line(char *line, gpointer data) {
  ...
  if (!ace_comm->list_started) {
    if (ace_comm->command_type == FR_ACE_COMMAND_PUBLIC) {
      if (g_str_has_prefix(line, "Date")) ace_comm->list_started = TRUE;
  ...
  if (g_str_has_prefix(line, "listed")) return;
  ...
  if ((fields == NULL) || (g_strv_length(fields) < 5)) return;
...
static void fr_command_ace_list(FrCommand *comm) {
...
static void fr_command_ace_extract(FrCommand *comm, const char *from_file,
...
static void fr_command_ace_test(FrCommand *comm) {
  fr_process_begin_command(comm->process, "unace");
...
static void fr_command_ace_handle_error(FrCommand *comm, FrProcError *error) {
  /* FIXME */
}

const char *ace_mime_type[] = {"application/x-ace", NULL};

static const char **fr_command_ace_get_mime_types(FrCommand *comm) {
  return ace_mime_type;
}

static FrCommandCaps fr_command_ace_get_capabilities(FrCommand *comm,
...
static const char *fr_command_ace_get_packages(FrCommand *comm,
                                               const char *mime_type) {
...
static void fr_command_ace_init(FrCommand *comm) {
...
GType fr_command_ace_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandAceClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandAce", &type_info, 0);
  }
