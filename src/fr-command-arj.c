...
static void list__process_line(char *line, gpointer data) {
  ...
  if (g_regex_match(arj_comm->filename_line_regex, line, 0,
                    NULL)) { /* Read the filename. */
    FileData *fdata;
...
static void fr_command_arj_list(FrCommand *comm) {
...
static void fr_command_arj_add(FrCommand *comm, const char *from_file,
...
static void fr_command_arj_delete(FrCommand *comm, const char *from_file,
                                  GList *file_list) {
  GList *scan;
...
static void fr_command_arj_extract(FrCommand *comm, const char *from_file,
  ...
  if (!overwrite) fr_process_add_arg(comm->process, "-n");
...
static void fr_command_arj_test(FrCommand *comm) {
  fr_process_begin_command(comm->process, "arj");
...
static void fr_command_arj_handle_error(FrCommand *comm, FrProcError *error) {
...
static const char **fr_command_arj_get_mime_types(FrCommand *comm) {
  return arj_mime_type;
}

static FrCommandCaps fr_command_arj_get_capabilities(FrCommand *comm,
...
static const char *fr_command_arj_get_packages(FrCommand *comm,
                                               const char *mime_type) {
  return PACKAGES("arj");
}
...
static void fr_command_arj_init(FrCommand *comm) {
  ...
  arj_comm->filename_line_regex =
      g_regex_new("[0-9]+\\) ", G_REGEX_OPTIMIZE, 0, NULL);
}
...
GType fr_command_arj_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandArjClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandArj", &type_info, 0);
  }
