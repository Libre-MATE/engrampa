...
static void installer_data_free(InstallerData *idata) {
  g_object_unref(idata->archive);
  g_object_unref(idata->window);
  g_free(idata);
}

static void package_installer_terminated(InstallerData *idata,
                                         FrProcErrorType error_type,
                                         const char *error_message) {
  GdkWindow *window;

  window = gtk_widget_get_window(GTK_WIDGET(idata->window));
  if (window != NULL) gdk_window_set_cursor(window, NULL);

  if (error_type != FR_PROC_ERROR_NONE) {
    fr_archive_action_completed(idata->archive, idata->action, error_type,
                                error_message);
  } else {
    ...
    if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED) ||
        (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_DBUS_ERROR) &&
         ...
         (strstr(error->message,
                 "org.freedesktop.Packagekit.Modify.Cancelled") != NULL))) {
...
static char **get_packages_real_names(char **names) {
  char **real_names;
  GKeyFile *key_file;
  int i;
  ...
  g_key_file_load_from_file(key_file, PRIVDATADIR "/packages.match",
  ...
  for (i = 0; names[i] != NULL; i++) {
    ...
    real_name =
        g_key_file_get_string(key_file, "Package Matches", names[i], NULL);
    if (real_name != NULL) real_name = g_strstrip(real_name);
...
static void install_packages(InstallerData *idata) {
  ...
  if (connection != NULL) {
    ...
    proxy = g_dbus_proxy_new_sync(
        connection, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.freedesktop.PackageKit",
    ...
    if (proxy != NULL) {
      ...
      g_dbus_proxy_call(
          proxy, "InstallPackageNames",
          g_variant_new("(u^ass)", xid, real_names,
                        "hide-confirm-search,hide-finished,hide-warning"),
          G_DBUS_CALL_FLAGS_NONE, G_MAXINT, NULL,
          packagekit_install_package_names_ready_cb, idata);
...
static void confirm_search_dialog_response_cb(GtkDialog *dialog,
                                              int response_id,
                                              gpointer user_data) {
  InstallerData *idata = user_data;
...
static GtkWidget *create_button(const char *icon_name, const char *text) {
...
void dlg_package_installer(FrWindow *window, FrArchive *archive,
  ...
  command_type = get_preferred_command_for_mime_type(
      idata->archive->content_type, FR_COMMAND_CAN_READ_WRITE);
  if (command_type == 0)
    command_type = get_preferred_command_for_mime_type(
        idata->archive->content_type, FR_COMMAND_CAN_READ);
  if (command_type == 0) {
    package_installer_terminated(idata, FR_PROC_ERROR_GENERIC,
  ...
  idata->packages =
      fr_command_get_packages(command, idata->archive->content_type);
...

  {
    ...
    secondary_text =
        g_strdup_printf(_("There is no command installed for %s files.\nDo you "
                          "want to search for a command to open this file?"),
                        description);
    dialog = _gtk_message_dialog_new(
        GTK_WINDOW(idata->window), GTK_DIALOG_MODAL, "dialog-error",
        _("Could not open this file type"), secondary_text, NULL);
    ...
    gtk_dialog_add_action_widget(
        GTK_DIALOG(dialog), create_button("edit-find", _("_Search Command")),
        GTK_RESPONSE_YES);

    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_YES);

    g_signal_connect(dialog, "response",
                     G_CALLBACK(confirm_search_dialog_response_cb), idata);
