...
static void list__process_line(char *line, gpointer data) {
  ...
  if (line[0] == 'D') {
    ...
    if ((name_field == NULL) || (strcmp(name_field, ".") == 0) ||
        (strcmp(name_field, "..") == 0)) {
      file_data_free(fdata);
      return;
    }
    ...
    if (comm_iso->cur_path[0] != '/')
      fdata->full_path =
          g_strstrip(g_strconcat("/", comm_iso->cur_path, name_field, NULL));
    else
      fdata->full_path =
          g_strstrip(g_strconcat(comm_iso->cur_path, name_field, NULL));
...
static void fr_command_iso_list(FrCommand *comm) {
...
static void fr_command_iso_extract(FrCommand *comm, const char *from_file,
  ...
  for (scan = file_list; scan; scan = scan->next) {
    ...
    if (temp_dest_dir == NULL) continue;
...
static const char **fr_command_iso_get_mime_types(FrCommand *comm) {
  return iso_mime_type;
}

static FrCommandCaps fr_command_iso_get_capabilities(FrCommand *comm,
...
static const char *fr_command_iso_get_packages(FrCommand *comm,
                                               const char *mime_type) {
...
static void fr_command_iso_init(FrCommand *comm) {
  FrCommandIso *comm_iso = FR_COMMAND_ISO(comm);
...
GType fr_command_iso_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandIsoClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandIso", &type_info, 0);
  }
