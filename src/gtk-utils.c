...
GtkWidget *_gtk_message_dialog_new(GtkWindow *parent, GtkDialogFlags flags,
  ...
  if (icon_name == NULL) icon_name = "dialog-information";
  ...
  if (message != NULL) {
    ...
    if (secondary_message != NULL) {
      char *escaped_secondary_message =
          g_markup_escape_text(secondary_message, -1);
      markup_text = g_strdup_printf(
          "<span weight=\"bold\" size=\"larger\">%s</span>\n\n%s",
          escaped_message, escaped_secondary_message);
  ...
  if (first_button_text == NULL) return dialog;
...
char *_gtk_request_dialog_run(GtkWindow *parent, GtkDialogFlags flags,
...
GtkWidget *_gtk_error_dialog_new(GtkWindow *parent, GtkDialogFlags flags,
  ...
  dialog = gtk_dialog_new_with_buttons("", parent, flags, "gtk-ok",
  ...
  if (view_output) {
    ...
    gtk_text_buffer_create_tag(text_buf, "monospace", "family", "monospace",
                               NULL);
    ...
    for (scan = row_output; scan; scan = scan->next) {
      ...
      gtk_text_buffer_insert_with_tags_by_name(
          text_buf, &iter, utf8_line, bytes_written, "monospace", NULL);
      g_free(utf8_line);
...
void _gtk_error_dialog_run(GtkWindow *parent, const gchar *main_message,
  ...
  d = _gtk_message_dialog_new(parent, GTK_DIALOG_MODAL, "dialog-error",
                              main_message, message, "gtk-close",
...
GdkPixbuf *get_icon_pixbuf(GIcon *icon, int size, GtkIconTheme *theme) {
...
void show_help_dialog(GtkWindow *parent, const char *section) {
  ...
  if (!gtk_show_uri_on_window(parent, uri, GDK_CURRENT_TIME, &error)) {
    ...
    dialog = _gtk_message_dialog_new(
        parent, GTK_DIALOG_DESTROY_WITH_PARENT, "dialog-error",
        _("Could not display help"), error->message, "gtk-ok", GTK_RESPONSE_OK,
        NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_OK);
