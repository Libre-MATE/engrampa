...
static void ok_clicked_cb(GtkWidget *widget, DialogData *data) {
  ...
  selected_files = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(data->d_selected_files_radio));
  pattern_files =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(data->d_files_radio));
  ...
  if (selected_files) {
    ...
    if (file_list == NULL) do_not_remove_if_null = TRUE;
  }
...
static void entry_changed_cb(GtkWidget *widget, DialogData *data) {
...
static void dlg_delete__common(FrWindow *window, GList *selected_files) {
  GtkBuilder *builder;
  DialogData *data;
  ...
  builder = gtk_builder_new_from_resource(
      ENGRAMPA_RESOURCE_UI_PATH G_DIR_SEPARATOR_S "delete.ui");
  ...
  if (data->selected_files != NULL)
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(data->d_selected_files_radio), TRUE);
  else {
    gtk_widget_set_sensitive(data->d_selected_files_radio, FALSE);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data->d_all_files_radio),
                                 TRUE);
  }

  /* Set the signals handlers. */
  gtk_builder_add_callback_symbols(
      builder, "on_delete_dialog_destroy", G_CALLBACK(destroy_cb),
      "on_d_ok_button_clicked", G_CALLBACK(ok_clicked_cb),
      "on_d_files_entry_changed", G_CALLBACK(entry_changed_cb), NULL);
...
void dlg_delete(GtkWidget *widget, gpointer callback_data) {
  FrWindow *window = callback_data;
  dlg_delete__common(window,
                     fr_window_get_file_list_selection(window, TRUE, NULL));
}

void dlg_delete_from_sidebar(GtkWidget *widget, gpointer callback_data) {
  FrWindow *window = callback_data;
  dlg_delete__common(window,
