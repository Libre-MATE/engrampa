...
void file_data_free(FileData *fdata) {
  ...
  if (fdata->free_original_path) g_free(fdata->original_path);
...
GType file_data_get_type(void) {
  ...
  if (type == 0)
    type = g_boxed_type_register_static("FRFileData",
                                        (GBoxedCopyFunc)file_data_copy,
                                        (GBoxedFreeFunc)file_data_free);
...
int find_path_in_file_data_array(GPtrArray *array, const char *path) {
  ...
  while (left < right) {
    ...
    if (cmp != 0) {
      ...
      if ((path_l == original_path_l - 1) &&
          (fd->original_path[original_path_l - 1] == '/')) {
        int cmp2;
