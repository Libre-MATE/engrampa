...
static const char introspection_xml[] =
     ...
     *   *) create_single_file: create an archive that can contain a single
     * file.
...
static void window_ready_cb(GtkWidget *widget, GError *error,
                            gpointer user_data) {
  if (error == NULL)
    g_dbus_method_invocation_return_value((GDBusMethodInvocation *)user_data,
                                          NULL);
  else
    g_dbus_method_invocation_return_error((GDBusMethodInvocation *)user_data,
                                          error->domain, error->code, "%s",
                                          error->message);
}

static gboolean window_progress_cb(FrWindow *window, double fraction,
                                   char *details, gpointer user_data) {
  GDBusConnection *connection = user_data;

  g_dbus_connection_emit_signal(connection, NULL, FR_SERVICE_PATH,
                                "org.mate.ArchiveManager", "Progress",
                                g_variant_new("(ds)", fraction, details), NULL);
  ...
  if (g_strcmp0(method_name, "GetSupportedTypes") == 0) {
    ...
    if (supported_types == NULL) {
      g_dbus_method_invocation_return_error(
          ...
          "Action not valid %s, valid values are: create, create_single_file, "
          "extract",
          action);
    } else {
      ...
      for (i = 0; supported_types[i] != -1; i++) {
        ...
        g_variant_builder_add(&builder, "{ss}", "mime-type",
                              mime_type_desc[supported_types[i]].mime_type);
        g_variant_builder_add(&builder, "{ss}", "default-extension",
                              mime_type_desc[supported_types[i]].default_ext);
        g_variant_builder_add(&builder, "{ss}", "description",
    ...
    g_variant_get(parameters, "(s^asb)", &archive, &files,
                  &use_progress_dialog);
    ...
    g_signal_connect(window, "progress", G_CALLBACK(window_progress_cb),
                     connection);
    ...
    fr_window_append_batch_action(FR_WINDOW(window), FR_BATCH_ACTION_QUIT, NULL,
                                  NULL);
    ...
    g_variant_get(parameters, "(^assb)", &files, &destination,
                  &use_progress_dialog);
    ...
    g_variant_get(parameters, "(ssb)", &archive, &destination,
                  &use_progress_dialog);
...
static void on_bus_acquired(GDBusConnection *connection, const char *name,
  ...
  registration_id = g_dbus_connection_register_object(
      connection, FR_SERVICE_PATH, introspection_data->interfaces[0],
...
static void on_name_acquired(GDBusConnection *connection, const char *name,
                             gpointer user_data) {}

static void on_name_lost(GDBusConnection *connection, const char *name,
                         gpointer user_data) {
  gtk_main_quit();
}
...
int main(int argc, char *argv[]) {
  ...
  owner_id = g_bus_own_name(G_BUS_TYPE_SESSION, FR_SERVICE_NAME,
