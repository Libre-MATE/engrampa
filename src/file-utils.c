...
static gboolean uri_is_filetype(const char *uri, GFileType file_type) {
  ...
  info =
      g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_TYPE, 0, NULL, &error);
  if (error == NULL) {
...
char *get_dir_content_if_unique(const char *uri) {
  ...
  file_enum = g_file_enumerate_children(file, G_FILE_ATTRIBUTE_STANDARD_NAME, 0,
                                        NULL, &err);
  if (err != NULL) {
  ...
  while ((info = g_file_enumerator_next_file(file_enum, NULL, &err)) != NULL) {
    ...
    if (err != NULL) {
      g_warning("Failed to get info while enumerating children: %s",
                err->message);
      g_clear_error(&err);
      g_object_unref(info);
      continue;
...
gboolean path_in_path(const char *dirname, const char *filename) {
  int dirname_l, filename_l, separator_position;

  if ((dirname == NULL) || (filename == NULL)) return FALSE;

  dirname_l = strlen(dirname);
  filename_l = strlen(filename);

  if ((dirname_l == filename_l + 1) && (dirname[dirname_l - 1] == '/'))
    return FALSE;

  if ((filename_l == dirname_l + 1) && (filename[filename_l - 1] == '/'))
    return FALSE;
...
static time_t get_file_time_type(const char *uri, const char *type) {
  time_t result = 0;
  GFile *file;
  GFileInfo *info;
  GError *err = NULL;
...
const gchar *file_name_from_path(const gchar *file_name) {
  ...
  if ((file_name[0] == '\0') || (strlen(file_name) == 0)) return "";

  last_char = strlen(file_name) - 1;

  if (file_name[last_char] == G_DIR_SEPARATOR) return "";
...
char *dir_name_from_path(const gchar *path) {
  ...
  if (path[0] == '\0') return g_strdup("");

  last_char = strlen(path) - 1;
  if (path[last_char] == G_DIR_SEPARATOR) last_char--;

  base = last_char;
  while ((base >= 0) && (path[base] != G_DIR_SEPARATOR)) base--;
...
gboolean make_directory_tree(GFile *dir, mode_t mode, GError **error) {
  ...
  if ((dir == NULL) || g_file_query_exists(dir, NULL)) return TRUE;
  ...
  if (success)
    g_file_set_attribute_uint32(dir, G_FILE_ATTRIBUTE_UNIX_MODE, mode, 0, NULL,
                                NULL);
...
const char *get_file_extension(const char *filename) {
  ...
  if (ext - 4 > filename) {
    ...
    if (strncmp(test, ".tar", 4) == 0) ext = ext - 4;
  }
  return ext;
}
...
gboolean file_extension_is(const char *filename, const char *ext) {
  ...
  if (filename_l < ext_l) return FALSE;
  return strcasecmp(filename + filename_l - ext_l, ext) == 0;
}

gboolean is_mime_type(const char *mime_type, const char *pattern) {
...
guint64 get_dest_free_space(const char *path) {
  ...
  info = g_file_query_filesystem_info(file, G_FILE_ATTRIBUTE_FILESYSTEM_FREE,
                                      NULL, &err);
  if (info != NULL) {
    freespace = g_file_info_get_attribute_uint64(
        info, G_FILE_ATTRIBUTE_FILESYSTEM_FREE);
    g_object_unref(info);
  } else {
    g_warning(
        "Could not get filesystem free space on volume that contains %s: %s",
        path, err->message);
    g_error_free(err);
  }
  g_object_unref(file);
...
static gboolean delete_directory_recursive(GFile *dir, GError **error) {
  char *uri;
  GFileEnumerator *file_enum;
  GFileInfo *info;
  ...
  file_enum = g_file_enumerate_children(
      dir, G_FILE_ATTRIBUTE_STANDARD_NAME "," G_FILE_ATTRIBUTE_STANDARD_TYPE, 0,
      NULL, error);

  uri = g_file_get_uri(dir);
  while (!error_occurred &&
         (info = g_file_enumerator_next_file(file_enum, NULL, error)) != NULL) {
    char *child_uri;
    GFile *child;
    ...
    switch (g_file_info_get_file_type(info)) {
      case G_FILE_TYPE_DIRECTORY:
        if (!delete_directory_recursive(child, error)) error_occurred = TRUE;
        break;
      default:
        if (!g_file_delete(child, NULL, error)) error_occurred = TRUE;
        break;
    }
...
gboolean remove_local_directory(const char *path) {
  char *uri;
  gboolean result;
...
static char *ith_temp_folder_to_try(int n) {
  const char *folder;
...
char *get_temp_work_dir(const char *parent_folder) {
  ...
  if (best_folder == NULL) return NULL;
...
gboolean is_temp_dir(const char *dir) {
  if (strncmp(dir, "file://", 7) == 0) dir = dir + 7;
  if (strcmp(g_get_tmp_dir(), dir) == 0) return TRUE;
...
int file_list__get_index_from_pattern(const char *line, const char *pattern) {
  ...
  if ((pattern_l == 0) || (line_l == 0)) return -1;

  for (l = line; *l != 0; l++)
    if (file_list__match_pattern(l, pattern)) return (l - line);
...
gboolean check_permissions(const char *uri, int mode) {
  GFile *file;
  gboolean result;
  ...
  if (err != NULL) {
    ...
    if ((mode & R_OK) == R_OK) {
      if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_READ))
        result = (result && g_file_info_get_attribute_boolean(
...
gboolean is_program_in_path(const char *filename) {
  ...
  g_hash_table_insert(ProgramsCache, g_strdup(filename), result ? "1" : "0");
...
void gio_file_list_free(GList *l) {
  GList *scan;
  for (scan = l; scan; scan = scan->next) g_object_unref(scan->data);
  g_list_free(l);
}
...
void g_key_file_save(GKeyFile *key_file, GFile *file) {
  ...
  if (error != NULL) {
    ...
    if (stream == NULL) {
      g_warning("Could not save options: %s\n", error->message);
      g_clear_error(&error);
    } else if (!g_output_stream_write_all(G_OUTPUT_STREAM(stream), file_data,
                                          size, NULL, NULL, &error)) {
      g_warning("Could not save options: %s\n", error->message);
      g_clear_error(&error);
