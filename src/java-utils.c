...
typedef struct {
  ...
  GSList *const_pool_class; /* (const_pool_count - 1) elements of tye
                               'CONST_class_info' */
  GSList *const_pool_utf;   /* (const_pool_count - 1) elements of type
                               'utf_strings' */
...
static void java_class_file_free(JavaClassFile *cfile) {
  GSList *scan;
 ...
 * class file. */
static void load_constant_pool_utfs(JavaClassFile *cfile) {
  guint8 tag;
  guint16 i = 0; /* should be comparable with const_pool_count */

  while ((i < cfile->const_pool_count - 1) &&
         (read(cfile->fd, &tag, 1) != -1)) {
    ...
    switch (tag) {
      case CONST_CLASS:
        ...
        cfile->const_pool_class =
            g_slist_append(cfile->const_pool_class, class);
        break;
...
static char *close_and_exit(JavaClassFile *cfile) {
...
char *get_package_name_from_class_file(char *fname) {
  ...
  if (!g_file_test(fname, G_FILE_TEST_EXISTS)) return NULL;

  cfile = java_class_file_new();
  cfile->fd = open(fname, O_RDONLY);
  if (cfile->fd == -1) return close_and_exit(cfile);

  if ((i = read(cfile->fd, &magic, 4)) != 4) return close_and_exit(cfile);
  cfile->magic_no = GUINT32_FROM_BE(magic);

  if (read(cfile->fd, &major, 2) != 2) return close_and_exit(cfile);
  cfile->major = GUINT16_FROM_BE(major);

  if (read(cfile->fd, &minor, 2) != 2) return close_and_exit(cfile);
  cfile->minor = GUINT16_FROM_BE(minor);

  if (read(cfile->fd, &count, 2) != 2) return close_and_exit(cfile);
  ...
  if (read(cfile->fd, &cfile->this_class, 2) != 2) return close_and_exit(cfile);
  cfile->this_class = GUINT16_FROM_BE(cfile->this_class);

  /* now search for the class structure with index = cfile->this_class */

  for (i = 0; (i < g_slist_length(cfile->const_pool_class)) && (utf_index == 0);
       i++) {
...
static void consume_comment(int fdesc, gboolean multiline) {
...
char *get_package_name_from_java_file(char *fname) {
  ...
  while (!valid_char_found && (read(cfile->fd, &ch, 1) == 1)) {
    switch (ch) {
      ...
      case '*':
        if (prev_char_is_bslash == TRUE) consume_comment(cfile->fd, TRUE);
        prev_char_is_bslash = FALSE;
        break;
