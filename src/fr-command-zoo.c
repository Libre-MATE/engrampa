...
static time_t mktime_from_string_zoo(char *mday_s, char *month_s, char *year_s,
                                     char *time_s) {
  struct tm tm = {
      0,
  };
...
static char **split_line_zoo(char *line) {
  char **fields;
...
static const char *get_last_field_zoo(char *line) {
  const char *field;
  int i;
  int n = 6;
...
static void process_zoo_line(char *line, gpointer data) {
  FileData *fdata;
  ...
  fdata->modified =
      mktime_from_string_zoo(fields[1], fields[2], fields[3], fields[4]);
  g_strfreev(fields);
...
static void fr_command_zoo_list(FrCommand *zoo_comm) {
...
static void fr_command_zoo_add(FrCommand *comm, const char *from_file,
...
static void fr_command_zoo_delete(FrCommand *comm, const char *from_file,
                                  GList *file_list) {
  GList *scan;
...
static void fr_command_zoo_extract(FrCommand *comm, const char *from_file,
...
static void fr_command_zoo_test(FrCommand *comm) {
  fr_process_begin_command(comm->process, "zoo");
...
static const char **fr_command_zoo_get_mime_types(FrCommand *comm) {
  return zoo_mime_type;
}

static FrCommandCaps fr_command_zoo_get_capabilities(FrCommand *comm,
...
static const char *fr_command_zoo_get_packages(FrCommand *comm,
                                               const char *mime_type) {
  return PACKAGES("zoo");
}
...
static void fr_command_zoo_init(FrCommand *comm) {
...
GType fr_command_zoo_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandZooClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandZoo", &type_info, 0);
  }
