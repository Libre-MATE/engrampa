...
static DroppedItemsData *dropped_items_data_new(
    FrArchive *archive, GList *item_list, const char *base_dir,
    const char *dest_dir, gboolean update, const char *password,
    gboolean encrypt_header, FrCompression compression, guint volume_size) {
  DroppedItemsData *data;

  data = g_new0(DroppedItemsData, 1);
  data->archive = archive;
  data->item_list = path_list_dup(item_list);
  if (base_dir != NULL) data->base_dir = g_strdup(base_dir);
  if (dest_dir != NULL) data->dest_dir = g_strdup(dest_dir);
  data->update = update;
  if (password != NULL) data->password = g_strdup(password);
...
static void xfer_data_free(XferData *data) {
  if (data == NULL) return;
...
GType fr_archive_get_type(void) {
  ...
  if (!type) {
    static const GTypeInfo type_info = {sizeof(FrArchiveClass),
                                        NULL,
                                        NULL,
    ...
    type = g_type_register_static(G_TYPE_OBJECT, "FrArchive", &type_info, 0);
  }
...
static void fr_archive_class_init(FrArchiveClass *class) {
  ...
  fr_archive_signals[START] =
      g_signal_new("start", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrArchiveClass, start), NULL, NULL,
                   fr_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  fr_archive_signals[DONE] =
      g_signal_new("done", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrArchiveClass, done), NULL, NULL,
                   fr_marshal_VOID__INT_BOXED, G_TYPE_NONE, 2, G_TYPE_INT,
                   FR_TYPE_PROC_ERROR);
  fr_archive_signals[PROGRESS] =
      g_signal_new("progress", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrArchiveClass, progress), NULL, NULL,
                   fr_marshal_VOID__DOUBLE, G_TYPE_NONE, 1, G_TYPE_DOUBLE);
  fr_archive_signals[MESSAGE] =
      g_signal_new("message", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrArchiveClass, message), NULL, NULL,
                   fr_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  fr_archive_signals[STOPPABLE] =
      g_signal_new("stoppable", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrArchiveClass, stoppable), NULL, NULL,
                   fr_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
  fr_archive_signals[WORKING_ARCHIVE] = g_signal_new(
      "working_archive", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FrArchiveClass, working_archive), NULL, NULL,
      fr_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
}

void fr_archive_stoppable(FrArchive *archive, gboolean stoppable) {
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[STOPPABLE], 0, stoppable);
}
...
void fr_archive_action_completed(FrArchive *archive, FrAction action,
  ...
  if (error_details != NULL)
    archive->error.gerror =
        g_error_new_literal(fr_error_quark(), 0, error_details);
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[DONE], 0, action,
                &archive->error);
}

static gboolean archive_sticky_only_cb(FrProcess *process, FrArchive *archive) {
...
static GFile *get_local_copy_for_file(GFile *remote_file) {
  char *temp_dir;
...
static void fr_archive_set_uri(FrArchive *archive, const char *uri) {
...
static void fr_archive_remove_temp_work_dir(FrArchive *archive) {
  if (archive->priv->temp_dir == NULL) return;
...
static void fr_archive_finalize(GObject *object) {
  ...
  if (archive->command != NULL) g_object_unref(archive->command);
...
static const char *get_mime_type_from_content(GFile *file) {
  GFileInfo *info;
  GError *err = NULL;
...
static const char *get_mime_type_from_magic_numbers(GFile *file) {
#ifdef ENABLE_MAGIC
  ...
  static const struct magic {
    ...
    const char *const mime_type;
      ...
      {0, 2, "\x1f\x9d",
       "application/x-compress"}, /* using Lempel-Ziv-Welch   algorithm */
      {0, 2, "\x1f\xa0",
       "application/x-compress"}, /* using Lempel-Ziv-Huffman algorithm */
  };

  char buffer[32];
  size_t i;

  if (!g_load_file_in_buffer(file, buffer, sizeof(buffer), NULL)) return NULL;
...
static const char *get_mime_type_from_filename(GFile *file) {
  const char *mime_type = NULL;
  char *filename;
...
static gboolean create_command_from_type(FrArchive *archive,
                                         const char *mime_type,
                                         GType command_type,
                                         FrCommandCaps requested_capabilities) {
  if (command_type == 0) return FALSE;

  archive->command = FR_COMMAND(g_object_new(
      command_type, "process", archive->process, "mime-type", mime_type, NULL));
  ...
  if (!fr_command_is_capable_of(archive->command, requested_capabilities)) {
    ...
    archive->is_compressed_file = !fr_command_is_capable_of(
        archive->command, FR_COMMAND_CAN_ARCHIVE_MANY_FILES);

  return (archive->command != NULL);
}

static gboolean create_command_to_load_archive(FrArchive *archive,
                                               const char *mime_type) {
  ...
  command_type =
      get_command_type_from_mime_type(mime_type, requested_capabilities);
  ...
  return create_command_from_type(archive, mime_type, command_type,
                                  requested_capabilities);
}

static gboolean create_command_to_create_archive(FrArchive *archive,
                                                 const char *mime_type) {
  FrCommandCaps requested_capabilities = FR_COMMAND_CAN_DO_NOTHING;
  GType command_type;
...
static void action_started(FrCommand *command, FrAction action,
                           FrArchive *archive) {
#ifdef MATE_ENABLE_DEBUG
  debug(DEBUG_INFO, "%s [START] (FR::Archive)\n", get_action_name(action));
#endif

  g_signal_emit(G_OBJECT(archive), fr_archive_signals[START], 0, action);
}

/* -- copy_to_remote_location -- */

static void fr_archive_copy_done(FrArchive *archive, FrAction action,
                                 GError *error) {
  FrProcErrorType error_type = FR_PROC_ERROR_NONE;
  const char *error_details = NULL;

  if (error != NULL) {
    error_type = (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED)
...
static void copy_to_remote_location_done(GError *error, gpointer user_data) {
...
static void copy_to_remote_location_progress(goffset current_file,
                                             goffset total_files, GFile *source,
                                             GFile *destination,
  ...
  g_signal_emit(G_OBJECT(xfer_data->archive), fr_archive_signals[PROGRESS], 0,
                (double)current_num_bytes / total_num_bytes);
}

static void copy_to_remote_location(FrArchive *archive, FrAction action) {
  XferData *xfer_data;
  ...
  g_copy_file_async(archive->local_copy, archive->file, G_FILE_COPY_OVERWRITE,
                    G_PRIORITY_DEFAULT, archive->priv->cancellable,
                    copy_to_remote_location_progress, xfer_data,
                    copy_to_remote_location_done, xfer_data);
}

/* -- copy_extracted_files_to_destination -- */

static void move_here(FrArchive *archive) {
  char *content_uri;
  char *parent;
  char *parent_parent;
  ...
  content_uri =
      get_dir_content_if_unique(archive->priv->extraction_destination);
  if (content_uri == NULL) return;
  ...
  if (g_strcmp0(parent, archive->priv->extraction_destination) == 0) {
    ...
    new_uri =
        get_alternative_uri_for_uri(archive->priv->extraction_destination);
    ...
    if (!g_file_move(source, destination, 0, NULL, NULL, NULL, &error)) {
      g_warning("could not rename %s to %s: %s",
                archive->priv->extraction_destination, new_uri, error->message);
      g_clear_error(&error);
    }
  ...
  new_content_uri =
      get_alternative_uri(parent_parent, file_name_from_path(content_uri));

  source = g_file_new_for_uri(content_uri);
  destination = g_file_new_for_uri(new_content_uri);
  if (!g_file_move(source, destination, 0, NULL, NULL, NULL, &error)) {
    g_warning("could not rename %s to %s: %s", content_uri, new_content_uri,
              error->message);
    g_clear_error(&error);
  }
...
static void copy_extracted_files_done(GError *error, gpointer user_data) {
  ...
  fr_archive_action_completed(archive, FR_ACTION_COPYING_FILES_TO_REMOTE,
                              FR_PROC_ERROR_NONE, NULL);

  if ((error == NULL) && (archive->priv->extract_here)) move_here(archive);

  fr_archive_copy_done(archive, FR_ACTION_EXTRACTING_FILES, error);
}

static void copy_extracted_files_progress(goffset current_file,
                                          goffset total_files, GFile *source,
                                          GFile *destination,
                                          goffset current_num_bytes,
                                          goffset total_num_bytes,
  ...
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[PROGRESS], 0,
                (double)current_file / (total_files + 1));
}

static void copy_extracted_files_to_destination(FrArchive *archive) {
  g_directory_copy_async(
      archive->priv->temp_extraction_dir, archive->priv->extraction_destination,
      G_FILE_COPY_OVERWRITE, G_PRIORITY_DEFAULT, archive->priv->cancellable,
      copy_extracted_files_progress, archive, copy_extracted_files_done,
      archive);
}

static void add_dropped_items(DroppedItemsData *data);

static void fr_archive_change_name(FrArchive *archive, const char *filename) {
  const char *name;
  GFile *parent;
...
static void action_performed(FrCommand *command, FrAction action,
                             FrProcError *error, FrArchive *archive) {
#ifdef MATE_ENABLE_DEBUG
  ...
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[DONE], 0, action, error);
}

static gboolean archive_progress_cb(FrCommand *command, double fraction,
                                    FrArchive *archive) {
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[PROGRESS], 0, fraction);
  return TRUE;
}

static gboolean archive_message_cb(FrCommand *command, const char *msg,
                                   FrArchive *archive) {
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[MESSAGE], 0, msg);
  return TRUE;
}

static gboolean archive_working_archive_cb(FrCommand *command,
                                           const char *archive_filename,
                                           FrArchive *archive) {
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[WORKING_ARCHIVE], 0,
                archive_filename);
  return TRUE;
}
...
gboolean fr_archive_create(FrArchive *archive, const char *uri) {
...
void fr_archive_set_fake_load_func(FrArchive *archive, FakeLoadFunc func,
                                   gpointer data) {
...
gboolean fr_archive_fake_load(FrArchive *archive) {
  if (archive->priv->fake_load_func != NULL)
    return (*archive->priv->fake_load_func)(archive,
                                            archive->priv->fake_load_data);
  else
    return FALSE;
}
...
static void load_local_archive(FrArchive *archive, const char *password) {
  FrCommand *old_command;
  const char *mime_type;

  if (!g_file_query_exists(archive->file, archive->priv->cancellable)) {
    fr_archive_action_completed(archive, FR_ACTION_LOADING_ARCHIVE,
                                FR_PROC_ERROR_GENERIC, _("File not found."));
    return;
  }
  ...
  g_object_set(archive->command, "file", archive->local_copy, "password",
               password, NULL);
  fr_command_list(archive->command);
}

static void copy_remote_file_done(GError *error, gpointer user_data) {
  XferData *xfer_data = user_data;
...
static void copy_remote_file_progress(goffset current_file, goffset total_files,
...
static gboolean copy_remote_file_done_cb(gpointer user_data) {
  XferData *xfer_data = user_data;
...
static void copy_remote_file(FrArchive *archive, const char *password) {
  XferData *xfer_data;
  ...
  if (!g_file_query_exists(archive->file, archive->priv->cancellable)) {
    ...
    error =
        g_error_new(G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("Archive not found"));
  ...
  if (password != NULL) xfer_data->password = g_strdup(password);
  ...
  g_copy_file_async(archive->file, archive->local_copy, G_FILE_COPY_OVERWRITE,
                    G_PRIORITY_DEFAULT, archive->priv->cancellable,
                    copy_remote_file_progress, xfer_data, copy_remote_file_done,
                    xfer_data);
}
...
gboolean fr_archive_load(FrArchive *archive, const char *uri,
  ...
  g_signal_emit(G_OBJECT(archive), fr_archive_signals[START], 0,
...
static char *create_tmp_base_dir(const char *base_dir, const char *dest_path) {
  char *dest_dir;
  char *temp_dir;
  char *tmp;
  char *parent_dir;
  ...
  if (!symlink(base_dir, dir))
    g_warning("Could not create the symbolic link '%s', pointing to '%s'", dir,
              base_dir);
...
static FileData *find_file_in_archive(FrArchive *archive, char *path) {
  int i;
...
static GList *newer_files_only(FrArchive *archive, GList *file_list,
                               const char *base_dir) {
...
static gboolean save_list_to_temp_file(GList *file_list, char **list_dir,
                                       char **list_filename, GError **error) {
  ...
  *list_filename = g_build_filename(*list_dir, "file-list", NULL);
  ...
  if (ostream != NULL) {
    ...
    for (scan = file_list; scan != NULL; scan = scan->next) {
      ...
      if ((g_output_stream_write(G_OUTPUT_STREAM(ostream), filename,
                                 strlen(filename), NULL, error) < 0) ||
...
static GList *split_in_chunks(GList *file_list) {
  GList *chunks = NULL;
  GList *new_file_list;
  GList *scan;
  ...
  for (scan = new_file_list; scan != NULL; /* void */) {
    ...
    while ((scan != NULL) && (l < MAX_CHUNK_LEN)) {
      if (l == 0) l = strlen(scan->data);
      prev = scan;
      scan = scan->next;
      if (scan != NULL) l += strlen(scan->data);
    }
    if (prev != NULL) {
      if (prev->next != NULL) prev->next->prev = NULL;
      prev->next = NULL;
    }
...
void fr_archive_add(FrArchive *archive, GList *file_list, const char *base_dir,
                    const char *dest_dir, gboolean update, gboolean recursive,
                    const char *password, gboolean encrypt_header,
                    FrCompression compression, guint volume_size) {
  ...
  g_object_set(archive->command, "password", password, "encrypt_header",
               encrypt_header, "compression", compression, "volume_size",
               volume_size, NULL);
  ...
  if ((dest_dir != NULL) && (*dest_dir != '\0') &&
    ...
    if (dest_dir[0] == G_DIR_SEPARATOR) rel_dest_dir = dest_dir + 1;

    new_file_list = NULL;
    for (scan = file_list; scan != NULL; scan = scan->next) {
      char *filename = scan->data;
      new_file_list = g_list_prepend(
          new_file_list, g_build_filename(rel_dest_dir, filename, NULL));
    }
  } else {
  ...
  if (new_file_list == NULL) {
    ...
    if (base_dir_created) remove_local_directory(tmp_base_dir);
    g_free(tmp_base_dir);

    archive->process->error.type = FR_PROC_ERROR_NONE;
    g_signal_emit_by_name(G_OBJECT(archive->process), "done",
                          &archive->process->error);
    return;
  }

  archive->command->creating_archive =
      !g_file_query_exists(archive->local_copy, archive->priv->cancellable);
...

  {
    ...
    tmp_archive_filename = g_build_filename(
        tmp_archive_dir, file_name_from_path(archive_filename), NULL);
  ...
  if (archive->command->propListFromFile &&
    ...
    if (!save_list_to_temp_file(new_file_list, &list_dir, &list_filename,
                                &error)) {
      ...
      fr_command_add(archive->command, list_filename, new_file_list,
                     tmp_base_dir, update, recursive);
    ...
    for (scan = chunks; scan != NULL; scan = scan->next) {
      ...
      fr_command_add(archive->command, NULL, chunk, tmp_base_dir, update,
                     recursive);
      g_list_free(chunk);
    }
...
static void fr_archive_add_local_files(
    FrArchive *archive, GList *file_list, const char *base_dir,
    const char *dest_dir, gboolean update, const char *password,
    gboolean encrypt_header, FrCompression compression, guint volume_size) {
  fr_process_clear(archive->process);
  fr_archive_add(archive, file_list, base_dir, dest_dir, update, FALSE,
                 password, encrypt_header, compression, volume_size);
  fr_process_start(archive->process);
}

static void copy_remote_files_done(GError *error, gpointer user_data) {
  XferData *xfer_data = user_data;

  fr_archive_copy_done(xfer_data->archive, FR_ACTION_COPYING_FILES_FROM_REMOTE,
                       error);

  if (error == NULL)
    fr_archive_add_local_files(xfer_data->archive, xfer_data->file_list,
                               xfer_data->tmp_dir, xfer_data->dest_dir, FALSE,
                               xfer_data->password, xfer_data->encrypt_header,
                               xfer_data->compression, xfer_data->volume_size);
  xfer_data_free(xfer_data);
}

static void copy_remote_files_progress(goffset current_file,
                                       goffset total_files, GFile *source,
                                       GFile *destination,
                                       goffset current_num_bytes,
                                       goffset total_num_bytes,
...
static void copy_remote_files(FrArchive *archive, GList *file_list,
                              const char *base_uri, const char *dest_dir,
                              gboolean update, const char *password,
                              gboolean encrypt_header,
                              FrCompression compression, guint volume_size,
                              const char *tmp_dir) {
  ...
  created_folders = g_hash_table_new_full(g_str_hash, g_str_equal,
    ...
    if (g_hash_table_lookup(created_folders, local_folder_uri) == NULL) {
      ...
      if (!ensure_dir_exists(local_folder_uri, 0755, &error)) {
        ...
        fr_archive_action_completed(archive,
                                    ...
                                    FR_PROC_ERROR_GENERIC, error->message);
        g_clear_error(&error);
        return;
      }

      g_hash_table_insert(created_folders, local_folder_uri,
                          GINT_TO_POINTER(1));
    } else
      g_free(local_folder_uri);
  ...
  g_copy_files_async(sources, destinations, G_FILE_COPY_OVERWRITE,
                     G_PRIORITY_DEFAULT, archive->priv->cancellable,
                     copy_remote_files_progress, xfer_data,
                     copy_remote_files_done, xfer_data);

  gio_file_list_free(sources);
  gio_file_list_free(destinations);
}

static char *fr_archive_get_temp_work_dir(FrArchive *archive) {
  fr_archive_remove_temp_work_dir(archive);
  archive->priv->temp_dir = get_temp_work_dir(NULL);
  return archive->priv->temp_dir;
}

void fr_archive_add_files(FrArchive *archive, GList *file_list,
                          const char *base_dir, const char *dest_dir,
                          gboolean update, const char *password,
                          gboolean encrypt_header, FrCompression compression,
                          guint volume_size) {
  if (uri_is_local(base_dir)) {
    char *local_dir = g_filename_from_uri(base_dir, NULL, NULL);
    fr_archive_add_local_files(archive, file_list, local_dir, dest_dir, update,
                               password, encrypt_header, compression,
                               volume_size);
    g_free(local_dir);
  } else
    copy_remote_files(archive, file_list, base_dir, dest_dir, update, password,
                      encrypt_header, compression, volume_size,
...
static void add_with_wildcard_data_free(AddWithWildcardData *aww_data) {
...
static void add_with_wildcard__step2(GList *file_list, GList *dirs_list,
                                     GError *error, gpointer data) {
  AddWithWildcardData *aww_data = data;
  FrArchive *archive = aww_data->archive;

  if (error != NULL) {
    fr_archive_action_completed(
        archive, FR_ACTION_GETTING_FILE_LIST,
  ...
  fr_archive_action_completed(archive, FR_ACTION_GETTING_FILE_LIST,
                              FR_PROC_ERROR_NONE, NULL);

  if (file_list != NULL)
    fr_archive_add_files(aww_data->archive, file_list, aww_data->source_dir,
                         aww_data->dest_dir, aww_data->update,
                         aww_data->password, aww_data->encrypt_header,
                         aww_data->compression, aww_data->volume_size);
...
void fr_archive_add_with_wildcard(
    FrArchive *archive, const char *include_files, const char *exclude_files,
    const char *exclude_folders, const char *source_dir, const char *dest_dir,
    gboolean update, gboolean follow_links, const char *password,
    gboolean encrypt_header, FrCompression compression, guint volume_size) {
  ...
  g_directory_list_async(
      source_dir, source_dir, TRUE, follow_links, NO_BACKUP_FILES, NO_DOT_FILES,
      include_files, exclude_files, exclude_folders, IGNORE_CASE,
      archive->priv->cancellable, add_with_wildcard__step2, aww_data);
}
...
static void add_directory_data_free(AddDirectoryData *ad_data) {
...
static void add_directory__step2(GList *file_list, GList *dir_list,
                                 GError *error, gpointer data) {
  ...
  if (file_list != NULL) {
    fr_archive_add_files(ad_data->archive, file_list, ad_data->base_dir,
                         ad_data->dest_dir, ad_data->update, ad_data->password,
                         ad_data->encrypt_header, ad_data->compression,
                         ad_data->volume_size);
    path_list_free(file_list);
  }
...
void fr_archive_add_directory(FrArchive *archive, const char *directory,
                              const char *base_dir, const char *dest_dir,
                              gboolean update, const char *password,
                              gboolean encrypt_header,
                              FrCompression compression, guint volume_size)

{
  ...
  g_directory_list_all_async(directory, base_dir, TRUE,
                             archive->priv->cancellable, add_directory__step2,
                             ad_data);
}

void fr_archive_add_items(FrArchive *archive, GList *item_list,
                          const char *base_dir, const char *dest_dir,
                          gboolean update, const char *password,
                          gboolean encrypt_header, FrCompression compression,
                          guint volume_size)

{
  ...
  g_list_items_async(item_list, base_dir, archive->priv->cancellable,
                     add_directory__step2, ad_data);
}

/* -- fr_archive_add_dropped_items -- */

static gboolean all_files_in_same_dir(GList *list) {
  ...
  if (first_basedir == NULL) return TRUE;
...
static void add_dropped_items(DroppedItemsData *data) {
  ...
  if (list == NULL) {
    dropped_items_data_free(archive->priv->dropped_items_data);
    archive->priv->dropped_items_data = NULL;
    fr_archive_action_completed(archive, FR_ACTION_ADDING_FILES,
                                FR_PROC_ERROR_NONE, NULL);
    return;
  }
  ...
  if (all_files_in_same_dir(list)) {
    ...
    fr_archive_add_items(data->archive, list, first_base_dir, data->dest_dir,
                         data->update, data->password, data->encrypt_header,
                         data->compression, data->volume_size);
  ...
  for (scan = list; scan; scan = scan->next) {
    ...
    fr_archive_add_directory(archive, file_name_from_path(path), base_dir,
                             data->dest_dir, data->update, data->password,
                             data->encrypt_header, data->compression,
                             data->volume_size);
  ...
  if (all_files_in_same_dir(list)) {
    ...
    fr_archive_add_files(archive, only_names_list, first_basedir,
                         data->dest_dir, data->update, data->password,
                         data->encrypt_header, data->compression,
                         data->volume_size);
  ...
  archive->command->creating_archive =
      !g_file_query_exists(archive->local_copy, archive->priv->cancellable);
  g_object_set(archive->command, "file", archive->local_copy, "password",
               data->password, "encrypt_header", data->encrypt_header,
               "compression", data->compression, "volume_size",
  ...
  for (scan = list; scan; scan = scan->next) {
    ...
    fr_command_add(archive->command, NULL, singleton, basedir, data->update,
                   FALSE);
...
void fr_archive_add_dropped_items(FrArchive *archive, GList *item_list,
                                  const char *base_dir, const char *dest_dir,
                                  gboolean update, const char *password,
                                  gboolean encrypt_header,
                                  FrCompression compression,
  ...
  if (archive->read_only) {
    fr_archive_action_completed(
        ...
        !archive->have_permissions ? _("You don't have the right permissions.")
                                   : _("This archive type cannot be modified"));
    return;
  }
  ...
  archive->priv->dropped_items_data = dropped_items_data_new(
      archive, item_list, base_dir, dest_dir, update, password, encrypt_header,
      compression, volume_size);
  add_dropped_items(archive->priv->dropped_items_data);
}

/* -- remove -- */

static gboolean file_is_in_subfolder_of(const char *filename,
                                        GList *folder_list) {
  GList *scan;
  ...
  for (scan = folder_list; scan; scan = scan->next) {
    ...
    if (path_in_path(folder_in_list, filename)) return TRUE;
  }

  return FALSE;
}

static gboolean archive_type_has_issues_deleting_non_empty_folders(
    FrArchive *archive) {
...
static void delete_from_archive(FrArchive *archive, GList *file_list) {
  ...
  if (!tmp_file_list_created) tmp_file_list = g_list_copy(file_list);

  if (file_list_created) g_list_free(file_list);
  ...
  if (archive->command->propListFromFile &&
    ...
    if (save_list_to_temp_file(tmp_file_list, &list_dir, &list_filename,
                               NULL)) {
      fr_command_delete(archive->command, list_filename, tmp_file_list);
...
void fr_archive_remove(FrArchive *archive, GList *file_list,
                       FrCompression compression) {
  char *tmp_archive_dir = NULL;
  char *archive_filename = NULL;
  char *tmp_archive_filename = NULL;
...
static void move_files_to_dir(FrArchive *archive, GList *file_list,
  ...
  for (scan = list; scan; /* void */) {
    ...
    if (g_file_test(destname, G_FILE_TEST_IS_DIR)) {
      ...
      if (filename[0] == '/')
        ...
        fr_process_add_arg_concat(archive->process, source_dir, "/", filename,
                                  NULL);
...
static void move_files_in_chunks(FrArchive *archive, GList *file_list,
                                 const char *temp_dir, const char *dest_dir,
                                 gboolean overwrite) {
  GList *scan;
  int temp_dir_l;
  ...
  for (scan = file_list; scan != NULL;) {
    ...
    l = 0;
    while ((scan != NULL) && (l < MAX_CHUNK_LEN)) {
      if (l == 0) l = temp_dir_l + 1 + strlen(scan->data);
      prev = scan;
      scan = scan->next;
      if (scan != NULL) l += temp_dir_l + 1 + strlen(scan->data);
    }
...
static void extract_from_archive(FrArchive *archive, GList *file_list,
                                 const char *dest_dir, gboolean overwrite,
                                 gboolean skip_older, gboolean junk_paths,
                                 const char *password) {
  ...
  if (file_list == NULL) {
    fr_command_extract(command, NULL, file_list, dest_dir, overwrite,
                       skip_older, junk_paths);
    return;
  }
  ...
  if (command->propListFromFile &&
    ...
    if (save_list_to_temp_file(file_list, &list_dir, &list_filename, NULL)) {
      fr_command_extract(command, list_filename, file_list, dest_dir, overwrite,
                         skip_older, junk_paths);
    ...
    for (scan = file_list; scan != NULL;) {
      ...
      prev->next = NULL;
      fr_command_extract(command, NULL, chunk_list, dest_dir, overwrite,
                         skip_older, junk_paths);
      prev->next = scan;
    }
  }
...
static char *compute_base_path(const char *base_dir, const char *path,
                               gboolean junk_paths, gboolean can_junk_paths) {
  ...
  if (path_len < base_dir_len) return NULL;

  base_path = path + base_dir_len;
  if (path[0] != '/') base_path -= 1;
...
static GList *compute_list_base_path(const char *base_dir, GList *filtered,
                                     gboolean junk_paths,
  ...
  for (scan = filtered; scan; scan = scan->next) {
    ...
    if (new_path != NULL) list = g_list_prepend(list, new_path);
  }
...
static gboolean archive_type_has_issues_extracting_non_empty_folders(
    FrArchive *archive) {
  /*if ((archive->command->files == NULL) || (archive->command->files->len ==
     0)) return FALSE;  FIXME: test with extract_here */
  return !archive->command->propCanExtractNonEmptyFolders;
}

static gboolean file_list_contains_files_in_this_dir(GList *file_list,
                                                     const char *dirname) {
  GList *scan;
  ...
  for (scan = file_list; scan; scan = scan->next) {
    ...
    if (path_in_path(dirname, filename)) return TRUE;
  }

  return FALSE;
}

static GList *remove_files_contained_in_this_dir(GList *file_list,
                                                 GList *dir_pointer) {
  ...
  for (scan = dir_pointer->next; scan; /* empty */) {
    ...
    if (strncmp(dirname, filename, dirname_l) != 0) break;
...
void fr_archive_extract_to_local(FrArchive *archive, GList *file_list,
                                 const char *destination, const char *base_dir,
                                 gboolean skip_older, gboolean overwrite,
  ...
  use_base_dir = !((base_dir == NULL) || (strcmp(base_dir, "") == 0) ||
                   (strcmp(base_dir, "/") == 0));
  ...
  if (extract_all &&
      (!all_options_supported || !archive->command->propCanExtractAll)) {
    guint i;
  ...
  if (all_options_supported) {
    ...
    if (!extract_all &&
        archive_type_has_issues_extracting_non_empty_folders(archive)) {
    ...
    if (!(created_filtered_list && (filtered == NULL)))
      extract_from_archive(archive, filtered, destination, overwrite,
                           skip_older, junk_paths, password);

    if (created_filtered_list && (filtered != NULL)) g_list_free(filtered);

    if (file_list_created) path_list_free(file_list);
  ...
  for (scan = file_list; scan; scan = scan->next) {
    ...
    if ((destination[strlen(destination) - 1] == '/') || (filename[0] == '/'))
    ...
    if (!archive->command->propExtractCanSkipOlder && skip_older &&
  ...
  if (filtered == NULL) {
    ...
    if (extract_all) path_list_free(file_list);
    return;
  }
  ...
  if (move_to_dest_dir) {
    ...
    extract_from_archive(archive, filtered, temp_dir, overwrite, skip_older,
                         junk_paths, password);

    if (use_base_dir) {
      GList *tmp_list =
          compute_list_base_path(base_dir, filtered, junk_paths,
                                 archive->command->propExtractCanJunkPaths);
      g_list_free(filtered);
      filtered = tmp_list;
    }

    move_files_in_chunks(archive, filtered, temp_dir, destination, overwrite);
    ...
    extract_from_archive(archive, filtered, destination, overwrite, skip_older,
                         junk_paths, password);

  if (filtered != NULL) g_list_free(filtered);
  if (file_list_created) path_list_free(file_list);
}

void fr_archive_extract(FrArchive *archive, GList *file_list,
                        const char *destination, const char *base_dir,
                        gboolean skip_older, gboolean overwrite,
                        gboolean junk_paths, const char *password) {
  g_free(archive->priv->extraction_destination);
  ...
  if (archive->priv->remote_extraction) {
    ...
    fr_archive_extract_to_local(archive, file_list,
                                archive->priv->temp_extraction_dir, base_dir,
                                skip_older, overwrite, junk_paths, password);
  } else {
    char *local_destination;

    local_destination = g_filename_from_uri(destination, NULL, NULL);
    fr_archive_extract_to_local(archive, file_list, local_destination, base_dir,
                                skip_older, overwrite, junk_paths, password);
    g_free(local_destination);
  }
}

static char *get_desired_destination_for_archive(GFile *file) {
  GFile *directory;
  char *directory_uri;
  char *name;
  const char *ext;
  ...
  if (ext == NULL) /* if no extension is present add a suffix to the name... */
...
static char *get_extract_here_destination(GFile *file, GError **error) {
...
gboolean fr_archive_extract_here(FrArchive *archive, gboolean skip_older,
                                 gboolean overwrite, gboolean junk_path,
                                 const char *password) {
  char *destination;
  GError *error = NULL;
  ...
  if (error != NULL) {
    fr_archive_action_completed(archive, FR_ACTION_EXTRACTING_FILES,
                                FR_PROC_ERROR_GENERIC, error->message);
    g_clear_error(&error);
    return FALSE;
  }
  ...
  fr_archive_extract(archive, NULL, destination, NULL, skip_older, overwrite,
                     junk_path, password);
...
gboolean uri_is_archive(const char *uri) {
  ...
  if (mime_type == NULL) mime_type = get_mime_type_from_content(file);
  if (mime_type == NULL) mime_type = get_mime_type_from_filename(file);
