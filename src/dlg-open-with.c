...
static void app_chooser_response_cb(GtkDialog *dialog, int response_id,
                                    gpointer user_data) {
  ...
  switch (response_id) {
    case GTK_RESPONSE_OK:
      ...
      if (app_info != NULL) {
        fr_window_open_files_with_application(o_data->window, o_data->file_list,
                                              app_info);
...
void dlg_open_with(FrWindow *window, GList *file_list) {
  ...
  app_chooser = gtk_app_chooser_dialog_new(GTK_WINDOW(window), GTK_DIALOG_MODAL,
                                           first_file);
  g_signal_connect(app_chooser, "response", G_CALLBACK(app_chooser_response_cb),
                   o_data);
  gtk_widget_show(app_chooser);
}

void open_with_cb(GtkWidget *widget, void *callback_data) {
