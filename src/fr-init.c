...
FrMimeTypeDescription mime_type_desc[] = {
    ...
    {"application/vnd.oasis.opendocument.presentation", ".odp",
     N_("OpenDocument Presentation"), 0},
    {"application/vnd.oasis.opendocument.spreadsheet", ".ods",
     N_("OpenDocument Spreadsheet"), 0},
    {"application/vnd.oasis.opendocument.text", ".odt", N_("OpenDocument Text"),
     0},
    {"application/vnd.oasis.opendocument.presentation-template", ".otp",
     N_("OpenDocument Presentation Template"), 0},
    {"application/vnd.oasis.opendocument.spreadsheet-template", ".ots",
     N_("OpenDocument Spreadsheet Template"), 0},
    {"application/vnd.oasis.opendocument.text-template", ".ott",
     N_("OpenDocument Text Template"), 0},
    {"application/epub+zip", ".epub", N_("Electronic Publication"), 0},
    {"application/x-7z-compressed", ".7z", N_("7-Zip"), 0},
    {"application/x-7z-compressed-tar", ".tar.7z", N_("Tar compressed with 7z"),
     0},
    ...
    {"application/x-brotli-compressed-tar", ".tar.br",
     N_("Tar compressed with brotli"), 0},
    {"application/x-bzip", ".bz2", N_("Bzip2 compressed file"), 0},
    {"application/x-bzip-compressed-tar", ".tar.bz2",
     N_("Tar compressed with bzip2"), 0},
    {"application/x-bzip1", ".bz", N_("Bzip compressed file"), 0},
    {"application/x-bzip1-compressed-tar", ".tar.bz",
     N_("Tar compressed with bzip"), 0},
    ...
    {"application/x-compressed-tar", ".tar.gz", N_("Tar compressed with gzip"),
     0},
...
static void migrate_options_directory(void) {
  ...
  if (g_file_query_exists(old_directory, NULL) &&
      !g_file_query_exists(new_directory, NULL)) {
    GFile *parent;
...
static FrRegisteredCommand *fr_registered_command_new(GType command_type) {
  ...
  for (i = 0; mime_types[i] != NULL; i++) {
    ...
    cap->current_capabilities =
        fr_command_get_capabilities(command, mime_type, TRUE);
    cap->potential_capabilities =
        fr_command_get_capabilities(command, mime_type, FALSE);
...
static FrCommandCaps fr_registered_command_get_potential_capabilities(
    FrRegisteredCommand *reg_com, const char *mime_type) {
  guint i;

  if (mime_type == NULL) return FR_COMMAND_CAN_DO_NOTHING;
...
static void register_command(GType command_type) {
...
GType get_preferred_command_for_mime_type(
    const char *mime_type, FrCommandCaps requested_capabilities) {
  guint i;
  ...
  for (i = 0; i < Registered_Commands->len; i++) {
    ...
    command = g_ptr_array_index(Registered_Commands, i);
    capabilities =
        fr_registered_command_get_potential_capabilities(command, mime_type);
...
void update_registered_commands_capabilities(void) {
  ...
  for (i = 0; i < Registered_Commands->len; i++) {
    ...
    for (j = 0; j < reg_com->caps->len; j++) {
      ...
      cap->current_capabilities =
          fr_command_get_capabilities(command, cap->mime_type, TRUE);
      cap->potential_capabilities =
          fr_command_get_capabilities(command, cap->mime_type, FALSE);
    }
...
const char *get_mime_type_from_extension(const char *ext) {
  ...
  for (i = G_N_ELEMENTS(file_ext_type) - 1; i >= 0; i--) {
    if (file_ext_type[i].ext == NULL) continue;
...
const char *get_archive_filename_extension(const char *filename) {
  ...
  for (i = G_N_ELEMENTS(file_ext_type) - 1; i >= 0; i--) {
    if (file_ext_type[i].ext == NULL) continue;
    if (strcasecmp(ext, file_ext_type[i].ext) == 0) return ext;
  }

  return NULL;
}

static gboolean is_mime_type_alias(const char *mime_type) {
  size_t i;
...
int get_mime_type_index(const char *mime_type) {
  ...
  for (i = 0; mime_type_desc[i].mime_type != NULL; i++)
    if (strcmp(mime_type_desc[i].mime_type, mime_type) == 0) return i;
  return -1;
}

static void add_if_non_present(int *a, int *n, int o) {
  int i;
...
static int cmp_mime_type_by_extension(const void *p1, const void *p2) {
  int i1 = *(int *)p1;
  int i2 = *(int *)p2;
...
static int cmp_mime_type_by_description(const void *p1, const void *p2) {
  int i1 = *(int *)p1;
  int i2 = *(int *)p2;
...
static void sort_mime_types(int *a, int (*compar)(const void *, const void *)) {
  int n = 0;
...
static void compute_supported_archive_types(void) {
...
static void command_done(CommandData *cdata) {
  ...
  if ((cdata->temp_dir != NULL) && path_is_dir(cdata->temp_dir)) {
    ...
    argv[3] = NULL;
    g_spawn_sync(g_get_tmp_dir(), argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL,
                 NULL, NULL, NULL, NULL);
  }

  g_free(cdata->command);
  if (cdata->app != NULL) g_object_unref(cdata->app);
  path_list_free(cdata->file_list);
  g_free(cdata->temp_dir);
  if (cdata->process != NULL) g_object_unref(cdata->process);
