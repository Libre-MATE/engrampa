...
enum { START, DONE, STICKY_ONLY, LAST_SIGNAL };
...
static FrCommandInfo *fr_command_info_new(void) {
  FrCommandInfo *info;
...
static void fr_command_info_free(gpointer data) {
...
static void fr_channel_data_init(FrChannelData *channel) {
  channel->source = NULL;
  channel->raw = NULL;
...
static void fr_channel_data_close_source(FrChannelData *channel) {
...
static GIOStatus fr_channel_data_read(FrChannelData *channel) {
  char *line;
  gsize length;
  ...
  while ((channel->status = g_io_channel_read_line(
              channel->source, &line, &length, &terminator_pos,
...
static GIOStatus fr_channel_data_flush(FrChannelData *channel) {
  GIOStatus status;

  while (((status = fr_channel_data_read(channel)) != G_IO_STATUS_ERROR) &&
...
static void fr_channel_data_reset(FrChannelData *channel) {
  fr_channel_data_close_source(channel);
...
static void fr_channel_data_free(FrChannelData *channel) {
  fr_channel_data_reset(channel);
}

static void fr_channel_data_set_fd(FrChannelData *channel, int fd,
                                   const char *charset) {
  fr_channel_data_reset(channel);
...
GType fr_process_get_type(void) {
  ...
  if (!type) {
    GTypeInfo type_info = {sizeof(FrProcessClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(G_TYPE_OBJECT, "FRProcess", &type_info, 0);
  }
...
static void fr_process_class_init(FrProcessClass *class) {
  ...
  fr_process_signals[START] =
      g_signal_new("start", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrProcessClass, start), NULL, NULL,
                   fr_marshal_VOID__VOID, G_TYPE_NONE, 0);
  fr_process_signals[DONE] =
      g_signal_new("done", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrProcessClass, done), NULL, NULL,
                   fr_marshal_VOID__BOXED, G_TYPE_NONE, 1, FR_TYPE_PROC_ERROR);
  fr_process_signals[STICKY_ONLY] =
      g_signal_new("sticky_only", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FrProcessClass, sticky_only), NULL, NULL,
                   fr_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
void fr_process_begin_command(FrProcess *process, const char *arg) {
  FrCommandInfo *info;
...
void fr_process_begin_command_at(FrProcess *process, const char *arg,
                                 int index) {
...
void fr_process_set_working_dir(FrProcess *process, const char *dir) {
  FrCommandInfo *info;
  ...
  if (info->dir != NULL) g_free(info->dir);
  info->dir = g_strdup(dir);
}

void fr_process_set_sticky(FrProcess *process, gboolean sticky) {
  FrCommandInfo *info;
...
void fr_process_set_ignore_error(FrProcess *process, gboolean ignore_error) {
  FrCommandInfo *info;
...
void fr_process_add_arg(FrProcess *process, const char *arg) {
  FrCommandInfo *info;
...
void fr_process_add_arg_concat(FrProcess *process, const char *arg1, ...) {
  GString *arg;
  va_list args;
  char *s;
  ...
  va_start(args, arg1);
  while ((s = va_arg(args, char *)) != NULL) g_string_append(arg, s);
  va_end(args);
...
void fr_process_add_arg_printf(FrProcess *fr_proc, const char *format, ...) {
  va_list args;
  char *arg;
...
void fr_process_set_arg_at(FrProcess *process, int n_comm, int n_arg,
...
void fr_process_set_begin_func(FrProcess *process, ProcFunc func,
                               gpointer func_data) {
  FrCommandInfo *info;
...
void fr_process_set_end_func(FrProcess *process, ProcFunc func,
                             gpointer func_data) {
  FrCommandInfo *info;
...
void fr_process_set_continue_func(FrProcess *process, ContinueFunc func,
                                  gpointer func_data) {
  FrCommandInfo *info;

  g_return_if_fail(process != NULL);

  if (process->priv->current_comm < 0) return;
...
void fr_process_end_command(FrProcess *process) {
  FrCommandInfo *info;
...
void fr_process_clear(FrProcess *process) {
  gint i;
...
void fr_process_set_out_line_func(FrProcess *process, LineFunc func,
                                  gpointer data) {
  g_return_if_fail(process != NULL);
...
void fr_process_set_err_line_func(FrProcess *process, LineFunc func,
                                  gpointer data) {
  g_return_if_fail(process != NULL);
...
static const char *fr_process_get_charset(FrProcess *process) {
...
static void start_current_command(FrProcess *process) {
  FrCommandInfo *info;
  GList *scan;
  char **argv;
  int out_fd, err_fd;
  ...
  debug(DEBUG_INFO, "%d/%d) ", process->priv->current_command,
        process->priv->n_comm);
  ...
  for (scan = info->args; scan; scan = scan->next) {
    ...
    if (g_str_has_prefix(commandline->str, "mv")) {
      if ((i == 3) && (!g_file_test(argv[2], G_FILE_TEST_EXISTS)) &&
          (!fixname)) {
...
#ifdef MATE_ENABLE_DEBUG
  {
    ...
    g_print("\t");
    for (j = 0; j < i; j++) g_print("%s ", argv[j]);
    g_print("\n");
  }
#endif
  ...
  if (info->begin_func != NULL) (*info->begin_func)(info->begin_data);

  if (!g_spawn_async_with_pipes(
          info->dir, argv, NULL,
          (G_SPAWN_LEAVE_DESCRIPTORS_OPEN | G_SPAWN_SEARCH_PATH |
           G_SPAWN_DO_NOT_REAP_CHILD),
          child_setup, process, &process->priv->command_pid, NULL, &out_fd,
          &err_fd, &process->error.gerror)) {
    process->error.type = FR_PROC_ERROR_SPAWN;
    g_signal_emit(G_OBJECT(process), fr_process_signals[DONE], 0,
                  &process->error);
  ...
  fr_channel_data_set_fd(&process->out, out_fd,
                         fr_process_get_charset(process));
  fr_channel_data_set_fd(&process->err, err_fd,
                         fr_process_get_charset(process));

  process->priv->check_timeout =
      g_timeout_add(REFRESH_RATE, check_child, process);
}

static gboolean command_is_sticky(FrProcess *process, int i) {
  FrCommandInfo *info;
...
static void allow_sticky_processes_only(FrProcess *process,
  ...
  if (emit_signal)
    g_signal_emit(G_OBJECT(process), fr_process_signals[STICKY_ONLY], 0);
}

static void fr_process_set_error(FrProcess *process, FrProcErrorType type,
                                 int status, GError *gerror) {
  process->error.type = type;
  process->error.status = status;
  if (gerror != process->error.gerror) {
    g_clear_error(&process->error.gerror);
    if (gerror != NULL) process->error.gerror = g_error_copy(gerror);
  }
}
...
static gint check_child(gpointer data) {
  ...
  if (fr_channel_data_read(&process->out) == G_IO_STATUS_ERROR) {
    fr_process_set_error(process, FR_PROC_ERROR_IO_CHANNEL, 0,
                         process->out.error);
    channel_error = TRUE;
  ...
  if (info->end_func != NULL) (*info->end_func)(info->end_data);

  /**/

  if (channel_error && (process->error.type == FR_PROC_ERROR_IO_CHANNEL) &&
      g_error_matches(process->error.gerror, G_CONVERT_ERROR,
    ...
    fr_process_set_error(process, FR_PROC_ERROR_BAD_CHARSET, 0,
  ...
  if (process->priv->sticky_only) {
    ...
    fr_process_set_error(process, process->priv->first_error.type,
...
void fr_process_use_standard_locale(FrProcess *process,
...
void fr_process_start(FrProcess *process) {
  ...
  if (process->priv->running) return;
  ...
  if (!process->restart) {
    process->priv->current_charset = -1;
    g_signal_emit(G_OBJECT(process), fr_process_signals[START], 0);
  }
...
static int fr_close_suspend_process(FrProcess *process) {
  int ret = -1;
  ...
  if (process->priv->suspend) {
    ...
    if (ret == 0) process->priv->suspend = FALSE;
  }
...
static int fr_switch_process_state(FrProcess *process) {
  ...
  if (process->priv->stopping) return ret;
  ...
  if (process->priv->suspend) {
    ...
    if (ret == 0) process->priv->suspend = TRUE;
  }
...
static void fr_process_stop_priv(FrProcess *process, gboolean emit_signal) {
  ...
  if (process->priv->stopping) return;
...
void fr_process_stop(FrProcess *process) {
