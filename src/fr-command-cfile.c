...
static char *get_uncompressed_name_from_archive(FrCommand *comm,
                                                const char *archive) {
  GFile *file;
  GInputStream *stream;
  char *filename = NULL;

  if (!is_mime_type(comm->mime_type, "application/x-gzip")) return NULL;
  ...
  if (stream != NULL) {
    ...
    if (g_input_stream_read(stream, buffer, 10, NULL, NULL) >= 0) {
      ...
      if (((unsigned char)(buffer[3]) & 0x08) != 0x08) filename_present = FALSE;

      /* Check whether the FLG.FEXTRA is set */
      if (((unsigned char)(buffer[3]) & 0x04) == 0x04) filename_present = FALSE;
    }
...
static void list__process_line(char *line, gpointer data) {
  ...
  if (fdata->size == 0) fdata->size = get_file_size(comm->filename);

  filename = get_uncompressed_name_from_archive(comm, comm->filename);
  if (filename == NULL) filename = remove_extension_from_path(comm->filename);

  fdata->full_path = g_strconcat("/", file_name_from_path(filename), NULL);
  g_free(filename);
...
static void fr_command_cfile_list(FrCommand *comm) {
  ...
  if (is_mime_type(comm->mime_type, "application/x-gzip")) {
    ...
    fr_process_set_out_line_func(FR_COMMAND(comm)->process, list__process_line,
                                 comm);
    ...
    g_signal_emit_by_name(G_OBJECT(comm), "done", comm->action,
                          &comm_cfile->error);
  }
}

static void fr_command_cfile_add(FrCommand *comm, const char *from_file,
  ...
  if ((file_list == NULL) || (file_list->data == NULL)) return;
...
static void fr_command_cfile_delete(FrCommand *comm, const char *from_file,
                                    GList *file_list) {
  /* never called */
}

static void fr_command_cfile_extract(FrCommand *comm, const char *from_file,
  ...
  temp_dir = get_temp_work_dir(NULL);
  temp_file =
      g_strconcat(temp_dir, "/", file_name_from_path(comm->filename), NULL);
  ...
  if (compr_file == NULL)
    compr_file =
        remove_extension_from_path(file_name_from_path(comm->filename));
  dest_file = g_strconcat(dest_dir, "/", compr_file, NULL);
...
static void fr_command_cfile_test(FrCommand *comm) {
...
static const char **fr_command_cfile_get_mime_types(FrCommand *comm) {
  return cfile_mime_type;
}

static FrCommandCaps fr_command_cfile_get_capabilities(FrCommand *comm,
  ...
  if (is_mime_type(mime_type, "application/x-gzip")) {
    ...
    if (is_program_available("uncompress", check_command) ||
        is_program_available("gzip", check_command))
...
static const char *fr_command_cfile_get_packages(FrCommand *comm,
                                                 const char *mime_type) {
  if (is_mime_type(mime_type, "application/x-gzip"))
...
static void fr_command_cfile_init(FrCommand *comm) {
...
GType fr_command_cfile_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandCFileClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(FR_TYPE_COMMAND, "FRCommandCFile", &type_info,
                                  0);
  }
