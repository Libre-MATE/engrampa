...
void open_file_free(OpenFile *ofile) {
  ...
  if (ofile->monitor != NULL) g_object_unref(ofile->monitor);
  g_free(ofile->path);
...
GType open_file_get_type(void) {
  ...
  if (type == 0)
    type = g_boxed_type_register_static("FROpenFile",
                                        (GBoxedCopyFunc)open_file_copy,
                                        (GBoxedFreeFunc)open_file_free);
