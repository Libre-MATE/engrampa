...
static Filter *filter_new(const char *pattern, FilterOptions options) {
  Filter *filter;
...
static void filter_destroy(Filter *filter) {
  if (filter == NULL) return;

  g_free(filter->pattern);
  if (filter->regexps != NULL) free_regexps(filter->regexps);
  g_free(filter);
}

static gboolean filter_matches(Filter *filter, const char *name) {
  ...
  if (filter->pattern == NULL) return TRUE;
...
static gboolean filter_empty(Filter *filter) {
...
static void for_each_child_data_free(ForEachChildData *fec) {
  if (fec == NULL) return;

  if (fec->base_directory != NULL) g_object_unref(fec->base_directory);
  if (fec->current != NULL) g_object_unref(fec->current);
  if (fec->already_visited) g_hash_table_destroy(fec->already_visited);
  if (fec->to_visit != NULL) g_list_free(fec->to_visit);
  g_free(fec);
}

static gboolean for_each_child_done_cb(gpointer user_data) {
  ...
  if (fec->done_func) fec->done_func(fec->error, fec->user_data);
  for_each_child_data_free(fec);

  return FALSE;
}

static void for_each_child_done(ForEachChildData *fec) {
  fec->source_id = g_idle_add(for_each_child_done_cb, fec);
}

static void for_each_child_start_current(ForEachChildData *fec);

static gboolean for_each_child_start_cb(gpointer user_data) {
  ForEachChildData *fec = user_data;
...
static void for_each_child_set_current_uri(ForEachChildData *fec,
                                           const char *directory) {
  if (fec->current != NULL) g_object_unref(fec->current);
  fec->current = g_file_new_for_uri(directory);
}

static void for_each_child_set_current(ForEachChildData *fec,
                                       GFile *directory) {
  if (fec->current != NULL) g_object_unref(fec->current);
  fec->current = g_file_dup(directory);
}

static void for_each_child_start_next_sub_directory(ForEachChildData *fec) {
...
static void for_each_child_close_enumerator(GObject *source_object,
                                            GAsyncResult *result,
                                            gpointer user_data) {
  ForEachChildData *fec = user_data;
  GError *error = NULL;

  if (!g_file_enumerator_close_finish(fec->enumerator, result, &error)) {
    if (fec->error == NULL)
...
static void for_each_child_next_files_ready(GObject *source_object,
  ...
  children = g_file_enumerator_next_files_finish(fec->enumerator, result,
                                                 &(fec->error));

  if (children == NULL) {
    g_file_enumerator_close_async(fec->enumerator, G_PRIORITY_DEFAULT,
                                  fec->cancellable,
                                  for_each_child_close_enumerator, fec);
    return;
  }
  ...
  for (scan = children; scan; scan = scan->next) {
    ...
    if (g_file_info_get_file_type(child_info) == G_FILE_TYPE_DIRECTORY) {
      ...
      if (g_hash_table_lookup(fec->already_visited, uri) == NULL) {
        ...
        g_hash_table_insert(fec->already_visited, sub_directory,
                            GINT_TO_POINTER(1));
  ...
  g_file_enumerator_next_files_async(fec->enumerator, N_FILES_PER_REQUEST,
                                     G_PRIORITY_DEFAULT, fec->cancellable,
                                     for_each_child_next_files_ready, fec);
}

static void for_each_child_ready(GObject *source_object, GAsyncResult *result,
                                 gpointer user_data) {
  ForEachChildData *fec = user_data;

  fec->enumerator =
      g_file_enumerate_children_finish(fec->current, result, &(fec->error));
...
static void for_each_child_start_current(ForEachChildData *fec) {
  ...
  g_file_enumerate_children_async(
      fec->current, "standard::name,standard::type",
      fec->follow_links ? G_FILE_QUERY_INFO_NONE
                        : G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
      G_PRIORITY_DEFAULT, fec->cancellable, for_each_child_ready, fec);
}
 ...
 */
void g_directory_foreach_child(GFile *directory, gboolean recursive,
                               gboolean follow_links, GCancellable *cancellable,
                               StartDirCallback start_dir_func,
                               ForEachChildCallback for_each_file_func,
                               ForEachDoneCallback done_func,
                               gpointer user_data) {
...
static void get_file_list_data_free(GetFileListData *gfl) {
  if (gfl == NULL) return;
  ...
  if (gfl->directory != NULL) g_object_unref(gfl->directory);
  if (gfl->base_dir != NULL) g_object_unref(gfl->base_dir);
  g_free(gfl);
}

/* -- g_directory_list_async -- */

static GList *get_relative_file_list(GList *rel_list, GList *file_list,
                                     GFile *base_dir) {
  GList *scan;
...
static GList *get_dir_list_from_file_list(GHashTable *h_dirs,
                                          const char *base_dir, GList *files,
  ...
  for (scan = files; scan; scan = scan->next) {
    ...
    if (strlen(filename) <= base_dir_len) continue;
    ...
    while ((dir_name != NULL) && (dir_name[0] != '\0') &&
           (strcmp(dir_name, "/") != 0)) {
      char *tmp;
      char *dir;
...
static void get_file_list_done(GError *error, gpointer user_data) {
  ...
  if (!filter_empty(gfl->include_filter) ||
      (gfl->exclude_filter->pattern != NULL)) {
    path_list_free(gfl->dirs);
    gfl->dirs = NULL;
  }
  ...
  gfl->dirs = g_list_concat(
      gfl->dirs, get_dir_list_from_file_list(h_dirs, uri, gfl->files, FALSE));

  if (filter_empty(gfl->include_filter))
    gfl->dirs = g_list_concat(
        gfl->dirs, get_dir_list_from_file_list(h_dirs, uri, gfl->dirs, TRUE));
...
static void get_file_list_for_each_file(const char *uri, GFileInfo *info,
                                        gpointer user_data) {
  GetFileListData *gfl = user_data;
  ...
  switch (g_file_info_get_file_type(info)) {
    case G_FILE_TYPE_REGULAR:
      if (filter_matches(gfl->include_filter, uri))
        if ((gfl->exclude_filter->pattern == NULL) ||
            !filter_matches(gfl->exclude_filter, uri))
...
static DirOp get_file_list_start_dir(const char *uri, GError **error,
                                     gpointer user_data) {
  GetFileListData *gfl = user_data;

  if ((gfl->exclude_folders_filter->pattern == NULL) ||
      !filter_matches(gfl->exclude_folders_filter, uri)) {
...
void g_directory_list_async(const char *directory, const char *base_dir,
                            gboolean recursive, gboolean follow_links,
                            gboolean no_backup_files, gboolean no_dot_files,
                            const char *include_files,
                            const char *exclude_files,
                            const char *exclude_folders, gboolean ignorecase,
                            GCancellable *cancellable,
                            ListReadyCallback done_func, gpointer done_data) {
  GetFileListData *gfl;
  ...
  if (no_backup_files) filter_options |= FILTER_NOBACKUPFILES;
  if (no_dot_files) filter_options |= FILTER_NODOTFILES;
  if (ignorecase) filter_options |= FILTER_IGNORECASE;
  gfl->include_filter = filter_new(include_files, filter_options);
  gfl->exclude_filter = filter_new(
      exclude_files, ignorecase ? FILTER_IGNORECASE : FILTER_DEFAULT);
  gfl->exclude_folders_filter = filter_new(
      exclude_folders, ignorecase ? FILTER_IGNORECASE : FILTER_DEFAULT);

  g_directory_foreach_child(gfl->directory, recursive, follow_links,
                            cancellable, get_file_list_start_dir,
                            get_file_list_for_each_file, get_file_list_done,
                            gfl);
}
...
static gboolean get_items_for_next_dir_idle_cb(gpointer data) {
...
static void get_items_for_current_dir_done(GList *files, GList *dirs,
                                           GError *error, gpointer data) {
  GetFileListData *gfl = data;

  if (error != NULL) {
    if (gfl->done_func) gfl->done_func(NULL, NULL, error, gfl->done_data);
...
static void get_items_for_current_dir(GetFileListData *gfl) {
  ...
  g_directory_list_all_async(directory_uri, base_dir_uri, TRUE,
                             gfl->cancellable, get_items_for_current_dir_done,
                             gfl);
...
void g_list_items_async(GList *items, const char *base_dir,
                        GCancellable *cancellable, ListReadyCallback done_func,
                        gpointer done_data) {
  GetFileListData *gfl;
  int base_len;
  GList *scan;
...
static CopyFilesData *copy_files_data_new(
    GList *sources, GList *destinations, GFileCopyFlags flags, int io_priority,
    GCancellable *cancellable, CopyProgressCallback progress_callback,
    gpointer progress_callback_data, CopyDoneCallback callback,
    gpointer user_data) {
  CopyFilesData *cfd;
...
static void copy_files_data_free(CopyFilesData *cfd) {
  if (cfd == NULL) return;
...
static void g_copy_next_file(CopyFilesData *cfd) {
...
static void g_copy_files_ready_cb(GObject *source_object, GAsyncResult *result,
                                  gpointer user_data) {
  ...
  if (!g_file_copy_finish(source, result, &error)) {
    ...
    if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_WOULD_RECURSE)) {
      g_clear_error(&error);
      g_file_make_directory((GFile *)cfd->destination->data, cfd->cancellable,
                            &error);
    }
  }

  if (error) {
    if (cfd->callback) cfd->callback(error, cfd->user_data);
    g_clear_error(&error);
...
static void g_copy_files_progress_cb(goffset current_num_bytes,
                                     goffset total_num_bytes,
                                     gpointer user_data) {
  CopyFilesData *cfd = user_data;

  if (cfd->progress_callback)
    cfd->progress_callback(cfd->n_file, cfd->tot_files,
                           (GFile *)cfd->source->data,
                           (GFile *)cfd->destination->data, current_num_bytes,
                           total_num_bytes, cfd->progress_callback_data);
}

static void g_copy_current_file(CopyFilesData *cfd) {
  if ((cfd->source == NULL) || (cfd->destination == NULL)) {
    if (cfd->callback) cfd->callback(NULL, cfd->user_data);
    copy_files_data_free(cfd);
    return;
  }

  g_file_copy_async((GFile *)cfd->source->data, (GFile *)cfd->destination->data,
                    cfd->flags, cfd->io_priority, cfd->cancellable,
                    g_copy_files_progress_cb, cfd, g_copy_files_ready_cb, cfd);
}

void g_copy_files_async(GList *sources, GList *destinations,
                        GFileCopyFlags flags, int io_priority,
                        GCancellable *cancellable,
                        CopyProgressCallback progress_callback,
                        gpointer progress_callback_data,
                        CopyDoneCallback callback, gpointer user_data) {
  CopyFilesData *cfd;

  cfd = copy_files_data_new(sources, destinations, flags, io_priority,
                            cancellable, progress_callback,
                            progress_callback_data, callback, user_data);
  g_copy_current_file(cfd);
}

void g_copy_file_async(GFile *source, GFile *destination, GFileCopyFlags flags,
  ...
  g_copy_files_async(source_files, destination_files, flags, io_priority,
                     cancellable, progress_callback, progress_callback_data,
                     callback, user_data);
...
static ChildData *child_data_new(const char *uri, GFileInfo *info) {
  ChildData *data;
...
static void child_data_free(ChildData *child) {
...
static void directory_copy_data_free(DirectoryCopyData *dcd) {
  if (dcd == NULL) return;

  if (dcd->source != NULL) g_object_unref(dcd->source);
  if (dcd->destination != NULL) g_object_unref(dcd->destination);
...
static gboolean g_directory_copy_done(gpointer user_data) {
  DirectoryCopyData *dcd = user_data;

  g_source_remove(dcd->source_id);

  if (dcd->callback) dcd->callback(dcd->error, dcd->user_data);
  if (dcd->error != NULL) g_clear_error(&(dcd->error));
  directory_copy_data_free(dcd);

  return FALSE;
}

static GFile *get_destination_for_uri(DirectoryCopyData *dcd, const char *uri) {
  GFile *f_uri;
  ...
  if (relative_path != NULL)
    destination_file =
        g_file_resolve_relative_path(dcd->destination, relative_path);
  else
...
static gboolean g_directory_copy_next_child(gpointer user_data) {
  DirectoryCopyData *dcd = user_data;
...
static void g_directory_copy_child_done_cb(GObject *source_object,
                                           GAsyncResult *result,
                                           gpointer user_data) {
  DirectoryCopyData *dcd = user_data;
...
static void g_directory_copy_child_progress_cb(goffset current_num_bytes,
                                               goffset total_num_bytes,
                                               gpointer user_data) {
  DirectoryCopyData *dcd = user_data;

  if (dcd->progress_callback)
    dcd->progress_callback(dcd->n_file, dcd->tot_files, dcd->current_source,
                           dcd->current_destination, current_num_bytes,
                           total_num_bytes, dcd->progress_callback_data);
}
...
static void g_directory_copy_current_child(DirectoryCopyData *dcd) {
  ...
  switch (g_file_info_get_file_type(child->info)) {
    case G_FILE_TYPE_DIRECTORY:
      ...
      g_file_make_directory(dcd->current_destination, NULL, NULL);
    ...
    case G_FILE_TYPE_SYMBOLIC_LINK:
      ...
      break;
    case G_FILE_TYPE_REGULAR:
      g_file_copy_async(dcd->current_source, dcd->current_destination,
                        dcd->flags, dcd->io_priority, dcd->cancellable,
                        g_directory_copy_child_progress_cb, dcd,
                        g_directory_copy_child_done_cb, dcd);
      async_op = TRUE;
      break;
    default:
      break;
  ...
  if (!async_op) dcd->source_id = g_idle_add(g_directory_copy_next_child, dcd);
}

static gboolean g_directory_copy_start_copying(gpointer user_data) {
  DirectoryCopyData *dcd = user_data;
...
static void g_directory_copy_list_ready(GError *error, gpointer user_data) {
  DirectoryCopyData *dcd = user_data;
...
static void g_directory_copy_for_each_file(const char *uri, GFileInfo *info,
                                           gpointer user_data) {
  DirectoryCopyData *dcd = user_data;
...
static DirOp g_directory_copy_start_dir(const char *uri, GError **error,
                                        gpointer user_data) {
  DirectoryCopyData *dcd = user_data;
  GFileInfo *info;
...
void g_directory_copy_async(const char *source, const char *destination,
                            GFileCopyFlags flags, int io_priority,
                            GCancellable *cancellable,
                            CopyProgressCallback progress_callback,
                            gpointer progress_callback_data,
  ...
  g_directory_foreach_child(
      dcd->source, TRUE, TRUE, dcd->cancellable, g_directory_copy_start_dir,
      g_directory_copy_for_each_file, g_directory_copy_list_ready, dcd);
}

gboolean g_load_file_in_buffer(GFile *file, void *buffer, gsize size,
                               GError **error) {
