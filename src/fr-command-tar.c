...
static time_t mktime_from_string(const char *time_s) {
  struct tm tm = {
      0,
  };
  tm.tm_isdst = -1;
...
static char *tar_get_last_field(const char *line, int start_from, int field_n) {
  const char *f_start, *f_end;

  line = line + start_from;

  f_start = line;
  while ((*f_start == ' ') && (*f_start != *line)) f_start++;
  f_end = f_start;

  while ((field_n > 0) && (*f_end != 0)) {
    if (*f_end == ' ') {
      field_n--;
      if (field_n == 1) f_start = f_end;
    }
    f_end++;
  }
...
static void process_line(char *line, gpointer data) {
  ...
  date_idx =
      file_list__get_index_from_pattern(line, "%n%n%n%n-%n%n-%n%n %n%n:%n%n");
  if (date_idx < 0) return;
...
static void add_compress_arg(FrCommand *comm) {
  if (is_mime_type(comm->mime_type, "application/x-compressed-tar"))
    ...
    if (is_program_in_path("gzip"))
      ...
      fr_process_add_arg(comm->process, "-Z");
  } else if (is_mime_type(comm->mime_type,
    ...
    option = g_strdup_printf("--use-compress-program=%s",
                             comm_tar->compress_command);
    fr_process_add_arg(comm->process, option);
    g_free(option);
  }
}

static void begin_tar_command(FrCommand *comm) {
  char *command = NULL;
...
static void fr_command_tar_list(FrCommand *comm) {
...
static gboolean can_create_a_compressed_archive(FrCommand *comm) {
  return comm->creating_archive &&
         !is_mime_type(comm->mime_type, "application/x-7z-compressed-tar");
}

static void process_line__generic(char *line, gpointer data, char *action_msg) {
  FrCommand *comm = FR_COMMAND(data);
...
static void process_line__add(char *line, gpointer data) {
  /* Translators: after the colon there is a filename. */
  process_line__generic(line, data, _("Adding file: "));
}

static void fr_command_tar_add(FrCommand *comm, const char *from_file,
                               GList *file_list, const char *base_dir,
                               gboolean update, gboolean recursive) {
  FrCommandTar *c_tar = FR_COMMAND_TAR(comm);
  GList *scan;

  fr_process_set_out_line_func(FR_COMMAND(comm)->process, process_line__add,
                               comm);

  begin_tar_command(comm);
  fr_process_add_arg(comm->process, "--force-local");
  if (!recursive) fr_process_add_arg(comm->process, "--no-recursion");
  fr_process_add_arg(comm->process, "--no-wildcards");
...
static void process_line__delete(char *line, gpointer data) {
  /* Translators: after the colon there is a filename. */
  process_line__generic(line, data, _("Removing file: "));
}

static void begin_func__delete(gpointer data) {
  FrCommand *comm = data;
  fr_command_progress(comm, -1.0);
  fr_command_message(comm, _("Deleting files from archive"));
}

static void fr_command_tar_delete(FrCommand *comm, const char *from_file,
                                  GList *file_list) {
  FrCommandTar *c_tar = FR_COMMAND_TAR(comm);
  GList *scan;

  fr_process_set_out_line_func(comm->process, process_line__delete, comm);
...
static void process_line__extract(char *line, gpointer data) {
  /* Translators: after the colon there is a filename. */
  process_line__generic(line, data, _("Extracting file: "));
}

static void fr_command_tar_extract(FrCommand *comm, const char *from_file,
                                   GList *file_list, const char *dest_dir,
                                   gboolean overwrite, gboolean skip_older,
                                   gboolean junk_paths) {
  GList *scan;

  fr_process_set_out_line_func(comm->process, process_line__extract, comm);
  ...
  if (!overwrite) fr_process_add_arg(comm->process, "-k");
  if (skip_older) fr_process_add_arg(comm->process, "--keep-newer-files");
...
static void begin_func__recompress(gpointer data) {
  FrCommand *comm = data;
  fr_command_progress(comm, -1.0);
  fr_command_message(comm, _("Recompressing archive"));
}

static gboolean gzip_continue_func(gpointer user_data) {
...
static void fr_command_tar_recompress(FrCommand *comm) {
  FrCommandTar *c_tar = FR_COMMAND_TAR(comm);
  char *new_name = NULL;

  if (can_create_a_compressed_archive(comm)) return;
  ...
  if (is_mime_type(comm->mime_type, "application/x-compressed-tar")) {
    ...
    switch (comm->compression) {
      ...
      case FR_COMPRESSION_FAST:
        fr_process_add_arg(comm->process, "-3");
        break;
      case FR_COMPRESSION_NORMAL:
        fr_process_add_arg(comm->process, "-6");
        break;
      case FR_COMPRESSION_MAXIMUM:
        fr_process_add_arg(comm->process, "-9");
        break;
    }
    fr_process_add_arg(comm->process, "-f");
    fr_process_add_arg(comm->process, c_tar->uncomp_filename);
    ...
    switch (comm->compression) {
      ...
      case FR_COMPRESSION_NORMAL:
        ...
        break;
      case FR_COMPRESSION_MAXIMUM:
        fr_process_add_arg(comm->process, "--best");
        break;  // i.e. -q 11
    }
    fr_process_add_arg(comm->process, "-f");
    fr_process_add_arg(comm->process, c_tar->uncomp_filename);
    ...
    switch (comm->compression) {
      ...
      case FR_COMPRESSION_FAST:
        ...
        break;
      case FR_COMPRESSION_NORMAL:
        fr_process_add_arg(comm->process, "-b");
        break;
      case FR_COMPRESSION_MAXIMUM:
        fr_process_add_arg(comm->process, "-z");
        break;
...
static void begin_func__uncompress(gpointer data) {
  FrCommand *comm = data;
  fr_command_progress(comm, -1.0);
  fr_command_message(comm, _("Decompressing archive"));
}

static char *get_uncompressed_name(FrCommandTar *c_tar,
  ...
  if (is_mime_type(comm->mime_type, "application/x-compressed-tar")) {
    ...
    if (file_extension_is(e_filename, ".tgz")) {
      ...
      new_name[l - 3] = 0;
                          ...
                          "application/x-brotli-compressed-tar")) {
    /* X.tar.br -->  X.tar */
    if (file_extension_is(e_filename, ".tar.br")) new_name[l - 3] = 0;
  } else if (is_mime_type(comm->mime_type,
                          "application/x-bzip-compressed-tar")) {
    /* X.tbz2    -->  X.tar
     * X.tar.bz2 -->  X.tar */
    ...
    if (file_extension_is(e_filename, ".tar.lzma")) new_name[l - 5] = 0;
  } else if (is_mime_type(comm->mime_type, "application/x-xz-compressed-tar")) {
    /* X.txz     -->  X.tar
     * X.tar.xz  -->  X.tar */
    ...
    /* X.tar.7z -->  X.tar */
    if (file_extension_is(e_filename, ".tar.7z")) new_name[l - 3] = 0;
  } else if (is_mime_type(comm->mime_type,
                          "application/x-zstd-compressed-tar")) {
    /* X.tar.zst -->  X.tar */
    if (file_extension_is(e_filename, ".tar.zst")) new_name[l - 4] = 0;
  }

  return new_name;
}

static char *get_temp_name(FrCommandTar *c_tar, const char *filepath) {
...
static void fr_command_tar_uncompress(FrCommand *comm) {
  FrCommandTar *c_tar = FR_COMMAND_TAR(comm);
  char *tmp_name;
...
static void fr_command_tar_handle_error(FrCommand *comm, FrProcError *error) {
  if (error->type != FR_PROC_ERROR_NONE) {
    if (error->status <= 1) error->type = FR_PROC_ERROR_NONE;
  }
}
...
static const char **fr_command_tar_get_mime_types(FrCommand *comm) {
  return tar_mime_types;
}

static FrCommandCaps fr_command_tar_get_capabilities(FrCommand *comm,
  ...
  /* In solaris gtar is present under /usr/sfw/bin */
  if (!is_program_available("tar", check_command) &&
      !is_program_available("/usr/sfw/bin/gtar", check_command))
    return capabilities;
  ...
  if (is_mime_type(mime_type, "application/x-tar")) {
    ...
    if (is_program_available("compress", check_command) &&
        is_program_available("uncompress", check_command))
      capabilities |= FR_COMMAND_CAN_READ_WRITE;
...
static void fr_command_tar_set_mime_type(FrCommand *comm,
                                         const char *mime_type) {
...
static const char *fr_command_tar_get_packages(FrCommand *comm,
                                               const char *mime_type) {
  if (is_mime_type(mime_type, "application/x-tar"))
    return PACKAGES("tar");
  else if (is_mime_type(mime_type, "application/x-compressed-tar"))
...
static void fr_command_tar_init(FrCommand *comm) {
  FrCommandTar *comm_tar = (FrCommandTar *)comm;
...
GType fr_command_tar_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandTarClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandTar", &type_info, 0);
  }
