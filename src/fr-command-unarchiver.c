...
static void process_line(char *line, gpointer data) {
  ...
  g_memory_input_stream_add_data(G_MEMORY_INPUT_STREAM(unar_comm->stream), line,
                                 -1, NULL);
}
...
static void list_command_completed(gpointer data) {
  FrCommandUnarchiver *unar_comm = FR_COMMAND_UNARCHIVER(data);
  JsonParser *parser;
  GError *error = NULL;
  ...
  if (json_parser_load_from_stream(parser, unar_comm->stream, NULL, &error)) {
    ...
    if (json_object_get_int_member(root, "lsarFormatVersion") ==
        LSAR_SUPPORTED_FORMAT) {
      JsonArray *content;
      guint i;
...
static void fr_command_unarchiver_list(FrCommand *comm) {
  ...
  if ((comm->password != NULL) && (comm->password[0] != '\0'))
    fr_process_add_arg_concat(comm->process, "-password=", comm->password,
                              NULL);
...
static void fr_command_unarchiver_extract(
    FrCommand *comm, const char *from_file, GList *file_list,
  ...
  if (dest_dir != NULL)
    fr_process_add_arg_concat(comm->process, "-output-directory=", dest_dir,
                              NULL);
...
static void fr_command_unarchiver_handle_error(FrCommand *comm,
                                               FrProcError *error) {
  GList *scan;
  ...
  if (error->type == FR_PROC_ERROR_COMMAND_ERROR) {
    for (scan = g_list_last(comm->process->out.raw); scan; scan = scan->prev) {
      ...
      if ((strstr(line, "This archive requires a password to unpack.") !=
           NULL) ||
...
const char *unarchiver_mime_type[] = {"application/zip", "application/x-cbr",
                                      "application/x-rar", NULL};

static const char **fr_command_unarchiver_get_mime_types(FrCommand *comm) {
  return unarchiver_mime_type;
}

static FrCommandCaps fr_command_unarchiver_get_capabilities(
    FrCommand *comm, const char *mime_type, gboolean check_command) {
  FrCommandCaps capabilities;

  capabilities = FR_COMMAND_CAN_DO_NOTHING;
  if (is_program_available("lsar", check_command) &&
      is_program_available("unar", check_command))
    capabilities |= FR_COMMAND_CAN_READ;

  return capabilities;
}

static const char *fr_command_unarchiver_get_packages(FrCommand *comm,
...
static void fr_command_unarchiver_init(FrCommand *comm) {
...
GType fr_command_unarchiver_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandUnarchiverClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(FR_TYPE_COMMAND, "FrCommandUnarchiver",
                                  &type_info, 0);
  }
