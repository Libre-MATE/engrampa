...
static gboolean fr_list_model_multi_row_draggable(
...
static gboolean fr_list_model_multi_drag_data_get(
  ...
  return fr_window_file_list_drag_data_get(window, context, selection_data,
                                           path_list);
}

static gboolean fr_list_model_multi_drag_data_delete(
    EggTreeMultiDragSource *drag_source, GList *path_list) {
  return TRUE;
}

static void fr_list_model_finalize(GObject *object) {
  if (G_OBJECT_CLASS(parent_class)->finalize)
    G_OBJECT_CLASS(parent_class)->finalize(object);
}

static void fr_list_model_init(FRListModel *model) {}

static void fr_list_model_class_init(FRListModelClass *klass) {
...
static void fr_list_model_multi_drag_source_init(
...
GType fr_list_model_get_type(void) {
  ...
  if (object_type == 0) {
    ...
    static const GInterfaceInfo multi_drag_source_info = {
        (GInterfaceInitFunc)fr_list_model_multi_drag_source_init, NULL, NULL};

    object_type = g_type_register_static(GTK_TYPE_LIST_STORE, "FRListModel",
                                         &object_info, 0);
    g_type_add_interface_static(object_type, EGG_TYPE_TREE_MULTI_DRAG_SOURCE,
...
GtkListStore *fr_list_model_new(int n_columns, ...) {
  ...
  for (i = 0; i < n_columns; i++) types[i] = va_arg(args, GType);
  va_end(args);
