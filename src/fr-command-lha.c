...
static time_t mktime_from_string(char *month, char *mday, char *time_or_year) {
  ...
  if (strchr(time_or_year, ':') == NULL)
    ...
    if (tm_now != NULL) tm.tm_year = tm_now->tm_year;
...
static char **split_line_lha(char *line) {
  char **fields;
  int n_fields = 7;
  const char *scan;
  int i;
...
static const char *get_last_field_lha(char *line) {
  int i;
  const char *field;
  int n = 7;

  if (strncmp(line, "[MS-DOS]", 8) == 0) n--;

  if (strncmp(line, "[generic]", 9) == 0) n--;

  if (strncmp(line, "[unknown]", 9) == 0) n--;

  if (strncmp(line, "[Amiga]", 7) == 0) n--;
...
static void process_line(char *line, gpointer data) {
  ...
  fdata->modified = mktime_from_string(fields[4], fields[5], fields[6]);
  g_strfreev(fields);
...
static void fr_command_lha_list(FrCommand *comm) {
...
static void fr_command_lha_add(FrCommand *comm, const char *from_file,
...
static void fr_command_lha_delete(FrCommand *comm, const char *from_file,
                                  GList *file_list) {
  GList *scan;
...
static void fr_command_lha_extract(FrCommand *comm, const char *from_file,
...
static void fr_command_lha_test(FrCommand *comm) {
  fr_process_begin_command(comm->process, "lha");
...
const char *lha_mime_type[] = {"application/x-lzh-compressed",
                               "application/x-lha", NULL};

static const char **fr_command_lha_get_mime_types(FrCommand *comm) {
  return lha_mime_type;
}

static FrCommandCaps fr_command_lha_get_capabilities(FrCommand *comm,
...
static const char *fr_command_lha_get_packages(FrCommand *comm,
                                               const char *mime_type) {
  return PACKAGES("lha");
}
...
static void fr_command_lha_init(FrCommand *comm) {
...
GType fr_command_lha_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandLhaClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandLha", &type_info, 0);
  }
