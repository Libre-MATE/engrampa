...
static time_t mktime_from_string(char *datetime_s) {
  struct tm tm = {
      0,
  };
  char *date;
...
static void list__process_line(char *line, gpointer data) {
  ...
  if (FR_COMMAND_ZIP(comm)->is_empty) return;
   ...
   * directory. */
  if ((line[0] != '?') && (line[0] != 'd') && (line[0] != '-')) return;
...
static void fr_command_zip_list(FrCommand *comm) {
...
static void process_line__common(char *line, gpointer data) {
  FrCommand *comm = FR_COMMAND(data);
...
static void fr_command_zip_add(FrCommand *comm, const char *from_file,
                               GList *file_list, const char *base_dir,
                               gboolean update, gboolean recursive) {
  GList *scan;

  fr_process_set_out_line_func(FR_COMMAND(comm)->process, process_line__common,
                               comm);
  ...
  if (update) fr_process_add_arg(comm->process, "-u");
...
static void fr_command_zip_delete(FrCommand *comm, const char *from_file,
                                  GList *file_list) {
  GList *scan;
...
static void fr_command_zip_extract(FrCommand *comm, const char *from_file,
  ...
  if (junk_paths) fr_process_add_arg(comm->process, "-j");
  add_password_arg(comm, comm->password);
...
static void fr_command_zip_test(FrCommand *comm) {
  fr_process_begin_command(comm->process, "unzip");
...
static void fr_command_zip_handle_error(FrCommand *comm, FrProcError *error) {
...
static const char **fr_command_zip_get_mime_types(FrCommand *comm) {
  GSettings *settings;
...
static FrCommandCaps fr_command_zip_get_capabilities(FrCommand *comm,
...
static const char *fr_command_zip_get_packages(FrCommand *comm,
                                               const char *mime_type) {
...
static void fr_command_zip_init(FrCommand *comm) {
...
GType fr_command_zip_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandZipClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandZip", &type_info, 0);
  }
