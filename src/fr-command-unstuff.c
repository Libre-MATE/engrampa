...
static char *unstuff_is_shit_with_filenames(const char *orig) {
  int i, num_slashes;
...
static void process_line(char *line, gpointer data) {
  ...
  if (strstr(line, "progressEvent - ")) {
    ...
    ssize = strstr(line, "progressEvent - ") + strlen("progressEvent - ");
    if (ssize[0] == '\0')
      size = 0;
    else
      size = g_ascii_strtoull(ssize, NULL, 10);

    if (unstuff_comm->fdata != NULL) unstuff_comm->fdata->size = size;

    return;
  }

  if (strstr(line, "fileEvent") == NULL) return;
  if (strstr(line, unstuff_comm->target_dir + 1) == NULL) return;
...
static void fr_command_unstuff_list(FrCommand *comm) {
  char *arg, *path;
  char *filename;
  char *path_dots;
...
static void fr_command_unstuff_extract(FrCommand *comm, const char *from_file,
...
static void fr_command_unstuff_handle_error(FrCommand *comm,
                                            FrProcError *error) {
  if ((error->type != FR_PROC_ERROR_NONE) && (error->status <= 1)) {
...
static const char **fr_command_unstuff_get_mime_types(FrCommand *comm) {
  return unstuff_mime_type;
}

static FrCommandCaps fr_command_unstuff_get_capabilities(
...
static const char *fr_command_unstaff_get_packages(FrCommand *comm,
...
static void fr_command_unstuff_init(FrCommand *comm) {
...
GType fr_command_unstuff_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandUnstuffClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(FR_TYPE_COMMAND, "FRCommandUnstuff",
                                  &type_info, 0);
  }
