...
static void fr_command_nomarch_list(FrCommand *comm) {
...
static void fr_command_nomarch_add(FrCommand *comm, const char *from_file,
...
static void fr_command_nomarch_delete(FrCommand *comm, const char *from_file,
                                      GList *file_list) {
  GList *scan;
...
static void fr_command_nomarch_extract(FrCommand *comm, const char *from_file,
...
static void fr_command_nomarch_test(FrCommand *comm) {
  fr_process_begin_command(comm->process, "nomarch");
...
static void fr_command_nomarch_handle_error(FrCommand *comm,
                                            FrProcError *error) {
...
static const char **fr_command_nomarch_get_mime_types(FrCommand *comm) {
  return nomarch_mime_type;
}

static FrCommandCaps fr_command_nomarch_get_capabilities(
...
static const char *fr_command_nomarch_get_packages(FrCommand *comm,
...
static void fr_command_nomarch_init(FrCommand *comm) {
...
GType fr_command_nomarch_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandNomarchClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(FR_TYPE_COMMAND, "FRCommandNomarch",
                                  &type_info, 0);
  }
