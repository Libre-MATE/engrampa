...
static int help_cb(GtkWidget *w, DialogData *data) {
  show_help_dialog(GTK_WINDOW(data->dialog),
                   "engrampa-view-archive-properties");
  return TRUE;
}
...
void dlg_prop(FrWindow *window) {
  ...
  builder = gtk_builder_new_from_resource(
      ENGRAMPA_RESOURCE_UI_PATH G_DIR_SEPARATOR_S "properties.ui");
  ...
  GDateTime *date_time;
  date_time = g_date_time_new_from_unix_local(
      get_file_mtime(fr_window_get_archive_uri(window)));
  ...
  gtk_label_set_text(GET_LABEL("p_mime_type_label"),
                     window->archive->command->mime_type);

  /* Set the signals handlers. */

  gtk_builder_add_callback_symbols(
      builder, "on_prop_dialog_destroy", G_CALLBACK(destroy_cb),
      "on_p_help_button_clicked", G_CALLBACK(help_cb), NULL);
