...
static time_t mktime_from_string(char *date_s, char *time_s) {
  struct tm tm = {
      0,
  };
  char **fields;
  ...
  if (fields[0] != NULL) {
    ...
    if (fields[1] != NULL) {
      ...
      if (fields[2] != NULL) tm.tm_sec = atoi(fields[2]);
    }
  }
  g_strfreev(fields);
...
static void list__process_line(char *line, gpointer data) {
  ...
  if (!p7z_comm->list_started) {
    if (strncmp(line, "p7zip Version ", 14) == 0) {
      ...
      if ((strcmp(version, "9.38") < 0) && (ver_len > 1) && (version[1] == '.'))
        ...
        spd_support = TRUE;
    } else if (p7z_comm->old_style &&
               (strncmp(line, "Listing archive: ", 17) == 0))
  ...
  if (p7z_comm->fdata == NULL) p7z_comm->fdata = file_data_new();
  ...
  if (strcmp(fields[0], "Path") == 0) {
    ...
    fdata->full_path = g_strconcat(
        (fdata->original_path[0] != '/') ? "/" : "", fdata->original_path,
        (fdata->dir &&
         (fdata->original_path[strlen(fdata->original_path) - 1] != '/'))
            ? "/"
            : "",
        NULL);
    ...
    if (modified_fields[0] != NULL)
      fdata->modified =
          mktime_from_string(modified_fields[0], modified_fields[1]);
    g_strfreev(modified_fields);
  } else if (strcmp(fields[0], "Encrypted") == 0) {
    if (strcmp(fields[1], "+") == 0) fdata->encrypted = TRUE;
  } else if (strcmp(fields[0], "Method") == 0) {
    if (strstr(fields[1], "AES") != NULL) fdata->encrypted = TRUE;
  } else if (strcmp(fields[0], "Attributes") == 0) {
    if (fields[1][0] == 'D') fdata->dir = TRUE;
  }
  g_strfreev(fields);
}

static void fr_command_7z_begin_command(FrCommand *comm) {
...
static void fr_command_7z_list(FrCommand *comm) {
...
static void process_line__add(char *line, gpointer data) {
  ...
  if (strstr(line, "Enter password") != NULL) password_required = TRUE;
...
static void fr_command_7z_add(FrCommand *comm, const char *from_file,
...
static void fr_command_7z_delete(FrCommand *comm, const char *from_file,
                                 GList *file_list) {
  GList *scan;
...
static void fr_command_7z_extract(FrCommand *comm, const char *from_file,
  ...
  if (unexpected_end_of_archive)
    fr_process_set_ignore_error(comm->process, TRUE);

  fr_process_end_command(comm->process);
}

static void fr_command_7z_test(FrCommand *comm) {
  fr_command_7z_begin_command(comm);
...
static void fr_command_7z_handle_error(FrCommand *comm, FrProcError *error) {
  if (error->type == FR_PROC_ERROR_NONE) {
    ...
    if (comm->files->len != 1) return;

    if (!g_str_has_suffix(comm->filename, ".001")) return;
...
const char *sevenz_mime_types[] = {
    "application/epub+zip", "application/x-7z-compressed", "application/x-arj",
    "application/vnd.ms-cab-compressed", "application/x-cd-image",
    /*"application/x-cbr",*/
    "application/x-cbz", "application/x-ms-dos-executable",
...
static const char **fr_command_7z_get_mime_types(FrCommand *comm) {
  GSettings *settings;
...
static FrCommandCaps fr_command_7z_get_capabilities(FrCommand *comm,
                                                    const char *mime_type,
                                                    gboolean check_command) {
  FrCommandCaps capabilities;

  capabilities = FR_COMMAND_CAN_ARCHIVE_MANY_FILES;
  if (!is_program_available("7za", check_command) &&
  ...
  if (is_mime_type(mime_type, "application/x-7z-compressed")) {
    ...
    if (is_mime_type(mime_type, "application/x-rar") ||
      ...
      if (!check_command ||
          ...
          g_file_test("/usr/libexec/p7zip/Codecs/Rar29.so",
                      G_FILE_TEST_EXISTS) ||
          g_file_test("/usr/libexec/p7zip/Codecs/Rar.so", G_FILE_TEST_EXISTS))
...
static const char *fr_command_7z_get_packages(FrCommand *comm,
                                              const char *mime_type) {
  if (is_mime_type(mime_type, "application/x-rar"))
    return PACKAGES("p7zip,p7zip-rar");
  else if (is_mime_type(mime_type, "application/zip") ||
           is_mime_type(mime_type, "application/vnd.ms-cab-compressed"))
...
static void fr_command_7z_init(FrCommand *comm) {
...
GType fr_command_7z_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommand7zClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommand7z", &type_info, 0);
  }
