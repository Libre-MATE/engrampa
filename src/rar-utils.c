...
static char *get_first_volume_name(const char *name, const char *pattern,
  ...
  if (g_regex_match(re, name, 0, NULL)) {
    ...
    switch (extension_type) {
      case FIRST_VOLUME_IS_000:
        for (i = 0; i < l; i++) parts[2][i] = '0';
        break;

      case FIRST_VOLUME_IS_001:
        for (i = 0; i < l; i++) parts[2][i] = (i < l - 1) ? '0' : '1';
        break;
...
void rar_check_multi_volume(FrCommand *comm) {
  ...
  if (memcmp(buffer, "Rar!", 4) != 0) return;

  if ((buffer[10] & 0x01) == 0x01) {
    char *volume_name = NULL;
    char *name;

    name = g_filename_to_utf8(file_name_from_path(comm->filename), -1, NULL,
                              NULL, NULL);

    volume_name = get_first_volume_name(name, "^(.*\\.part)([0-9]+)(\\.rar)$",
                                        FIRST_VOLUME_IS_001);
    if (volume_name == NULL)
      volume_name = get_first_volume_name(name, "^(.*\\.r)([0-9]+)$",
                                          FIRST_VOLUME_IS_RAR);
    if (volume_name == NULL)
      volume_name =
          get_first_volume_name(name, "^(.*\\.)([0-9]+)$", FIRST_VOLUME_IS_001);
