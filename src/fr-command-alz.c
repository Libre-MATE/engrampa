...
static time_t mktime_from_string(char *date_s, char *time_s) {
  ...
  if (fields[0] != NULL) {
    ...
    if (fields[1] != NULL) {
      ...
      if (fields[2] != NULL) tm.tm_year = 100 + atoi(fields[2]);
    }
  }
  g_strfreev(fields);
  ...
  if (fields[0] != NULL) {
    ...
    if (fields[1] != NULL) tm.tm_min = atoi(fields[1]);
  }
  g_strfreev(fields);
...
static void process_line(char *line, gpointer data) {
  ...
  if (!alz_comm->list_started) {
    if (strncmp(line, "-----", 5) == 0) alz_comm->list_started = TRUE;
    return;
  }
  ...
  if (!alz_comm->list_started) return;
  ...
  if (fdata->dir || fdata->encrypted) name_field[--name_len] = '\0';
...
static void add_codepage_arg(FrCommand *comm) {
...
static void fr_command_alz_list(FrCommand *comm) {
...
static void process_extract_line(char *line, gpointer data) {
  FrCommand *comm = FR_COMMAND(data);
  FrCommandAlz *alz_comm = FR_COMMAND_ALZ(comm);
...
static void fr_command_alz_extract(FrCommand *comm, const char *from_file,
  ...
  fr_process_set_out_line_func(FR_COMMAND(comm)->process, process_extract_line,
                               comm);
...
static void fr_command_alz_handle_error(FrCommand *comm, FrProcError *error) {
...
static const char **fr_command_alz_get_mime_types(FrCommand *comm) {
  return alz_mime_type;
}

static FrCommandCaps fr_command_alz_get_capabilities(FrCommand *comm,
...
static const char *fr_command_alz_get_packages(FrCommand *comm,
                                               const char *mime_type) {
...
static void fr_command_alz_init(FrCommand *comm) {
...
GType fr_command_alz_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandAlzClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FrCommandAlz", &type_info, 0);
  }
