...
#define GET_FILE_CHOOSER(x) \
  (GTK_FILE_CHOOSER(gtk_builder_get_object(data->builder, (x))))
#define GET_TOGGLE_BUTTON(x) \
  (GTK_TOGGLE_BUTTON(gtk_builder_get_object(data->builder, (x))))
...
static const char *get_ext(DialogData *data) {
  int idx;
...
static void destroy_cb(GtkWidget *widget, DialogData *data) {
  g_settings_set_string(data->settings, PREF_BATCH_ADD_DEFAULT_EXTENSION,
                        get_ext(data));
  /*g_settings_set_boolean (data->settings, PREF_BATCH_ADD_OTHER_OPTIONS,
   * data->add_clicked ? FALSE : gtk_expander_get_expanded (GTK_EXPANDER
   * (GET_WIDGET ("a_other_options_expander"))));*/
  g_settings_set_boolean(data->settings_general, PREF_GENERAL_ENCRYPT_HEADER,
                         gtk_toggle_button_get_active(GET_TOGGLE_BUTTON(
                             "a_encrypt_header_checkbutton")));
...
static void set_archive_options(DialogData *data) {
  int idx;

  idx = gtk_combo_box_get_active(GTK_COMBO_BOX(data->archive_type_combo_box));
  if (mime_type_desc[data->supported_types[idx]].capabilities &
      FR_COMMAND_CAN_ENCRYPT) {
    const char *pwd;
    ...
    if (pwd != NULL) {
      if (strcmp(pwd, "") != 0) {
        ...
        if (mime_type_desc[data->supported_types[idx]].capabilities &
          ...
          fr_window_set_encrypt_header(
              data->window, gtk_toggle_button_get_active(GET_TOGGLE_BUTTON(
                                "a_encrypt_header_checkbutton")));
      }
    }
  }
...
static void help_clicked_cb(GtkWidget *widget, DialogData *data) {
  show_help_dialog(GET_WINDOW("dialog"), "engrampa-fmgr-add");
}

static void add_clicked_cb(GtkWidget *widget, DialogData *data) {
  ...
  archive_name = g_uri_escape_string(
      gtk_entry_get_text(GET_ENTRY("a_add_to_entry")), NULL, FALSE);
  ...
  if ((archive_name == NULL) || (*archive_name == '\0')) {
    ...
    d = _gtk_error_dialog_new(GTK_WINDOW(window),
                              ...
                              _("Could not create the archive"), "%s",
    ...
    d = _gtk_error_dialog_new(
        ...
        _("The name \"%s\" is not valid because it cannot contain the "
          "characters: %s\n\n%s"),
        utf8_name, BAD_CHARS, _("Please use a different name."));
    gtk_dialog_run(GTK_DIALOG(d));
    gtk_widget_destroy(GTK_WIDGET(d));
  ...
  archive_dir = gtk_file_chooser_get_uri(
  ...
  if (!check_permissions(archive_dir, R_OK | W_OK | X_OK)) {
    ...
    d = _gtk_error_dialog_new(
        GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, NULL,
        _("Could not create the archive"), "%s",
        _("You don't have the right permissions to create an archive in the "
          "destination folder."));
    gtk_dialog_run(GTK_DIALOG(d));
    gtk_widget_destroy(GTK_WIDGET(d));
  ...
  if (!uri_is_dir(archive_dir)) {
    ...
    d = _gtk_message_dialog_new(GET_WINDOW("dialog"), GTK_DIALOG_MODAL,
  ...
  if (do_not_add) {
    ...
    d = _gtk_message_dialog_new(
        GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, "dialog-warning",
        _("Archive not created"), NULL, "gtk-ok", GTK_RESPONSE_OK, NULL);
  ...
  if (uri_exists(archive_file)) {
    ...
    d = _gtk_message_dialog_new(
        GET_WINDOW("dialog"), GTK_DIALOG_MODAL, "dialog-question",
        _("The archive is already present.  Do you want to overwrite it?"),
        NULL, "gtk-no", GTK_RESPONSE_NO, _("_Overwrite"), GTK_RESPONSE_YES,
        NULL);
    ...
    if (r == GTK_RESPONSE_YES) {
      ...
      if (err != NULL) {
        g_warning("Failed to delete file %s: %s", archive_file, err->message);
        g_clear_error(&err);
      }
      g_object_unref(file);
    } else {
...
static void update_sensitivity_for_mime_type(DialogData *data,
  ...
  if (mime_type == NULL) {
    ...
    gtk_toggle_button_set_inconsistent(
        GET_TOGGLE_BUTTON("a_encrypt_header_checkbutton"), TRUE);
  ...
  for (i = 0; mime_type_desc[i].mime_type != NULL; i++) {
    if (strcmp(mime_type_desc[i].mime_type, mime_type) == 0) {
      ...
      gtk_widget_set_sensitive(GET_WIDGET("a_encrypt_header_checkbutton"),
                               sensitive ? has_password : FALSE);
      gtk_toggle_button_set_inconsistent(
          GET_TOGGLE_BUTTON("a_encrypt_header_checkbutton"),
          sensitive ? (!has_password) : TRUE);
...
static void archive_type_combo_box_changed_cb(GtkComboBox *combo_box,
                                              DialogData *data) {
  const char *mime_type;
...
static void update_archive_type_combo_box_from_ext(DialogData *data,
                                                   const char *ext) {
  int idx = 0;
  int i;
  ...
  for (i = 0; data->supported_types[i] != -1; i++)
    if (strcmp(ext, mime_type_desc[data->supported_types[i]].default_ext) ==
        0) {
      idx = i;
      break;
    }
...
static void update_sensitivity(DialogData *data) {
  const char *password;

  gtk_widget_set_sensitive(
      GET_WIDGET("a_volume_spinbutton"),
      gtk_toggle_button_get_active(GET_TOGGLE_BUTTON("a_volume_checkbutton")));
  ...
  gtk_toggle_button_set_inconsistent(
      GET_TOGGLE_BUTTON("a_encrypt_header_checkbutton"),
      can_encrypt_header ? (!has_password) : TRUE);
  gtk_widget_set_sensitive(GET_WIDGET("a_encrypt_header_checkbutton"),
                           can_encrypt_header ? has_password : FALSE);
}

static void password_entry_notify_text_cb(GObject *object, GParamSpec *spec,
                                          gpointer user_data) {
...
void dlg_batch_add_files(FrWindow *window, GList *file_list) {
  DialogData *data;
  ...
  data->builder = gtk_builder_new_from_resource(
      ENGRAMPA_RESOURCE_UI_PATH G_DIR_SEPARATOR_S "batch-add-files.ui");
  data->window = window;
  data->file_list = file_list;
  data->single_file =
      ((file_list->next == NULL) && uri_is_file((char *)file_list->data));
  ...
  gtk_toggle_button_set_active(
      GET_TOGGLE_BUTTON("a_encrypt_header_checkbutton"),
      g_settings_get_boolean(data->settings_general,
                             PREF_GENERAL_ENCRYPT_HEADER));
  ...
  if (file_list->next == NULL)
    automatic_name = g_uri_unescape_string(
        file_name_from_path((char *)file_list->data), NULL);
  else {
    ...
    if ((automatic_name == NULL) || (automatic_name[0] == '\0')) {
      ...
      automatic_name =
          g_uri_unescape_string(file_name_from_path(first_filename), NULL);
    }
  }
  ...
  if (check_permissions(parent, R_OK | W_OK))
    gtk_file_chooser_set_current_folder_uri(
        GET_FILE_CHOOSER("a_location_filechooserbutton"), parent);
  else
    gtk_file_chooser_set_current_folder_uri(
        GET_FILE_CHOOSER("a_location_filechooserbutton"), get_home_uri());
  g_free(parent);
  ...
  for (i = 0; data->supported_types[i] != -1; i++)
    gtk_combo_box_text_append_text(
        GTK_COMBO_BOX_TEXT(data->archive_type_combo_box),
        mime_type_desc[data->supported_types[i]].default_ext);

  gtk_box_pack_start(GTK_BOX(GET_WIDGET("a_archive_type_box")),
                     data->archive_type_combo_box, TRUE, TRUE, 0);
  gtk_widget_show_all(GET_WIDGET("a_archive_type_box"));

  /* Set the signals handlers. */

  gtk_builder_add_callback_symbols(
      data->builder, "on_dialog_destroy", G_CALLBACK(destroy_cb),
      "on_a_add_button_clicked", G_CALLBACK(add_clicked_cb),
      "on_a_help_button_clicked", G_CALLBACK(help_clicked_cb),
      "on_a_volume_checkbutton_toggled", G_CALLBACK(volume_toggled_cb), NULL);
  ...
  default_ext =
      g_settings_get_string(data->settings, PREF_BATCH_ADD_DEFAULT_EXTENSION);
  update_archive_type_combo_box_from_ext(data, default_ext);
  g_free(default_ext);

  gtk_widget_grab_focus(GET_WIDGET("a_add_to_entry"));
  gtk_editable_select_region(GTK_EDITABLE(GET_WIDGET("a_add_to_entry")), 0, -1);
