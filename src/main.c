...
static const GOptionEntry options[] = {
    ...
    {"add", 'd', 0, G_OPTION_ARG_NONE, &add,
     N_("Add files asking the name of the archive and quit the program"), NULL},
    ...
    {"extract-here", 'h', 0, G_OPTION_ARG_NONE, &extract_here,
     N_("Extract the contents of the archives in the archive folder and quit "
        "the program"),
     NULL},
    ...
    {"force", '\0', 0, G_OPTION_ARG_NONE, &ForceDirectoryCreation,
     N_("Create destination folder without asking confirmation"), NULL},

    {"version", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
     _print_version_and_exit, N_("Show the application's version"), NULL},

    {G_OPTION_REMAINING, 0, 0, G_OPTION_ARG_STRING_ARRAY, &remaining_args, NULL,
     NULL},

    {NULL, 0, 0, G_OPTION_ARG_NONE, NULL, NULL, NULL}};

static void fr_restore_session(EggSMClient *client) {
  GKeyFile *state = NULL;
  guint i;
...
static void prepare_app(void) {
  ...
  if (add_to != NULL) add_to_uri = get_uri_from_command_line(add_to);
  ...
  if ((add_to != NULL) || (add == 1)) { /* Add files to an archive */
    ...
    while ((filename = remaining_args[i++]) != NULL)
      file_list =
          g_list_prepend(file_list, get_uri_from_command_line(filename));
    ...
    while ((archive = remaining_args[i++]) != NULL) {
      ...
      if (extract_here == 1)
        ...
        fr_window_set_batch__extract(FR_WINDOW(window), archive_uri,
                                     extract_to_uri);
      g_free(archive_uri);
    }
    fr_window_append_batch_action(FR_WINDOW(window), FR_BATCH_ACTION_QUIT, NULL,
...
static void fr_save_state(EggSMClient *client, GKeyFile *state,
                          gpointer user_data) {
  ...
  if (application != NULL) {
    ...
    for (window = gtk_application_get_windows(GTK_APPLICATION(application)),
        i = 0;
         window != NULL; window = window->next, i++) {
