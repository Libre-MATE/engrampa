...
gboolean strchrs(const char *str, const char *chars) {
  const char *c;
  for (c = chars; *c != '\0'; c++)
    if (strchr(str, *c) != NULL) return TRUE;
  return FALSE;
}

char *str_substitute(const char *str, const char *from_str,
                     const char *to_str) {
  char **tokens;
  int i;
  GString *gstr;
  ...
  if (from_str == NULL) return g_strdup(str);

  if (strcmp(str, from_str) == 0) return g_strdup(to_str);
...
static char *escape_str_common(const char *str, const char *meta_chars,
  ...
  if (prefix) extra_chars++;
  if (postfix) extra_chars++;
  ...
  while (*s) {
    ...
    if (is_bad && prefix) *t++ = prefix;
    *t++ = *s++;
    if (is_bad && postfix) *t++ = postfix;
  }
  *t = 0;
...
static const char *g_utf8_strstr(const char *haystack, const char *needle) {
  const char *s;
  gsize i;
  ...
  for (i = 0; i <= haystack_len - needle_len; i++) {
    if (strncmp(s, needle, needle_size) == 0) return s;
...
static char **g_utf8_strsplit(const char *string, const char *delimiter,
                              int max_tokens) {
...
static char *g_utf8_strchug(char *string) {
  char *scan;
  gunichar c;
...
static char *g_utf8_strchomp(char *string) {
  char *scan;
  gsize len;
...
gboolean match_regexps(GRegex **regexps, const char *string,
  ...
  if ((regexps == NULL) || (regexps[0] == NULL)) return TRUE;
...
void free_regexps(GRegex **regexps) {
  ...
  for (i = 0; regexps[i] != NULL; i++) g_regex_unref(regexps[i]);
  g_free(regexps);
}
...
char **search_util_get_patterns(const char *pattern_string) {
  ...
  if (pattern_string == NULL) return NULL;
...
GRegex **search_util_get_regexps(const char *pattern_string,
  ...
  for (i = 0; patterns[i] != NULL; i++)
    regexps[i] = g_regex_new(patterns[i], G_REGEX_OPTIMIZE | compile_options,
...
const char *get_static_string(const char *s) {
  ...
  if (!g_hash_table_lookup_extended(static_strings, s, (gpointer *)&result,
                                    NULL)) {
    result = g_strdup(s);
    g_hash_table_insert(static_strings, (gpointer)result, GINT_TO_POINTER(1));
  }
...
const gchar *_g_path_get_file_name(const gchar *file_name) {
  register char *base;
...
const char *_g_path_get_base_name(const char *path, const char *base_dir,
  ...
  if (junk_paths) return _g_path_get_file_name(path);

  base_dir_len = strlen(base_dir);
  if (strlen(path) < base_dir_len) return NULL;
