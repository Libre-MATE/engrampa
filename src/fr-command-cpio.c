...
static void fr_command_cpio_list(FrCommand *comm) {
  fr_process_set_out_line_func(comm->process, list__process_line, comm);

  fr_process_begin_command(comm->process, "sh");
  fr_process_add_arg(comm->process, "-c");
  fr_process_add_arg_concat(comm->process, CPIO_PATH " -itv < ",
                            comm->e_filename, NULL);
  fr_process_end_command(comm->process);
  fr_process_start(comm->process);
}

static void fr_command_cpio_extract(FrCommand *comm, const char *from_file,
...
static const char **fr_command_cpio_get_mime_types(FrCommand *comm) {
  return cpio_mime_type;
}

static FrCommandCaps fr_command_cpio_get_capabilities(FrCommand *comm,
...
static const char *fr_command_cpio_get_packages(FrCommand *comm,
                                                const char *mime_type) {
...
static void fr_command_cpio_init(FrCommand *comm) {
...
GType fr_command_cpio_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandCpioClass),
                           NULL,
                           NULL,
    ...
    type =
        g_type_register_static(FR_TYPE_COMMAND, "FRCommandCpio", &type_info, 0);
  }
