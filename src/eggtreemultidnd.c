...
static GtkTargetEntry target_table[] = {{"XdndDirectSave0", 0, 0},
                                        {"XdndEngrampa0", 0, 1}};
...
GType egg_tree_multi_drag_source_get_type(void) {
  ...
  if (!our_type) {
    ...
    our_type = g_type_register_static(G_TYPE_INTERFACE,
                                      "EggTreeMultiDragSource", &our_info, 0);
  }
 ...
 **/
gboolean egg_tree_multi_drag_source_row_draggable(
...
gboolean egg_tree_multi_drag_source_drag_data_get(
    EggTreeMultiDragSource *drag_source, GdkDragContext *context,
    GtkSelectionData *selection_data, GList *path_list) {
  EggTreeMultiDragSourceIface *iface =
      EGG_TREE_MULTI_DRAG_SOURCE_GET_IFACE(drag_source);
  ...
  return (*iface->drag_data_get)(drag_source, context, selection_data,
                                 path_list);
}
...
static void selection_foreach(GtkTreeModel *model, GtkTreePath *path,
  ...
  *list_ptr =
      g_list_prepend(*list_ptr, gtk_tree_row_reference_new(model, path));
}
...
static gboolean egg_tree_multi_drag_drag_data_get(
  ...
  if (!EGG_IS_TREE_MULTI_DRAG_SOURCE(model)) return FALSE;

  return egg_tree_multi_drag_source_drag_data_get(
      EGG_TREE_MULTI_DRAG_SOURCE(model), context, selection_data, path_list);
}

static gboolean egg_tree_multi_drag_motion_event(GtkWidget *widget,
  ...
  if (gtk_drag_check_threshold(widget, priv_data->x, priv_data->y,
    ...
    if (egg_tree_multi_drag_source_row_draggable(
            EGG_TREE_MULTI_DRAG_SOURCE(model), path_list)) {
      ...
      context = gtk_drag_begin_with_coordinates(
          widget, target_list, GDK_ACTION_COPY, priv_data->pressed_button,
          (GdkEvent *)event, (gint)event->x, (gint)event->y);
      set_context_data(context, path_list);

      if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), priv_data->x,
                                        priv_data->y, &tree_path, NULL, &cell_x,
                                        &cell_y)) {
        cairo_surface_t *drag_icon;

        drag_icon = gtk_tree_view_create_row_drag_icon(GTK_TREE_VIEW(widget),
                                                       tree_path);
...
static gboolean egg_tree_multi_drag_button_press_event(GtkWidget *widget,
                                                       GdkEventButton *event,
                                                       gpointer data) {
  GtkTreeView *tree_view;
  ...
  if (priv_data == NULL) {
    ...
    priv_data->pending_event = FALSE;
    g_object_set_data(G_OBJECT(tree_view), EGG_TREE_MULTI_DND_STRING,
                      priv_data);
  }

  if (g_slist_find(priv_data->event_list, event)) return FALSE;
  ...
  if (path) {
    ...
    if (gtk_tree_selection_path_is_selected(selection, path)) {
      ...
      if (!call_parent)
        priv_data->event_list = g_slist_append(
            priv_data->event_list, gdk_event_copy((GdkEvent *)event));
