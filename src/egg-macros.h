...
#define EGG_BOILERPLATE(type, type_as_function, corba_type, parent_type,      \
                        parent_type_macro, register_type_macro)               \
  static void type_as_function##_class_init(type##Class *klass);              \
  static void type_as_function##_instance_init(type *object);                 \
  static parent_type##Class *parent_class = NULL;                             \
  static void type_as_function##_class_init_trampoline(gpointer klass,        \
                                                       gpointer data) {       \
  ...
  GType type_as_function##_get_type(void) {                                   \
    ...
    if (object_type == 0) {                                                   \
      ...
      object_type = register_type_macro(type, type_as_function, corba_type,   \
...
#define EGG_CALL_PARENT(parent_class_cast, name, args) \
  ((parent_class_cast(parent_class)->name != NULL)     \
       ? parent_class_cast(parent_class)->name args    \
       : (void)0)

/* Same as above, but in case there is no implementation, it evaluates
 * to def_return */
#define EGG_CALL_PARENT_WITH_DEFAULT(parent_class_cast, name, args, \
                                     def_return)                    \
  ((parent_class_cast(parent_class)->name != NULL)                  \
       ? parent_class_cast(parent_class)->name args                 \
       : def_return)
...
#define EGG_CALL_VIRTUAL(object, get_class_cast, method, args)            \
  (get_class_cast(object)->method ? (*get_class_cast(object)->method)args \
                                  : (void)0)

/* Call a virtual method with default */
#define EGG_CALL_VIRTUAL_WITH_DEFAULT(object, get_class_cast, method, args, \
                                      default)                              \
  (get_class_cast(object)->method ? (*get_class_cast(object)->method)args   \
                                  : default)

#define EGG_CLASS_BOILERPLATE(type, type_as_function, parent_type, \
                              parent_type_macro)                   \
  EGG_BOILERPLATE(type, type_as_function, type, parent_type,       \
                  parent_type_macro, EGG_REGISTER_TYPE)

#define EGG_REGISTER_TYPE(type, type_as_function, corba_type, parent_type, \
                          parent_type_macro)                               \
...
#define _EGG_DEFINE_BOXED_TYPE_EXTENDED_BEGIN(TypeName, type_name, boxed_copy, \
                                              boxed_free)                      \
                                                                               \
  GType type_name##_get_type(void) {                                           \
    ...
    if (g_once_init_enter(&g_define_type_id__volatile)) {                      \
      GType g_define_type_id = g_boxed_type_register_static(                   \
          g_intern_static_string(#TypeName), boxed_copy, boxed_free);          \
      { /* custom code follows */
...
#define EGG_IS_POSITIVE_RESPONSE(response_id)                                  \
  ((response_id) == GTK_RESPONSE_ACCEPT || (response_id) == GTK_RESPONSE_OK || \
   (response_id) == GTK_RESPONSE_YES || (response_id) == GTK_RESPONSE_APPLY)

#define EGG_IS_NEGATIVE_RESPONSE(response_id) \
  ((response_id) == GTK_RESPONSE_REJECT ||    \
   (response_id) == GTK_RESPONSE_CANCEL || (response_id) == GTK_RESPONSE_NO)
