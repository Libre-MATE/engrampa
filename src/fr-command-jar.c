...
static void fr_command_jar_add(FrCommand *comm, const char *from_file,
  ...
  for (scan = jardata_list; scan; scan = scan->next) {
    ...
    if ((retval != -1) || (errno == EEXIST))
      jar_list = g_list_append(
          jar_list, g_build_filename(jdata->package_minus_one_level,
                                     jdata->link_name, jdata->filename, NULL));
...
const char *jar_mime_type[] = {"application/x-java-archive", NULL};

static const char **fr_command_jar_get_mime_types(FrCommand *comm) {
  return jar_mime_type;
}

static FrCommandCaps fr_command_jar_get_capabilities(FrCommand *comm,
...
static const char *fr_command_jar_get_packages(FrCommand *comm,
                                               const char *mime_type) {
...
static void fr_command_jar_init(FrCommand *comm) {}
...
GType fr_command_jar_get_type() {
  static GType type = 0;

  if (!type) {
    GTypeInfo type_info = {sizeof(FrCommandJarClass),
                           NULL,
                           NULL,
    ...
    type = g_type_register_static(FR_TYPE_COMMAND_ZIP, "FRCommandJar",
                                  &type_info, 0);
  }
